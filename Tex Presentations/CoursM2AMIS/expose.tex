...
\begin{frame}{Cas d’étude : Radio Access Network (RAN) }
...
\begin{frame}{Une communication de bout en bout entre deux terminaux}
\scalebox{0.4}{
...
\begin{frame}{Modèle: Un "réseau routé"}
Représentation de l'aller et retour des messages
 \begin{center}
...
\end{center}
\end{frame}
  \begin{frame}{Modèle: dynamique des messages}
...
\begin{center}
 ...
 \vspace{0.5cm}
     \only<2>{
  Tous les $P$ tics, un message de taille $\tau$ est envoyé par chaque RRH.
$P$ et $\tau$ sont toujours les mêmes.

 Ce processus est \textcolor{red}{periodique} : chaque message est envoyé dans chaque période à la même date, appelée \textcolor{blue}{décalage}.}

   \only<3->{
\begin{definition}
\centering
Charge = $\frac{\text{Taille du message}}{\text{Periode}} \pause = \frac{3\tau}{P}$ 
\end{definition}
}
\end{frame}
...
Il y a une \textcolor{blue}{collision} entre deux messages quand ils arrivent au même moment dans un sommet commun.
\end{definition}

\pause
\centering
 \textcolor{red}{La périodicité doit être prise en compte.}
...
\vspace{1cm}
\begin{center}
Il faut choisir le décalage de façon à ce qu'il n'y ait pas de collisions.
\end{center}
\begin{definition}
Une \textcolor{blue}{affectation} est un choix d'offset pour chaque message.
\end{definition}
\end{frame}

\begin{frame}{Un premier problème}
\only<1>{
 \begin{center}
...
\pause
 \begin{exampleblock}{Periodic Assignment for Zero Latency (PAZL)}
  Étant donné un réseau routé, trouver une affectation sans collisions dans $c_1$ et dans $c_2$.
 \end{exampleblock}
\end{frame}
...
Exercice : Montrer que PAZL est NP-Complet.
\end{frame}
