...
\item  Physical Delay of a link $\rightarrow$ Weight of the arcs
...
Find an assignment such that the \textbf{deadline} constraint is satisfied for each route.

\textbf{PAZL (variant):}
No waiting times in the BBUs: Solutions for PAZL are a subset of optimal solutions for PALL.
\end{frame}
...
\only<3>\draw[red] (10,2) circle (8ex);
\end{tikzpicture}
}
...
\pause
We choose a buffering time for each message in every contention points of the same level. 
