...
\begin{itemize}
	\item Exsitence of a solution for medarate loads using polynomial time algorithms.
...
\begin{frame}{Solving WTA: Polynomial time algorithms}
   ...
   \textbf{MLS} : Polynomial time scheduling algorithm finding a solution minimizing makespawn. 
   \pause
   ...
   \textbf{PMLS} 
\begin{center}
   \includegraphics[width=0.7\textwidth]{pmls.pdf}
\pause
\includegraphics[width=0.7\textwidth]{pmls2.pdf}
\end{center}
 

   \textbf{ASPMLS} FPT-Algorithm based on PMLS $\rightarrow$ Always find the optimal solution when parametrized by the number of routes.
...
\begin{center}
  ...
  Construction of a compact representation from an assignment.
\pause
\begin{itemize}
...
\end{center}
\begin{itemize}
  \item The assignment built by a compact representation is \textcolo{red}{minimal}.
  \pause
  \item Helps to reduce the number of solutions to explore for an FPT algorithm with the number of routes and the contention depth as parameters.
  \end{itemize}
\end{frame}
...
\pause
   \textbf{Branch and Bound algorithm} optimized FPT algorithm.
