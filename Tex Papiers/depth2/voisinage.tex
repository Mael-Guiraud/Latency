...
In order to lighten the number of experiences presented here, we consider that the number of datacenter is set to $2$, and the number of contention points of level $1$ (and $3$, since the network is symetric) is set to $4$. The results are not significantly impacted if we change those values. 
Also, in a C-RAN context, the number of route is low. In the network we study, there is $6$ routes on the graph. This kind of graphs allows us to compare the FPT algorithm to the other algorithms. We will study the impact of the numer of routes in the graph at the end of this section.
...
Figure~\ref{tab:instances} shows the average additional latency needed by the branch and bound and the GNS for the different ways to draw the length of the arcs. Those values are draw for $1000$ instances of each kind.
...
\hline
    ...
    \hline
    Branch and bound & $4273$ & $2704$& $222$ \\
 
    GNS & $12177$ & $9648$& $10628$\\
 ...
 \end{figure}
 \end{center}
 The objective is too see the instances in which a solution given GNS can be the most improoved by the local search heuristics. As observed, drawing the size of the arcs betwenn $P-0.1\times P$ and $P$, the lower bound (found by the branch and bound algorithm) is clearly lower than with the other kinds of instances while the additional latency needed by GNS remains in the same order of magnitude.
 
 Thus, for the following experiences, we will generate instances using all the synthethic datas described here.
 
\todo{definition de load et additional lattency ici ? + mettre la partie sur les greedy ici} 
