...
\end{figure} 
 ...
 \subsection{Dynamic of Datagrams Transmissions}
 		...
 		In this article, we consider a discretized time. The unit of time is called a {\bf tic}. This is the time needed to send an atomic data in a link of the network. We assume that the speed of the links is the same over all the network. In practice, the size of an atomic data is usually 64 bits, the speed of the links is 10Gbps, and the length of a tic is thus about $6.4$ nanoseconds. The authors are developing a prototype basted on ethernet base-X~\cite{ieee_8023}, which corresponds to this values.
...
%   In the application we study the full transmission time is bounded . We define $Tmax$ such that $TR(A) \leq Tmax$.
  ...
  %Remark that the routed network induced by such topologies are symmetric. Pas vraiment
    ...
    Several BBUs are gathered in one or several datacenters. The length of the links between the entrance of the datacenter and all BBUs may or may not be the same. In the first case, once the messages have been scheduled to go in the datacenter, they go out in the exact same order and thus, even if all the routes uses the same node in the way back, it is not considerd as a contention point, as illustrated in figure~\ref{fig:allerretour}. In the case the length of the links into the datacenter are different, the contention point before the datacenter is also a contention point at the exit of the datacenter and the routes are symmetrical.
...
\caption{The routed network modelizing both ways of messages in the fronthaul network of figure~\ref{fig:fronthaul}.}
\label{fig:allerretour}
\end{figure}

In the problem we describe in next subsection, the routing is given. It is considered to be coherent~\cite{schwiebert1996necessary}, that is, acyclic. Thus, the routed network studied in this paper are some directed acyclic graphs.
...
is equivalent. 
By removing the periodicity of the problem, solving \spall on a single contention point is a single processor scheduling problem: Given a set of jobs of same length with each a release time and a deadline, we want to find a scheduling, that is a time for each jobs such that no two jobs uses the machine at the same time. Each jobs must be scheduled after its release time and satisfy its deadline. The makespan of a solution is the time elapsed between the begining of the computation of the first job and the end of the last job.
      The paper~\cite{simons_fast_1978} gives a polynomial time algorithms that find a scheduling that minimize the makespan in $O(n^2\log(n))$.
      

We now want to solve the same problem in which the process is periodic. It has already been defined as a useful subproblem in~\cite{barth2018deterministic} under the name \bra. Here, the messages are the jobs, the releases times are given by the length of the routes, and the deadline of each jobs is computed from $TR(A)$.\\
...
\end{theorem}
\begin{proof}
The two flow shop problem studied in \cite{yu2004minimizing} is show as $\NP$-hard. In two flow shop problem, a set of jobs with have to be processed in sequence on two machines; a job must be processed on machine $1$ before beeing processed on machine $2$. There is, for each job, a delai between the end of the processing on the first machine and the begining of the processing on the second one. The objective is to minimize the makespan.\\
...
The contention vertices are sorted by contention level, and the contention levels are computed in ascending order. Each assignment on contention vertices of the same contention level is computed independently. The \greedydeadline algorithm consists in selecting all routes of arrival time less than the current time and among them,
...
We present here a variant of \greedydeadline: select as first datagram the one with minimal $t(r,u)$, then select the datagrams by lowest normalised arrival times instead of arrival times. We will show that in practice, this algorithm, called \greedynormalized performs better than \greedydeadline.

However, \greedydeadline and \greedynormalized may fail to give a compact assignment for all routed networks. The way we select departure times for the routes can make sequences of times of size less than $\tau$ unusable for unscheduled routes. If too much time is wasted, 
...
In this paper, we focus on Cloud-RAN networks. For the results presented here, we
focus on networks of contention depth three, as illustrated in figure~\ref{fig:randomnetworks}, in which the dotted arcs represents each the arcs of two routes. As explained is section~\ref{sec:contentiondepth}, there is only one contention point due to the datacenters.
   \begin{figure} 
	\centering
...
To generate random routed networks, several parameters might be fixed: The load of the network, the number of routes, the length of the routes, the distribution of the length of the routes, and the number of contention points.  We first want to determine the impact in terms of computation time and quality of the results of those parameters on the algorithms studied.
In order to lighten the number of experiences presented here, we fix the number of contention point as in figure~\ref{fig:randomnetworks}. The results are not significantly impacted if we change those values. 
 The impact of the load on the quality of the results was investigated: When the load is increased, the performances of the local search algorithm does not changes significantly. Otherwise, we choose to fix the load to $80\%$, which is, in practice, an already high load. This means that $P = \frac{\tau \times n}{0.8} $, whith $n$ the number of routes on the graph.
 
In a C-RAN context, the number of route is low. In the network we study, there is $8$ routes on the graph. This kind of graphs with few routes allows us to use the branch and boud algorithm as a lower bound for evaluating the performances of the other algorithms. We will study the impact of the number of routes in the graph at the end of this section. The lenght of the routes is drawn uniformly between $0$ and $P$. This allows the instances to challenge the robustness to the periodicity of our algorithms.
...
Figure~\ref{fig:descente07} shows the difference between initalizing the hill climbing with \hybridgreedynormalized, or with one or several random compact representation. Those resultas are drawn on $100$ random instances, with $80\%$ of load.
\begin{figure}[h]
	\centering
