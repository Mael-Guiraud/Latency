...
%   In the application we study the full transmission time is bounded . We define $Tmax$ such that $TR(A) \leq Tmax$.
  ...
  \subsection{Periodic sending of the datagrams}
    ...
    Let us call $[r,u]_{P,\tau}$ the set of tics used by a datagram on $r$ at vertex $u$ in a period $P$, that is $[r,u]_{P,\tau} = \{s(r,u) + i \mod P \mid 0 \leq i < \tau \}$. 
      ...
      \noindent {\bf Decision problem:} is there a valid assignment $A$ of $(G,{\cal R})$ such that $ TR(A) \leq Tmax$ ?
...
TODO: généraliser à un réseau quelconque.


\begin{lemma}
Let $CA$ be a realizable compact assignment, then $Real(A)$ is a valid assignment.
\end{lemma}
\begin{proof}
TODO
\end{proof}
...
\begin{lemma}
For all compact assignments $CA$, $CR(Real(CA)) = CA$.
\end{lemma}
\begin{proof}
...
\end{lemma}
\begin{proof}
TODO: utile pour le théorème suivant
\end{proof}

Notion d'assignement canoniques (images de compact, ça veut juste dire valide + contrainte sur les buffers (peut être introduit avant))
