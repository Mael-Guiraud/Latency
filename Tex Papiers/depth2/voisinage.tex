...
is equivalent. It has already been defined as a useful subproblem in~\cite{barth2018deterministic} under the name \bra, which is the periodic version of the problem \ADO defined below.

  \noindent {\bf Assignment for Depth One (\ADO)} 

      \noindent {\bf Input:} A set of jobs with each a release time and a deadline $d_i$ , a size of the jobs.\\
      \noindent {\bf Decision problem:} Find, on a signle machine, a scheduling, that is a time for each jobs such that no two jobs uses the machine at the same time.\\
      \noindent {\bf Optimisation problem:} Minimize the time between the begining of the computation of the first job and the end of the last job.
      \\
       \cite{simons_fast_1978} gives a polynomial time algorithms that find a scheduling that minimize the makespan in $O(n^2\log(n))$. If we add the periodic aspect to \ADO, this problem becomes \bra :
...
\noindent{\bf \bra} 
   ...
   \bra is a periodic version of \ADO in which the messages are the jobs, the releases times are given by the length of the routes, and the deadline of each jobs is computed from $TR(A)$.\\
   
   \begin{theorem}\label{th:braFPT}
$\bra \in \FPT$ when parametrized by the number of routes.
\end{theorem}
   The proof of this theorem is explained in~\cite{barth2018deterministic}. There is still the open question: is \bra solvable in polynomial time.
...
the two flow shop problem studied in \cite{yu2004minimizing}. \todo{mettre sous forme théorème et dans la preuve montrer pourquoi la périodicité ne change rien: trivial, juste mettre une période plus grande que la latence possible}
