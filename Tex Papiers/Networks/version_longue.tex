...
\end{figure} 
    ...
    we denote $[r,u]_{P,\tau}$ by $[r,u]$ and say that $A$ is a valid assignment. 
     ...
     In the same example, by taking $P=5$ and $\tau = 2$, there is a no solution to the problem defined above better than the optimal one without waiting time. Indeed, the optimal solution is the assignment $(0,0),(1,2),(0,0))$.
...
\end{tikzpicture}
  	...
  	\end{center}
    \caption{A routed network with $((0,0),(0,0),(0,0))$ as a $(2,1)$-periodic assignment and $(0,0),(1,2),(0,0))$ as a $(5,2)$-periodic assignment}
    \label{fig:example}
\end{figure}
...
\end{proof}
   ...
   \begin{tabularx}{\textwidth}{|c|X|}
	...
	In this section, we define a family of simple routed networks modeling a Point-to-Multipoint fronthaul (PtMP, see figure~\ref{fig:ptmp}), which has been designed for C-RAN \cite{tayq2017real}. Let $N = (\cal{R},\,\cal{B},\,\omega)$ be a routed network, we say it is a \textbf{star routed network} if and only if the routes are $\{r_0,\dots,r_{n-1}\}$, $r_i$ is $(s_i,c_1,c_2,t_i)$ and ${\cal B} = \{ c_2 \}$ (datagrams can wait in $c_2$). Star routed networks have contention depth two but a maximal contention width of $n$. The load on each of the two contention points is thus $n\tau / P$.
   ...
   \begin{minipage}{0.40\linewidth}
        ...
        \captionof{figure}{A point to multipoint fronthaul network}
        \label{fig:ptmp}
   \end{minipage}\hfill
...
\end{tikzpicture}
  ...
  \captionof{figure}{A star routed network }
\label{fig:star}
\end{minipage}
...
\begin{figure}
\begin{center}
Graph of figure~\ref{fig:star}

$P= 5$, $\tau = 1$, $d_1 = 30$, $d_2 = 34$, $d_3 = 32$
...
\path (s2) edge [->] node[anchor=south]{$0$} (c1);
\path (s3) edge [->] node[anchor=south]{$0$} (c1);
\path (c2) edge [->] node[anchor=south]{$10$} (s2p);
\path (c2) edge [->] node[anchor=south]{$14$} (s3p);

\path (c2) edge [->] node[anchor=south]{$12$} (s1p);

\path (c1) edge [->] node[anchor=south]{$15$} (c2);

\path (c1) edge [->,bend left=30] node[anchor=south]{$11$} (c2);
\path (c1) edge [->,bend right=30] node[anchor=north]{$9$} (c2);
   \node[circle] (buf) at (10, 0.5) {$\cal{B}$};
   \draw[dashed] (10,2) ellipse  (0.6cm and 1cm);
  %\draw[->,line width=0.5pt] (5,2.51) parabola bend (7.5,3.5) (10,2.51);
 %\draw[->,line width=0.5pt] (5,1.49) parabola bend (7.5,0.5) (10,1.49);
...
\path (s2) edge [->] node[anchor=south]{$0$} (c1);
\path (s3) edge [->] node[anchor=south]{$0$} (c1);
\path (c2) edge [->] node[anchor=south]{$0$} (s2p);
\path (c2) edge [->] node[anchor=south]{$0$} (s3p);

\path (c2) edge [->] node[anchor=south]{$0$} (s1p);
...
\path (c1) edge [->] node[anchor=south]{$0$} (c2);

\path (c1) edge [->,bend left=30] node[anchor=south]{$4$} (c2);
\path (c1) edge [->,bend right=30] node[anchor=north]{$4$} (c2);
   \node[circle] (buf) at (10, 0.5) {$\cal{B}$};
   \draw[dashed] (10,2) ellipse  (0.6cm and 1cm);
  %\draw[->,line width=0.5pt] (5,2.51) parabola bend (7.5,3.5) (10,2.51);
 %\draw[->,line width=0.5pt] (5,1.49) parabola bend (7.5,0.5) (10,1.49);
...
$P= 5$, $\tau = 1$, $d_1 = 4$, $d_2 = 6$, $d_3 = 6$
 
\end{center}

\caption{The cannonical network of figure~\ref{fig:star}}
\label{fig:canonical}
\end{figure}
...
We say that that $A$ is \textbf{compact} if there is a route $r_0 \in \cal{R}$ such that the following holds: for all subsets $S\subset \cal{R}$ with $r_0 \notin S$, the bufferless assignment $A'$, defined by $A'(r) = A(r) - 1 \mod P$ if $r \in S$ and $A(r)$ otherwise, is not valid. In other words, an assignment is compact if for all routes $r$ but one, $A(r)$ cannot be reduced by one, that is either in $c_1$ or in $c_2$, there is a route $r'$ using the tics just before $A(r)$. See Fig.~\ref{fig:compact} for an example of a compact assignment, obtained by the procedure of the next proposition. 
  \begin{figure}
      ...
      \end{center}
      \caption{A compact assignment $A'$ built from an assignment $A$}
      \label{fig:compact}
      \end{figure}
