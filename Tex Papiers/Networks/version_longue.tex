...
\newcommand\EPMLS{\texttt{EPMLS}\xspace}
...
%      \STATE  deadline[$r_i$]  $\leftarrow$  $m_{i} + T_{max} - \Omega(s_i,c_s)$
     ...
     \textbf{TODO:} Donner un exemple de cet algo.



    The complexity of \greedydeadline is in $O(n\log(n))$, using the proper data structures. The set of routes $U$ must be maintained in a binary heap
    ...
    of time during which a datagram can go through $c_2$. Each time the waiting time of a route is fixed, an interval is split into at most two intervals in constant time. During the whole algorithm, each element of this list is used at most twice
    either when doing an insertion or looking for the next free interval. Hence, the time needed to maintain the list is in $O(n)$. 
     ...
     The problem \wta is the same as this scheduling problem (but the periodicity).
     The jobs are the routes, the size of a datagram is the running time of a job, 
     and both the deadline and the release time are defined in the same way.
     ...
     Recall that $t(r,c_2)$ is the time at which the message of $r$ goes through $c_2$. Let us denote by $t_{min}$ and $t_{max}$ the smallest and largest value of the $t(r_i,c_2)$ for all $i \in[n]$. When \MLS finds an assignment $A$, it always satisfies $PT(r) < d(r)$ for all $r$. Let us assume that $t_{max}- t_{min} \leq P -\tau $, then all datagrams go through $c_2$ in a interval of time less than $P$. Hence, when we compute potential collisions modulo $P$, all their 
     relative position stay the same and the correction of the used scheduled 
     algorithm guarantes that there is no collision. However, if $t_{max}- t_{min} > P -\tau $, then computing the $t(r_i,c_2)$ modulo $P$ could reveal somme collision. Since the scheduling algorithm minimizes $t_{max}$, it tends to find 
     small values for $t_{min} - t_{max}$ and \PMLS may succeed in finding a valid assignment (as shown in Sec.~\ref{sec:resultsPALL}), but not for all instances. 
     ...
     Algorithm \PMLS runs, for each route $r \in \cal{R}$, the algorithm \MLS on an instance defined as follows. 
     Let $RT(r)$ be the release time of $r$, subtract it to all the release times and deadlines of the other routes. Therefore $RT(r)$ is zero in this instance
     and the waiting time $w_r$ is set to zero, so that the datagram of $r$ goes through $c_2$ at time $0$. Then, as in Prop.~\ref{prop:canonical}, the instance is modified so that all release times are in $[P-\tau]$. Each release time $RT(r_i)$ is replaced by $RT(r_i) \mod P$ and $d(r_i) = d(r_i) - (RT(r_i) - RT(r_i) \mod P)$. Furthermore, if the release time of a route $r$ is between $P-\tau$ and $P$, we set it to $0$ and $d(r) = d(r) - P$.  The deadline of each route is set to the minimum of its deadline and $P - \tau$. Hence, by design, if \MLS finds a solution for such a modified instance, we have $t_{max}- t_{min} \leq P -\tau $. Therefore, the assignment is valid.
     ...
     $r$ to be one. Since at least one route has waiting time zero if there is some valid assignment, if \MLS finds an assignment \PMLS also finds one. Algorithm \MLS is used at most $n$ times, thus the complexity of \PMLS is in $O(n^3\log(n))$. Note that \PMLS is a heuristic and may fail if for the $n$ modified instances
     there is no solution using only an interval of time of size $P$ in $c_2$. 


     \todo{reprendre -> bien dire que la construction garantit tmin = 0.
     Donc, on va grace a MLS toujours trouver ce qu'il faut ? En tout cas simplifier tmax - tmin en tmax}
...
$\wta \in \FPT$ when parametrized by the number of routes.
\end{theorem}
\begin{proof}
