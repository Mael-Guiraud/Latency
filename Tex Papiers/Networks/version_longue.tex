...
Let $[n]$ denote the interval of $n$ integers $\{0,\dots,n-1\}$.
	...
	\subsection{Routes and Contention Points}
  	...
  	We study a communication network constituted of pairs of vertices between which messages are sent periodically. The routing between each pair of such nodes is represented by \textbf{route}, a sequence of vertices $r=(s, c_1, \ldots, c_l, t)$, such that no vertex appears twice in the sequence (there is no loop in a route). Each vertex $c_i$ represents a contention point, which is the beginning of a link of the communication network shared by several routes. Hence, all vertices of $r$ appear in several routes, except $s$, the first vertex of $r$, and $t$ its last vertex, which are exclusive to $r$ and represent the source and the target of the message. When modeling a C-RAN network, the first vertex represents the sending of the message by the RRH and the last vertex represents the same RRH that receives the answer, sent back by the BBU (see Figure~\ref{fig:routeexample}).
...
\begin{examplee}
Consider the route $r$ of Figure~\ref{fig:routeexample} and an assignment for this route such that $o_r=2$ and $w_r = 6$. We can thus compute the arrival time of a datagram in each vertices of the route: that is $t(r,c_1) =  \lambda(r,c_1) + o_r =  3 + 2 = 5$, $t(r,c_2) = \lambda(r,c_2) + o_r = 12$ and  $t(r,t) =\lambda(r,t) + o_r + w_r  = 22$.
\end{examplee}
...
\end{figure}
	...
	\subsection{Periodic Assignment for Low Latency}
      ...
      \noindent {\bf Question:} Find the minimum value $TR(A)$ for all $A$ valid $(P,\tau)$-periodic assignment of $N$. 
      \bigskip
      ...
      \noindent {\bf Question:} Does there exist a valid $(P,\tau)$-periodic assignment of $N$ such that for all $r \in {\cal R}$, $TR(r,A) \leq d(r)$?
      \bigskip
...
Finding an assignment in that case is trivial: send all datagrams so that they follow each other without gaps in $c_1$. In the corresponding canonical routed network, one can set $o_i = i\tau$.  Since all arcs $(c_1,c_2)$ are of weight zero in this case, the interval of time used in $c_2$ are the same as for $c_1$ and there is no collision in $c_2$.
	...
	Finally, the complexity is in $O(n)$ since we have to find the maximum of the length of the $n$ routes and the computation of each $w_i$ is done by a constant number of arithmetic operations.
     ...
     For the first two heuristics heuristics, we need to define the \textbf{margin} of a route $r$ in a routed network $N$, with a deadline function $d$ as $ d(r) - \lambda(r)$. The margin is a bound on the waiting time of a route in a valid assignment.
 ...
 \bigskip
      ...
      \noindent {\bf Input:}  A routed network $N=({\cal R},{\cal B},\omega)$, integers $P$ and $\tau$ and a deadline function $d$ and an offset $o_r$ for each route $r \in {\cal R}$.
      
      \noindent {\bf Question:} Does there exist a valid $(P,\tau)$-periodic assignment of $N$ such that for all $r \in {\cal R}$, $TR(r,A) \leq d(r)$ and $A(r) = (o_r,w_r)$?
      \bigskip
...
\begin{center} 
  ...
  \includegraphics[width=0.7\linewidth]{departs_gp_250001_trunc.pdf}
