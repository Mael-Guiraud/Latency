...
% Note that the model is technology agnostic, i.e. it is compatible with an optical network with a fixed packet size. 
 ...
 In current cellular networkw, the RRHs are synchronized, and simulating our settings would cost a large additional latency, since datagrams should be buffered in their RRHs to honor their offsets. 
Hence, the model we propose should be seen as a suggestion to design future cellular networks, where emissions of the RRHs are not synchronized. It could already used to modelize sensor networks in cars, logistic problems in production lines or multiprocessor systems, where we need to schedule periodic messages (or goods) over a bus and where we have a better handle on when they are generated.
...
\subsection*{Outline}
 ...
 In Section~\ref{sec:def}, we propose a model of the network and the periodic sending of datagrams along its routes.  We then introduce \pall to formalize the problem of sending periodic messages in a network without collision and its variant \pazl, with all waiting times zero. In Section~\ref{sec:complexity}, we prove that both \pazl and \pall are $\NP$-hard for very restricted classes of graphs, and that their optimization counterparts are hard to approximate. 

In Section~\ref{sec:star_routed_network}, we introduce a simple but very common topology, called the star routed network, with a single shared link, that is studied in the rest of the article. In Section~\ref{sec:PAZL}, we study the problem \pazl. We provide polynomial time algorithms which works when the load or the routes are small, and an algorithm exponential in the number of routes only. We use them to provide experimental evidences that \pazl can be solved positively when the network is not too loaded or when the routes are small. In Section~\ref{sec:PALL}, we propose polynomial time heuristics and an exact FPT algorithm for the general \pall problem and experimentally show that they work well, even in extremely loaded networks. 
...
Let $[n]$ denote the interval of $n$ integers $\{0,\dots,n-1\}$.
	...
	\subsection{The C-RAN Network and its Contention Points}
  	...
  	A {\bf route} $r$ in $G$ is a directed path, that is, a sequence of adjacent vertices $u_0, \ldots , u_{l}$, with $(u_i,u_{i+1}) \in A$. A route represent the path of a message in the network, hence $u_0 \in \cal{S}$ represent the sending by the RRH, $u_l \in \cal{D}$ represents the reception of the answer in the same RRH. On each route, there is exactly one vertice in ${\cal C}$, which is the next contention point after the message leaves the BBU. We denote the set of these contention point by ${\cal B}$.
	...
	We denote by ${\cal R}$ a set of routes of the weighted multigraph $G$, the pair $(G,{\cal R})$ is called a \textbf{routed network} and represents our telecommunication network. When a routed network is given, we always assume it comes with the weight function $\omega$ and the set of BBUs vertices $\cal{B}$.
...
\end{proof}
   ...
   \begin{tabularx}{\textwidth}{|c|X|}
    \hline
     ...
     \hline
      $\omega(a)$ & Weight of the arc $a \in A$ \\
      \hline
    ...
    \hline
      ...
      \end{center}
		...
		We call \textbf{star routed network} $(G, \cal{R})$ if and only if $G$ has only two contention points $c_1$ and $c_2$, the route $r_i$ is $(s_i,c_1,c_2,t_i)$ and ${\cal B} = \{ c_2 \}$ (datagrams can wait in $c_2$). Star routed network have contention depth two but a maximal contention width.
		...
		The two contention points, $c_1$ and $c_2$, model the beginning of the shared link used from the RRHs to the BBUs, and the other end of the shared link used in the other direction. 
		The computation in the BBU of an answer to a datagram on the route $r$ takes some times.
		In the star routed network, this time is encoded in the weight of the arc between $c_1$ and $c_2$ in $r$. This weight is the time needed to go through the shared link, then to arrive at the BBU, plus the computation time and the time to return to the shared link, see Fig.~\ref{fig:star}.
...
\end{tikzpicture}
  ...
  When solving \pall or \pazl on a star routed network, we can do several assumptions on the parameters of the star routed network  without loss of generality. We say that a star routed network is \textbf{canonical} (for a given period $P$) if the weights of the arcs between $c_1$ and $c_2$ ar in $[P]$ and the other are zero. Hence, $\lambda(r_i)$, the length of a route is equal to the length ot its arc between $c_1$ and $c_2$. Moreover, $\lambda(r_0) = 0$. See Fig.~\ref{fig:canonical} for an example of a canonical star routed network.  
  
  \begin{proposition}\label{prop:canonical}
   Let $I = ((G,{\cal R}), P, \tau , d)$, with $(G,{\cal R})$ a star routed network, then there is 
   $I' = ((G',{\cal R}), P, \tau , d')$, with  $(G',{\cal R})$ a canonical star routed network, such that:
     $$I \in \pall \Leftrightarrow I' \in \pall \text{ and } I \in \pazl \Leftrightarrow I' \in \pazl$$
  \end{proposition}

  \begin{proof}
  Let us define $G'$ and $d'$ from $G$ and $d$ in such a way that there is a bijection 
   between valid assignments of $I$ and $I'$, which proves the proposition.
  
  We set all arcs $(s_i,c_1)$ to zero, then we obtain the bijection between valid assigments of $I$ and $I'$
  by setting $o_i' = o_i + \omega'(s_i,c_1)$ and $d'(i) = d(i) - \omega(s_i,c_1)$.
  The weigths of the arcs $\omega'(c_2,t_i)$ are also set to $0$, it does not change the possible collisions
  for an assignment but it changes the transmission time, hence we set $d'(r_i) = d'(r_i) - \omega(c_2,t_i)$. 

  Let $a_i$ denotes the arc between $c_1$ and $c_2$ for the route $r_i$, we let 
  $\omega'(a_i) = \omega(a_i) \mod P$. Again, it does not change the collisions since computing 
  a possible collision is done modulo $P$. However, we must change $d'$ to be $d'(r_i) = d'(r_i) - \omega(a_i) / P$.

  Finally, we assume w.l.o.g. that $\omega'(a_0)$ is the smallest among the $\omega'(a_i)$.
  We let $\omega'(a_i) = \omega'(a_i) - \omega(a_0)$, which implies that $\omega'(a_0) = 0$.
  All lengths between $c_1$ and $c_2$ change by the same value, hence collisions are not modified.
  We change $d'(r_i)$ to  $d'(r_i) - \omega'(a_0)$.
  \end{proof}

   From now on, we assume that all star routed networks have the properties given in Proposition~\ref{prop:canonical}. To define an instance of \pall, it is enough to give the weights of the arcs 
   $(c_1,c_2)$ for all routes, the period, the message size, and $d$ the deadline function. We need the same
   input for \pazl but $d$.
...
\end{theorem}
 ...
 class of graphs for which \pazl is $\NP$-hard. We could ask the weights to be strictly positive, another restriction which makes more sense in our model, since weights represent the delay of physical links. Then, we can prove $\NP$-completness using the same proof, by setting them to be all of size $d$.
...
\begin{proof}
 ...
 By construction, a contention vertex corresponds to an edge and belong to exactly the two routes corresponding to the vertices of the edge, thus the routed network is of contention width $2$.
...
\section{Assignments with no waiting time} \label{sec:PAZL}
  ...
  we give several simple heuristics and an exact fixed parameter tractable algorithm, in time exponential in the number of routes only. We show in the experiments of Section~\ref{sec:exp_PAZL}, that \pazl can be very often solved positively, in particular for shorter routes and when the load is moderate. This sensibility t the load has been studied in details in a following work~\cite{guiraud2020scheduling}, in which \pazl is solved for higher load using more complex, but polynomial time, algorithms. 
    ...
    We first present a simple policy, which works when the period is large with regard to the lengths of the routes. The datagrams are sent in order from the shortest route to the longest route, without any gap between two datagrams in the contention point $c_1$. We assume the routes $r_i$ are sorted by increasing $\lambda(r_i)$ and we set $o_{r_i}$ the offset of $r_i$ to $i\tau$. We call this algorithm {\bf Shortest-Longest}. By definition, there are no collision in $c_1$ and if the period is long enough, 
     it is easy to see that in $c_2$ the order of the datagrams are the same as in the forward period and that no collision can occur. 
      ...
      If the period is slightly smaller that the bound of Prop.~\ref{prop:SL}, a collision with the $r_0$ on $c_1$. Hence, this policy is not useful even as a heuristic for longer routes as confirmed by the experimental results of Subsection~\ref{sec:exp_PAZL}. 
