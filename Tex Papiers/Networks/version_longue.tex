...
\end{proof}
    ...
    We define the {\bf margin} of an instance as the minimum of the margins of the routes. The margin represents the \emph{logical latency} imposed by the communication process without taking into account the physical length of the network, since it cannot be changed. For a given star routed network, choosing the margin or the deadline is the same. However, to compare different star routed networks with different sizes of routes, the margin is more relevant than the deadline. Hence, in our experiments, we vary the margin, from $0$ to $3,000$.
   	...
   	We first try to understand what is the best choice of heuristics for the first stage of the algorithm. The first stage is followed in this experiment by \greedydeadline, the simplest algorithm to solve \wta. In Figure~\ref{fig:success80} and \ref{fig:success95}, the success rate of all possible first stage heuristicthe to solve \pall is given, function of the margin of the instances. The success rate is an average computed over $10,000$ random star routed networks. For the three heuristics choosing an order at random, we repeat the algorithms
	$1000$ times, that is we draw $1000$ different random orders. The algorithm is considered to succed as soon as there is a valid assignment for one order. Each random order drawn is used for RO, RORS and ROBS to make the comparison fairer.
...
\begin{figure}[h] 
  \centering
    \includegraphics[width=0.5\textwidth]{departs_gp_21000.pdf}
     ...
     First remark that our algorithms often finds assignments withr $95\%$ of load and long routes which was not possible when looking for bufferless assignments (see Sec.~\ref{sec:exp_PAZL}). It justifies the interest of studying \pall and not only \pazl.
...
\section{Periodic Assignments vs Statistical Multiplexing}\label{sec:comparison}
    ...
    The algorithms proposed in this paper are designed to manage deterministic flows on dedicated networks. In this section, the objective is to sutdy the impact of deterministic flows on non-deterministic flows (internet traffic, best effort) when managed by classical approaches like statistical multiplexing or deterministicaly scheduled.
    
    Since the algorithms are not designed to take care about best effort traffics, and more precicely tend to gather all deterministic messages, one can expect that deterministic scheduling worthen the latency of best efforts packets, compared to a classic approach.
    Nevertheless, we propose an adaptation of \PMLS that smooth the latency of C-RAN traffic in order to frequently leave some ressource for best effort traffic.
    ...
    The first one is The First In First Out policy, the trivial one, which consists in sending the messages in the same order in which they reached the contention point.

    The second proposed policy is called Longest Time Elapsed and consists in sending first the message which has almost spent the largest time in the network between all buffered messages
    
    \subsubsection{Adapted \PMLS for BE traffic}
    Since \PMLS always finds an optimal solution for deterministic traffic, we focus on the impact it has on statistical traffic. As mentioned previously, \PMLS tends to create some long sequences of deterministic messages and then, avoid the best effort traffic to go through the nodes during those intervals.
    ...
    The star networks are generated with $8$ routes.
