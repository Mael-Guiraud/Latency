...
Cloud Radio Access Network or C-RAN, have been proposed as a next generation mobile network architecture to reduce energy consumption~\cite{mobile2011c,checko2014cloud} and more generally the total cost of ownership.
...
Our aim is to operate a C-RAN on a low-cost shared switched packet network.
...
% Note that the model is technology agnostic, i.e. it is compatible with an optical network with a fixed packet size. 
 ...
 In this article, we assume that all RRHs send datagrams with the same periodicity of 1ms as required by 5G specifications~\cite{romano2019imt}. Also, it is needed that all the answers be back to the RRHs before a given deadline. This is why we focus on minimizing the datagrams latency. We consider that the datagram of each RRH is available at a different time in the period, given by its offset. In order to not consider the offest as a buffer delay, we make the choice to consider that control and data planes are separated. We work on minimizing the latency on the data plane, while some synchronization mechanism can be applied on the control plane~\cite{khalili2016uplink,yemini2016multiple}. In the case of networks with long routes, the use of Timing Advance is deployed to guarantee the synchronization of the receptions despite the difference in length of the routes in the network~\cite{mahmood2019time}. 
...
Hence, the model we propose can be seen as a suggestion to design future cellular networks, where emissions of the RRHs are not synchronized. It can already model sensor networks in cars, logistic problems in production lines or multiprocessor systems, where periodic messages (or goods) must be scheduled over a bus (or an assembly line), since we have a better handle on when these messages are generated. Also, the results obtained in this paper are applicable for the guarantees of asynchronous deterministic latency necessary for remote control in the context of industry 4.0~\cite{peng2021latency,garcia2019latency}.
 
In this article, we focus on \emph{star routed networks}, which represent a simple but common topology~\cite{electronics9122131,bhattacharjee2020time},
...
additional latency most of the time, eventhough buffering can only be done in the BBU. For more complex networks and synchronized RRHs, studied in a follow-up work~\cite{guiraud2021deterministic}, we need to allow buffering at each node to obtain good sending schemes.
...
One of the solution consists in relying on an almost full optical approach, where each end-point (RRH on one side, BBU on the other side) is connected through direct fiber or full optical switches~\cite{kai2020amplify,tayq2017real}. This architecture is very expensive and hardly scales in the case of a mobile network. As illustrative purpose, a single (one operator) mobile network in France is composed of about $10,000$ base stations. This number will increase by a factor of $2$ to $20$ with the emergence of “small cells” which increases base station density to reach higher throughput \cite{dahlman20185g,romano2019imt}. It is thus needed to find a solution to offer low latency over commoditized packet based networks.  Another approach consists in using optical rings dimentioned for Both fronthaul and best-effort flows~\cite{rommel2020towards,luu2021dynamic}, but this approach rely to network dimentioning and optical multiplexing technologies.
...
Our approach of consists in deterministically managing datagrams in each node in order to prevent contention. This approach may look similar to the concept of Deterministic Networking. A working group from IETF called DetNet~\cite{finn-detnet-architecture-08} works in collaboration with TSN (Time Sensitive Networking)~\cite{ieee802}, a task group of IEEE, to develop technical solutions for deterministic networking~\cite{bhattacharjee2020time,8613095,durr2016no}. Nevertheless, using the term "deterministic" is inappropriate since all works related to DetNet or TSN are still based on statistical models. The latency guarantee given by those approaches are an upper bound on the latency, while we aim to minimize it. In the same spirit, network calculus relies on a statistical model to guarantee bounded latency~\cite{9217707}.

 The model and the algorithms of this paper make the hypothesis that the network components are able to collect some information and send it to a centralized entity, to differentiate several kinds of flow and to forward it at an exact date, imposed by the controller. TSN standards like 802.1Qat Stream Reservation Protocol~\cite{ieee802qat} improoved by IEEE 802.1Qcc~\cite{6755436} propose technical solution for those hypothesis, related to the principle of Software Defined Network (SDN)~\cite{mohamed2021software,li2015software,7356556}.
