...
Let $[n]$ denote the interval of $n$ integers $\{0,\dots,n-1\}$.
    ...
    The {\bf weight of a vertex} $u_i$ in a route $r=(u_0,\dots,u_l)$ is defined by $\lambda(r,u_i)= \sum\limits_{0 \leq j <i} \omega(r,u_j)$. It is the time needed by a message to go from the first vertex of the route to $u_i$. The \textbf{length} of the route $r$ is defined by $\lambda(r)= \lambda(r,u_l)$. 
  	...
  	A \textbf{routed network}, which models the telecommunication network, is a triple $N = (\cal{R},\,\cal{B},\,\omega)$ and is represented in figure~\ref{fig:graphmodel}. 
...
\caption{A routed network in which each colored path is a route}
...
\end{figure} 
    ...
    Figure~\ref{fig:datagramtimeline} represents the different events of the lifetime  of a datagram  sent on a route $r$.
  		\begin{figure}
  		 \begin{center}
    ...
    we denote $[r,u]_{P,\tau}$ by $[r,u]$ and say that $A$ is a valid assignment. 
     ...
     In the same example, by taking $P=5$ and $\tau = 2$, there is a no solution to the problem without waiting time. Nevertheless, with just $1$ waiting time for the route, we build the valid assignment $(0,0),(2,1),(0,0))$.
...
%\end{minipage}
             \caption{A physical fronthaul network at left and the star routed network representing the round trip at left}
	         \label{fig:star}
            \end{center}
	         \end{figure}
...
\caption{Different steps of the transformation from the star routed network of figure~\ref{fig:star} to its cannonical form where $\tau = 1$, $P=5$, $d_1 = 30$, $d_2 = 34$, $d_3 = 32$.}
\label{fig:canonical}
\end{figure}
...
We say that that $A$ is \textbf{compact} if there is a route $r_0 \in \cal{R}$ such that the following holds: for all subsets $S\subset \cal{R}$ with $r_0 \notin S$, the bufferless assignment $A'$, defined by $A'(r) = A(r) - 1 \mod P$ if $r \in S$ and $A(r)$ otherwise, is not valid. In other words, an assignment is compact if for all routes $r$ but one, $A(r)$ cannot be reduced by one, that is either in $c_1$ or in $c_2$, there is a route $r'$ using the tics just before $A(r)$. See Fig.~\ref{fig:compact} for an example of a compact assignment, obtained by the procedure of the next proposition. 
  \begin{figure}
      ...
      \end{center}
      \caption{Representation of the process of the Prop.\ref{prop:compactification} to transform a bufferless assignment $A$ into a compact bufferless assignment $A'$}
      \label{fig:compact}
      \end{figure}
...
Let $N = ({\cal R},\omega)$ be a canonical star routed network and let $P$ be the period and $\tau$ the size of a datagram. First, remark that for a given assignment and a route $r_0$ with offset $o$, by removing $o$ to all offsets, we can always assume that its offset is zero. By this remark and Prop.~\ref{prop:compactification} we need only to consider all \emph{compact assignments} with an \emph{offset $0$} for the route $r_0$. We now evaluate the number of compact assignments and prove that it only depends on $n$ the number of routes to prove the theorem.
 ...
 the offset of an unscheduled route is chosen so that it follows a route of $S_1$ in $c_1$ or a route of $S_2$ in $c_2$.
 ...
 to $s$, then $r$ with the smallest deadline is selected and set $w_r = s - \lambda(r,c_2)$, $t = s + \tau$ and $ U = U \setminus \{r\}$.
   ...
   \begin{tabularx}{0.7\textwidth}{|c|X|X|X|X|X|X|}
    ...
    \hline
      ...
      \caption{Example of a running of \greedydeadline with $P = 20, \tau = 4$.}
...
\section{Periodic Assignments vs Statistical Multiplexing}\label{sec:comparison}
    ...
    \begin{figure}
       \begin{center}
      \includegraphics[width = \textwidth]{stochasticdistrib.pdf}
...
\end{proof}
    ...
    Figure~\ref{fig:belatency} shows the cumulative distribution of the logical latency of BE packets, that is, the time each BE packets has wait in the buffers of the network. The experiment is made under several ways to schedule the C-RAN traffic. In \FIFO, all CRAN and BE traffic are managed following the \FIFO policy, regardless of the packet kind.
    ...
    The experiment are made over $100$ random instances.
     ...
     As expected, the latency of BE packets is better when both CRAN and BE packets are managed in \FIFO than when the CRAN packets are prioritized without consideration of BE packets(\framepre or \ASPMLS). Also, \ASPMLS performs worst than \framepre because of the long sequence of BE traffic it creates.
