...
\newcommand\SASPMLS{\texttt{SASPMLS}\xspace}
\newcommand\FIFO{\texttt{FIFO}\xspace}
\newcommand\framepre{\texttt{Frame preemption}\xspace}
\newcommand\critdead{\texttt{Critical deadline}\xspace}
...
%\end{minipage}
             \caption{Left, a physical fronthaul network and right, the star routed network modeling a round trip in the fronthaul network, the computation time in the BBU (in green in the figure) is encoded in the arcs.}
	         \label{fig:star}
            \end{center}
	         \end{figure}
...
To make \ESCA more efficient in practice, we make cuts in the search tree used to explore all compact assignments. Consider a set $S$ of $k$ routes whose offsets have been fixed at some point in the search tree. We consider the times used by these routes in $c_1$. It divides the period into $[(a_0,b_0), \dots, (a_{k-1},b_{k-1})]$ where the intervals $(a_i,b_i)$ are the times not used yet in $c_1$. Therefore at most $\displaystyle{ \sum_{i=0}^{k-1} \lfloor(b_{i} -a_i)/\tau\rfloor}$ routes can still send a datagram through $c_1$. If this value is less than $n - k$, it is not possible to create a compact assignment by extending the current one on $S$ and we backtrack in the search tree. The same cut is also used for the contention point $c_2$. These cuts rely on the fact that the partial assignment is wasting bandwith by creating intervals which are not multiples of $\tau$. It helps with instances of large load, which are also the hardest to solve.
   ...
   \paragraph{Instance generation}
    The defaults parameters of all experiment in this paper are derived from the C-RAN context: a tic correspond to the sending time of $64$ Bytes of data on links of bandwidth $10$~Gbps. The messages are approximately of size $1$~Mbit that correspond to $2,500$ tics. The number of routes is set to $n = 8$.
    We thus generate a physical star fronhaul network with $8$ routes. The size of the physical links are drawn uniformely in a different range following the experiment. The size of the period also depends of the experiment.
   ...
   Notice that both \metaoffset and \shortestlongest are polynomial time algorithms but are not always able to find a solution, depending on the load or the size of the routes. On the other hand, \ESCA finds a solution if it exists, but works in exponential time in $n$. We compare the performance of the algorithms in two different regimes: routes are either short with regard to $\tau$, or unrestricted.
      ...
      We evaluate the highest load under which a $(P,\tau)$-periodic assignment can be found by each algorithm when we change the number of routes. In our experiment, we generate $10,000$ random instances of \pazl for $100\%$ to $40\%$ of load. We represent in Figure~\ref{fig:short} the percentage of instance for which each algorithm finds a solution at each loads for $8$,$12$ and $16$ routes. A bound on the maximal success rate is given by the exhaustive search which always finds a solution if there is one. 
%       The lower and upper bound $n\tau$ and $3n\tau$ are also represented.
	 ...
	 \includegraphics[width=0.32\textwidth]{pazlshort16.pdf}
      \end{center}
      \caption{Success rate of \pazl algorithms for short routes. Left $8$ routes, middle $12$ routes and right $16$ routes.}\label{fig:short}
      \end{figure}
...
%      \STATE  deadline[$r_i$]  $\leftarrow$  $m_{i} + T_{max} - \Omega(s_i,c_s)$
    ...
    of time during which a datagram can go through $c_2$. Each time the waiting time of a route is fixed, an interval is split into at most two intervals in constant time. During the whole algorithm, each element of this list is used at most twice either when doing an insertion or when looking for the next free interval. Hence, the time needed to maintain the list is in $O(n)$. 
     ...
     small values for $t_{max} - t_{min}$ and \PMLS may succeed in finding a valid assignment (as shown in Section~\ref{sec:resultsPALL}), but not for all instances. 
...
\end{proof}
    ...
    We set the number of routes to $8$ to make comparisons with the results of Section~\ref{sec:exp_PAZL} easier. We draw uniformly the weights of the arcs of the star routed network between $0$ and $20,000$. We use \emph{the same deadline} for all routes, which is the most common constraint, when modeling a C-RAN problem: all RRHs have the same latency constraint and all BBUs take the same time to process the answer. 
...
\begin{figure}[h] 
  \centering
     ...
     According to our experiments, sending the messages on increasing order on the lenght of the arcs $(c_1,c_2)$ does not work well. It corresponds to the policy of Proposition~\ref{prop:SL} which we already know to be bad for \pazl when the routes are long as in this experiment. Sending on decreasing order on the margin of the route or on the length of the arcs $(c_1,c_2)$ works better and it seems that sorting the routes according to the length of the arc $(c_1,c_2)$ rather than the margin on the route is better, at least in a loaded network. 
     
     Also, remark that sending the messages on random orders does not perform well since only one random order is drawn, and thus the probability of drawing an order that allows \greedydeadline to find an assignment is low.
     
     In Figure~\ref{fig:success1000random} the same experiment is made but only for the three heuristics choosing an order at random. We now draw $1,000$ different random orders and solve each induced \wta instance. The algorithm is considered to succeed as soon as there is a valid assignment for one order. Each random order drawn is used for RO, RORS and ROBS to make the comparison fairer.
...
\begin{figure}[h] 
  ...
  First remark that our algorithms often finds assignments with $95\%$ of load and long routes which was not possible when looking for bufferless assignments (see Section~\ref{sec:exp_PAZL}). It justifies the interest of studying \pall and not only \pazl.
     ...
     With a load of $95\%$, a solution is found with margin $0$ most of the time. The three random order policies have similar performances, but ROBS gives slightly more solutions than the two others ones, under high load and small margin. Hence, in the following experiments, we will always draw $1,000$ random orders using the policy ROBS to set the offsets of the assignments.
  ...
  The complexity of both these algorithm depends on the number of routes. As shown in figure~\ref{fig:tps_fpt}, the time complexity of \PMLS seems linear on \emph{average}, while its theoretical worst case complexity is cubic. \ASPMLS scales exponentially with the number of routes as expected. Both algorithms are usable for instances of $20$ routes, but for $40$ routes or more \ASPMLS becomes too slow. Since \ASPMLS almost never finds a solution when \PMLS does not and is much slower, one should prefer to use \PMLS. 
    ...
    We investigate the success rate of our algorithms with regards to the number of random orders drawn, a load of $95\%$ and a margin $0$. The table of Figure~\ref{fig:randomdrawing} presents the success rate for different number of sending orders, averaged over $10,000$ instances, for \greedydeadline, \PMLS and \ASPMLS.
   ...
   \caption{Success rates function of the number of random orders drawn}
	...
	The number of different orders is $7!= 5,040$ since we have $8$ routes and the solutions are invariant up to a circular permutation of the order. Hence, for $8$ routes it is possible to test every possible order. However the computation time of this exhaustive method scales badly with $n$. The fact that \PMLS and \ASPMLS have already high success rates for $10$ random orders hints that even for a larger number of routes, drawing $1000$ random orders is sufficient to obtain good assignments.
     ...
     The figures~\ref{fig:1grp} and~\ref{fig:2grp} show the cumulative distribution of the margin needed by \ASPMLS to find an assignment computed on $10,000$ instances.
     Figure~\ref{fig:1grp} represents the success rate of \ASPMLS for instances where length of arcs are drawn in $[P-I,P+I]$, where $I$ goes from $0$ to $3200$. As expected the success rate decreases when the range increases until $I = 800$ and then increases again.  In the the most difficult settings, only $78\%$ of the instances can be solved with margin $0$, and we need a margin of $1,900$ to ensure that \ASPMLS always finds a solution.
     ...
     In Figure~\ref{fig:2grp}, we do the same experiment, except that the weight of arcs of half of the routes is drawn in $[P-I,P+I]$ and the length of the other half is drawn in $[P/2-I,P/2 + I]$. The situation is the same as for the previous experiment but with better success rates. 
       ...
       \begin{center}
      \includegraphics[width = \textwidth]{departs_distrib1Grp.pdf}
     
      \captionof{figure}{Cumulative distribution of the margin needed to find an assignment}
      \label{fig:1grp}   
...
\begin{minipage}{0.47\linewidth}   
       ...
       \begin{center}
      \includegraphics[width = \textwidth]{departs_distrib2Grp.pdf}
   
    \captionof{figure}{Cumulative distribution of the margin needed to find an assignment with two groups of routes}
      \label{fig:2grp}   
         \end{center}
      \end{minipage}\hfill
...
\section{Periodic Assignments vs Statistical Multiplexing}\label{sec:comparison}
     ...
     \subsection{\texttt{PMLS} Compared to Statistical Multiplexing}

    
      \subsubsection{Description of statistical multiplexing}
    Here, we describe how the statistical mulitplexing simulator is implemented.
    The networks are generated are the same as in the previous experiments. However, consider that the contention points can receive several messages to send at the same tic: it is thus now possible to buffer somme messages in every contention points of the network. 
    ...
    The first one is the \FIFO (First In First Out) policy, the trivial one, which consists in sending the messages, regardless their kind, in the same order in which they reached the contention point.
    

    
    The second proposed policy is called \critdead, and consists in sending first the message which has almost spent the largest time in the network between all buffered messages. 
  ...
  \caption{Probability of success of statistical multiplexing and PMLS for several margins on random topologies when the size of the routes are distributed either on $P$ (left) or on the same range of value (right).}
    ...
    To fix this issue, we compute an optimal solution with \ASPMLS with the largest messages size $\tau' \geq \tau$ possible. Once this solution is computed, the times at which the deterministic traffic will reach every contention points of the network without latency are well computed, but since the messages are shorter than in the computed solution, it induce some free tics to avoid best effort traffic flow.
...
Let $N = ({\cal R}, \omega)$ be a star routed network. If there is a $(P,\tau)$-periodic assignment $A$ of $N$, then there is a $(P,\tau')$-periodic assignment $A'$ of $N$ with $\tau' \geq \tau$, for which $TR(A') = TR(A)$ and each message is spaced by $\tau'-\tau$ free tics.

Consider $A$ a $(P,\tau)$-periodic assignment of $N$ given by \ASPMLS. Then, $A$ is the assignment with the minimal margin $m$.
By running \ASPMLS with the same $m$ but a greater $\tau$, if \ASPMLS finds an assignment, $TR(A') = TR(A)$ since $m$ is minimal.

Thus, by computing \ASPMLS with a larger $\tau'$ untill there is no solutions, we compute the assignment with the largest $\tau'$ possible. 

We propose the algorithm \texttt{Spaced ASPMLS} (\SASPMLS): starting from a $(P,\tau)$-periodic assignment $A$ with margin $m$, compute a $(P,\tau')$-periodic assignment $A'$ with the same margin $m$ and the maximal $\tau'$. Then, transform $A'$ into $A$, keeping the same offsets and waiting time, and reducing only the message size from $\tau' - \tau$. Thus, there is somme free space of size $\tau' - \tau$ between each messages in $A$.

The space given by \SASPMLS is illustrated in Figure~\ref{fig:space}
      ...
      \end{center} 
      \caption{Illustration of free space given by \SASPMLS}
      \label{fig:space}   
     \end{figure}   
    ...
    The best effort traffic is generated as follow. The size of a BE message is set to $50$ tics. Here, we want to generate $20\%$ of best effort traffic on the network. Since the best effort does not follow the round trip process of the cloud-RAN, some best effort message representing $20\%$ of the load are generated for each of the two contention points $c_1$ and $c_2$. Since a best effort message only have one contention point on its route the latency of a best effort message is defined as the time a best effort message waits in the contention point.
    
    On each contention point, the generation is split in two different exponential distribution. A first one, which loads the network in average of $15\%$, and generate one BE packets every $333$ tics in average. The second models a burst of BE messages, that is, a generation of $10$ packets every $10,000$ tics, in average. 
   
    
    
    In order to help C-RAN traffic to be as less buffered as possible in the network, we implemented the \framepre. This policy rally the TSN 802.1Qbu principle~\cite{ieee802} that allows to define priority class in the traffic and to schedule first the traffic whith the highest prioirity (here the C-RAN traffic).
    
    Also, when the CRAN traffic is scheduled with \ASPMLS or \SASPMLS, we want to avoid the transit of a best effort message at the begining of a CRAN message. We thus reserve the link $50$ tics (the size of a BE message) before the arrival of a cloud ran message, in order to forbid any new best effort message to use the link.
    
    Figure~\ref{fig:belatency} shows the cumulative distribution of the logical latency of BE packets, that is, the time each BE packets has wait in the buffers of the network. The experiment is made under several ways to schedule the C-RAN traffic. In \FIFO, all CRAN and BE traffic are managed following the \FIFO policy, regardless of the message kind.
    In the three other experiment, the BE traffic is managed in \FIFO, but the C-RAN is always prioritized following either \framepre, \ASPMLS or \SASPMLS.
    The experiment are made over $1,000$ random instances.
     \begin{figure}
       \begin{center}
      \includegraphics[width = 0.8\textwidth]{res.pdf}
       %\input{figures/stochastic}
      \end{center}
      \caption{Cumulative distribution of the latency of BE message in different networks management}
      \label{fig:belatency}   
     \end{figure}    
     
     First, remark that the latency of BE message is better ($1977$ tics in average ) when both CRAN and BE message are managed in \FIFO. Indeed, there is no frame preemption, the best effort is not impacted by any mechanism. Such a policy impact the CRAN latency: in average, the CRAN traffic waits $5265$ tics in the buffers during a round trip.
     
     When the CRAN message are prioritized without consideration of BE message with \framepre, the CRAN latency is lower ($1919$ tics in average), but the BE latency increases ($3256$ tics in average).
     Whith \ASPMLS, the CRAN latency downs to $0$, but the BE traffic is more impacted ($4909$ tics in average), because both the reservation and the long sequence of CRAN traffic it creates.
     
     In \SASPMLS, the CRAN traffic is smoothed on the period, in order to leave some free tics for BE traffic. One can remark that this phenomenon improve the BE traffic latency, even more than with a statistical approach ($949$ tics in average). 
    ...
    \begin{figure}
      ...
      \includegraphics[width = 0.8\textwidth]{distribtau.pdf}
       %\input{figures/stochastic}
      \end{center}
      \caption{Cumulative distribution of $\tau'$ for \SASPMLS. $P/n=4166$, the drop point of the function}
      \label{fig:belatency}   
     \end{figure}    
