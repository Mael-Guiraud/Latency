...
\section{The Star Routed Network} \label{sec:star_routed_network}
		...
		We call \textbf{star routed network} $(G, \cal{R})$ if and only if $G$ has only two contention points $c_1$ and $c_2$, all routes are of the form $s_i,c_1,c_2,t_i$ and $\cal{B} = \{c_2\}$ (datagrams can wait in $c_2$). Star routed network have contention depth two but a maximal contention width.
		...
		Donner la représentation canonique des stars routed network -> les arcs au milieu positif
		et dans [P], les arcs de fin avec la longueur restante, comme dans la proposition suivante.
		Donner une figure qui montre la canonicisation de l'exemple précédent.
		Parler de first et second period.
