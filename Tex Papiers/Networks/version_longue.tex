...
\end{figure}
	...
	\subsection{Periodic assignment for low latency}
      	...
      	The period $P$, as well as the size of a message $\tau$ are fixed in our $C-RAN$ settings, but not the buffering policy. Hence, the aim of this article is to find a valid assignment which minimizes the worst latency of the transmissions over the network, that is $TR(A)$.
      	For simpler hardness proofs and easier reductions, we study the decision version of the optimization problem we are interested in, that we call \pall for  \textbf{P}eriodic \textbf{A}ssignment for \textbf{L}ow \textbf{L}atency. Each route must respects a time limit called a \emph{deadline}. These limits are encoded in a deadline function $d$, which maps to each route $r$ an integer such that $PT(r)$ must be less than $d(r)$.
      ...
      \noindent {\bf Question:} does there exist a $(P,\tau)$-periodic assignment of $(G,{\cal R})$ such that for all $r \in {\cal R}$, $PT(r) \leq d(r)$?
...
\section{Hardness of \pall and \pazl}
  \label{sec:complexity}
	...
	The first proof works even for contention depth two. However, in Sec.\ref{sec:PAZL} we will see
  	that for some graphs of contention depth $2$, called star routed network, and small load, \pazl can be solved in polynomial time.
...
\begin{proof}
 We reduce graph coloring to \pazl. Let $H = (V,E)$ be a graph instance of the $k$-coloring problem. 
 We define the routed network $(G,{\cal R})$ as in Prop.\ref{prop:monotonic}.
 Let $G = (V', A)$ with $V' = \{v_e \mid e \in E\} \cup \{u^1, u^2 \mid u \in V\}$. 
...
$(P,\tau)$-periodic assignment, may correspond to a very small load. We can build on the reduction of the previous theorem to prove that computing the minimum transmission time for a given routed network and period
is hard to approximate.
\todo{define the problem \mintra comme la version d'optimization de \pall}

\begin{theorem}
If $\P \neq \NP$, the problem \mintra cannot be approximated in polynomial time within a factor $n^{1-o(1)}$ where $n$ is the number of routes.
\end{theorem}

\begin{proof}
$n$ est la période et le nombre de routes, qui vient du nombre de sommets du graphe à colorié qu'on simule.
Faire un gadget qui force à la fin le fait que les $n$ routes $r_{v_i}$ arrivent chacune dans un sommet
$v_i^1$ avec un temps dont les restes modulo $n$ sont égaux à une certaine valeur $t$.
Il suffit de montrer qu'on force les $v_i$ à avoir comme offset $t + i \mod n$. Facile de forcer la distance relative de deux offsets.
Ensuite, il suffit d'enchainer
par exactement le gadget précédent. Faire une figure pour tout ça.
\end{proof}
    %\begin{figure}[ht]
   % \centering
   % \scalebox{0.37}{
