...
\end{figure}
	...
	\subsection{Periodic Assignment for Low Latency}
      	...
      	We define the \textbf{margin} of a orute $r$ of a routed network $N$, for a deadline function $d$ as 
...
\section{Finding Bufferless Assignments} \label{sec:PAZL}
  ...
  we give several simple heuristics and an exact fixed parameter tractable algorithm, in time exponential in the number of routes only. We show in the experiments of Section~\ref{sec:exp_PAZL}, that \pazl can be very often solved positively, in particular for short routes and when the load is moderate. The sensivity to the load has been studied in details in a follow-up work~\cite{guiraud2020scheduling}, in which \pazl is solved for higher load using more involved polynomial time algorithms. 
    ...
    We first present a simple policy, which works when the period is large with regard to the lengths of the routes. More generally, it works as soon as the length of the routes modulo the period are close. The algorithm is called \shortestlongest: it sends datagrams on the shared link from the route with the shortest arc $(c_1,c_2)$ to the longest. There is no idle time in the contention point $c_1$, i.e. a datagram goes through $c_1$ right after the previous one has left $c_1$.
      ...
      If the period is slightly smaller that the bound of Prop.~\ref{prop:SL}, there is a collision with $r_0$ on $c_1$. Hence, this policy is not useful as a heuristic for longer routes, as confirmed by the experimental results of Section~\ref{sec:exp_PAZL}. 
...
\begin{figure}[h]
   ...
   Time (s) & $6.10^{-5}$&$8.10^{-4}$&$2.10^{-2}$& $0.4$& $11$\\
	...
	Finally, the complexity is in $O(n)$ since we have to find the maximum of the length of the $n$ routes and the computation of each $w_i$ is done by a constant number of arithmetic operations.
     ...
     First, we give several heuristics to choose the offsets, which will be experimentally be evaluated in Section~\ref{sec:resultsPALL}. We assume that the sar routed network is in canonical form. 
...
The space given by \SASPMLS is illustrated in Figure~\ref{fig:space}
   ...
   We now investigate the size of the free tics gaps created by \SASPMLS. Remember that, in experiment of Figure~\ref{fig:belatency}, we increase the size $\tau'$ of the message untill \ASPMLS does not find an assignment. Figure~\ref{fig:space} show the density function of the size of the maximal $\tau'$ on $10,000$ instances .
    ...
    \begin{figure}
      ...
      \label{fig:belatency}   
     \end{figure}    
Remark that most of the time ($80\%$) of the instance, the size $\tau' = \frac{P}{n}$ that is, the maximal size of message allowed for $n$ route in a period $P$. This means that in practice, \ASPMLS often perfectly smooth the free tics in the period.
