...
When building a periodic assignment, we must take into account the periodicity which makes many scheduling methods unusable. Not only a message must not collide with the messages sent by the others BBU/RRH in the same period, but also in the previous and following period. The latency, that is the time between the emission of a message and the complete return of its answer, must be minimized. This means that the only buffering we are allowed -- the waiting time before sending back the answer-- must be small, in particular when the route is long.
% Note that the model is technology agnostic, i.e. it is compatible with an optical network with a fixed packet size. 
 In this article, we assume that all RRHs send datagrams with the same period, but are not synchronized. It means that the datagram of each RRH is available at a different time in the period, given by its offset.
 In current cellular networkw, the RRHs are synchronized, and simulating our settings would cost a large additional latency, since datagrams should be buffered in their RRH to honor their offsets. 
Hence, the model we propose should be seen as a suggestion to design future cellular networks, where emissions of the RRH are not synchronized. It could already used to modelize sensor networks in cars, logistic problems in production lines or multiprocessor systems, where we need to schedule periodic messages (or goods) over a bus and where we have a better handle on when they are generated.
 
In this article, we focus on \emph{star routed networks}, which represent a simple but common topology,
where all RRHs share a single link. We give efficient algorithms for two version of the problems, called 
\pall and \pazl, the second one forbidding buffering even in the BBU. The algorithms are either polynomial or mildly exponential in $n$, the number of RRHs and are able to compute periodic sending schemes
...
\subsection*{Outline}
 ...
 In Section~\ref{sec:def} we propose a model of the network and the periodic sending of datagrams along its routes.  We then introduce \pall to formalize the problem of sending periodic messages in a network without collision. In Section~\ref{sec:complexity}, we prove that the problem \pall, and its variant \pazl, with all waiting times zero, are $\NP$-hard even for very restricted classes of graphs. 

In Section~\ref{sec:star_routed_network}, we introduce a simple but very common topology, called the star routed network, that is studied in the rest of the article. In Section~\ref{sec:PAZL}, we study the problem \pazl: We provide polynomial time algorithms which works when the load or the routes are small, and an algorithm exponential in the number of routes only. We give experimental evidences that \pazl can be solved positively when the network is not too loaded or when the routes are small. In Section~\ref{sec:PALL}, we propose polynomial time heuristics and an exact FPT algorithm for the general \pall problem and experimentally show that they work well even in loaded networks. 
Finally, in Section~\ref{sec:comparison}, we show how our solution largely outperforms stochastic multiplexing, even using a buffering policy taking into account the latency, or in the presence of additionnal random traffic in the network.
...
Let $[n]$ denote the interval of $n$ integers $\{0,\dots,n-1\}$.
	...
	\subsection{The C-RAN Network and its Contention Points}

  	We study a communication network with pairs of source-destination nodes between which messages are sent periodically. The routing between each pair of such nodes is given. The network is represented by a directed acyclic multigraph $G=(V,A)$. The set of vertices is composed of three disjoint subsets:  ${\cal S}$ the set of sources of the messages, ${\cal D}$ the set of destination of the messages, and ${\cal C}$ the set of contention points in the network. A contention point represents the beginning of a link shared by several routes, i.e. the physical node of the network which sends the messages into the link. An arc in $G$ may represent several physical links or nodes, which do not induce contention points. Each arc  $(u,v)$ in $A$ is labeled by an integer weight $\omega(u,v)$ which represents the time elapsed between the sending time of the message in $u$ and the reception time of this message in $v$ using this arc. 

  	A {\bf route} $r$ in $G$ is a directed path, that is, a sequence of adjacent vertices $u_0, \ldots , u_{l}$, with $(u_i,u_{i+1}) \in A$. A route represent the path of a datagram in the network, hence $u_0 \in \cal{S}$ represent the sending by the RRH, $u_l \in \cal{D}$ represents the reception of the answer in the same RRH. On each route, there is exactly one vertice in ${\cal C}$, which is the next contention point after the datagram leaves the BBU. We denote the set of these contention point by ${\cal B}$.

    The {\bf weight of a vertex} $u_i$ in a route $r=(u_0,\dots,u_l)$ is defined by $\lambda(u_i,r)= \sum\limits_{1 \leq j <i} \omega(u_j, u_{j+1})$. It is the number of tics needed by a message to go from the first vertex of the route to $u_i$. The \textbf{length} of the route $r$ is defined by $\lambda (r)= \lambda (u_l,r)$. 


	We denote by ${\cal R}$ a set of routes of the weighted multigraph $G$, the pair $(G,{\cal R})$ is called a \textbf{routed network} and represents our telecommunication network.
...
\end{figure} 
 	...
 	\subsection{Dynamic of Datagrams Transmissions}
        ...
        Let $r=(u_0,\dots,u_l)$ be a route. In order to avoid contention, it is possible to buffer datagrams in contention points of $\cal{B}$. An \textbf{assignment} $A$ of a routed network $(G,{\cal R})$, is a function which associates to each route $r \in \cal{R}$, the pair of integers $A(r) = (o_r,w_r)$.
        ...
        for $w_r$ tics in $u_j \in \cal{B}$, the vertex representing the BBU.
 		The \textbf{arrival time} of a datagram in vertex $u_i$ of $r$, is the first time at which the datagram sent on $r$ reaches $u_i$, and is defined by $t(r,u_i) = \lambda(u_i,r) + o_r $ if 
 		$i < j$ and $t(r,u_i) = \lambda(u_i,r) + o_r + w_r$ otherwise.

 		 If $u_l$ is the last vertex of the route $r$, the \textbf{transmission time} of the datagram on 
  		$r$ is denoted by $TR(A,r)$ and is equal to $\lambda(r) + w_r$ or $t(r,u_l) - o_r$. This is the total time taken by the process we study: the sending of the datagram from the RRH to the BBU and 
  		back to the RRH. We can decompose this time into $\lambda(r)$, the \emph{physical latency} of the process and $w_r$, the \emph{logical latency}. 
  		We define the \textbf{transmission time} of an assignment $A$ as the worst transmission time of a route: $TR(A) = \displaystyle \max\limits_{r \in {\cal R}} TR(A,r) $. 
...
\todo{dire qu'on ne modélise pas l'envoie d'un seul datagramme, mais l'envoi périodique de datagrammes}
    ...
    An assignment $A$ of a routed network $(G,{\cal R})$ is said to be \textbf{valid} if for all contention points $u$ and routes $r_1$ and $r_2$ containing $u$, \emph{$r_1$ and $r_2$ have no collision}. 
    The validity of an assignment depends on $P$ the period and $\tau$ the size of the messages,
    thus we say that $A$ is a valid $(P,\tau)$ assignment. When $P$ and $\tau$ are fixed or clear from the context, we only say that $A$ is a valid assignment. 

     In the example of Fig.~\ref{fig:example}, the three routes are given by three different colors. If we let $P = 2$ and $\tau = 1$, then there is a $(2,1)$-periodic assignment without waiting time by taking $0$ as offset for each route.

\todo{Mettre deux jeux de paramètres pour l'exemple, par exemple $\tau = 2$ et $P = 5$ ? Mettre les BBU.}

\todo{Trouver une meilleure notation que A pour les assignements qui collisionne avec les arcs. Mettre à jour le tableau de notations. Mettre une jolie police pour les algorithmes. Mettre à jour les figures pour concorder avec les nouvelles définitions. Mettre des figures d'illustration des définitions.}
...
\end{figure}
	...
	\subsection{Periodic assignment for low latency}
      	...
      	For simpler hardness proofs and easier reductions, we rather study the decision version of \mintra, that we call \pall for  \textbf{P}eriodic \textbf{A}ssignment for \textbf{L}ow \textbf{L}atency. Each route must respects a time limit called a \emph{deadline}. These limits are encoded in a deadline function $d$, which maps to each route $r$ an integer such that $TR(r)$ must be less than $d(r)$.
       
       % We will prove in Section~\ref{sec:complexity} that the problem \pra is $\NP$-complete, even in restricted settings.
     
      \noindent {\bf Periodic Assignment for Low Latency (\pall)} 

      \noindent {\bf Input:}  A routed network $(G,{\cal R})$, the integers $P$, $\tau$ and a deadline function $d$.
      
      \noindent {\bf Question:} does there exist a $(P,\tau)$-periodic assignment of $(G,{\cal R})$ such that for all $r \in {\cal R}$, $TR(r) \leq d(r)$?

	  In the next subsection, this problem is proved to be $\NP$-hard. In Section~\ref{sec:PALL} we study heuristics solving the search version of \pall (computing an assignment), also denoted by \pall for simplicity. In \pall, we have chosen to bound the process time of each route, in particular we can control the worst case latency. It is justified by our C-RAN application with hard constraints on the latency. It would be interesting to study the case of a constraint on the \emph{average} of the process times of the routes (or equivalently waiting times), which may be more relevant in other contexts.
     ...
     transmission time must be equal to the size of the route, which implies $w_r = 0$ for all $r \in \cal{R}$. This restriction is called \textbf{P}eriodic \textbf{A}ssignment for \textbf{Z}ero \textbf{L}atency or \pazl. Studying \pazl is simpler, there is no need to give the position of the BBU in the routed network
     and an assignment is given by the offset of each route. Moreover, a solution to \pazl is simpler to implement in real telecommunication networks, since we do not need to deal with any buffering at all.    
	...
	\begin{proposition} \label{prop:monotonic}
	 For any odd $P$, and the deadline function $d(r) = \lambda(r)$ (zero waiting time), there is a routed network such that there is a $(2,1)$-periodic assignment but no $(P,1)$-periodic assignment.
	\end{proposition}

	\begin{proof}
      We consider a generalization of the routed network $(G,{\cal R})$ given in Fig.~\ref{fig:example}. 
      For an integer $n$, we build the graph over the vertices $v_{ij}$, with $0 \leq i < j <n$. 
      There are $n$ routes and the route $r_i$ goes through all vertices $v_{ij}$, in order of increasing $j$.
	  The weights of the arcs are set so that $\lambda(v_{ij},r_i) - \lambda(v_{ij},r_j)= P$, where $P$ is an odd number smaller than $n$. It is always possible by choosing appropriate values for $\omega(v_{i-1j}v_{ij})$ and $\omega(v_{ij-1}v_{ij})$. In such a graph, there is no $(P,\tau)$-periodic assignment with no waiting time, since the problem reduces to finding a $P$-coloring in a complete graph with $n > P$ vertices, the colors being the offsets of the routes.

      If we consider a period of $2$, for all $i \neq j$, $\lambda(v_{ij},r_i) - \lambda(v_{ij},r_j) \mod 2 = 1$, hence two messages of same offset and size $1$ do not have a collision at $v_{ij}$. Therefore, assigning $(0,0)$ to each route is a $(2,1)$-periodic assignment of ${\cal R}$.      
\end{proof}
   ...
   \begin{tabularx}{\textwidth}{|c|X|}
    \hline
     ...
     \hline
      ...
      $\lambda(u_i,r)$ & Latency of the vertex $u_i$ in $r$\\
         ...
         \hline
         $ [t(v,r)]$& Times used by route $r$ at vertex $v$\\
         \hline 
         $A=((o_0,w_0), \ldots ,(o_{n-1},w_{n-1}))$& Assignment: an offset and a waiting time for each route\\
              \hline 
    ...
    \hline
     ...
     %\begin{figure}[h]
      ...
      %In the process time, we count the time between the time the first tic of the message is emitted and the first time at which the first slot of the message comes back. Alternatively we could consider the time between the emission of the first slot and the reception of the last slot of the message, which adds $\tau$ to the process time. Both definitions are equivalent in our context where all messages are of size $\tau$, hence we chose the first definition which is slightly simpler. 
...
\section{Hardness of \pall and \pazl}
  \label{sec:complexity}
	...
	The \textbf{contention depth} of a routed network $(G,{\cal R})$ is the size of the longest route of the network. The \textbf{width} of a vertex is the number of routes which contain it.
	The \textbf{contention width} of a routed network is the maximal width of its vertices. 
	Remark that a $(P,\tau)$-periodic assignment must satisfy that $P/\tau$ is larger or equal to its conflict width. Now, let us fix $P$ and $\tau$, for a given vertex of contention width $c$, we define its \textbf{load} as $c\tau/P$. The load of the routed network is the maximum of the loads of its vertices. Remark that a routed network must have a load less or equal to one to admit a valid assignment.


	We show in this section that \pall is $\NP$-hard by proving $\NP$-hardness of \pazl with $\tau =1$. 
	We give two alternate proofs that \pazl is $\NP$-complete.
	The first proof works even for contention depth two. However, in Sec.\ref{sec:PAZL} we will see
  	that for some graphs of contention depth $2$, called star routed networks, and small load, \pazl can be solved in polynomial time.
	 For contention depth one, either the load is less than one and \pall can be solved using zero waiting time in linear time or there is no valid assignment. The second proof works for graphs with contention width $2$, that is even if the conflicts are locally very small, the problem is complex globally.   Finally, solving \pall is trivial on trees and it may be interesting to study its complexity on bounded treewidth (or dagwidth) networks, since it is a common property of real networks~\cite{de2011treewidth}.
...
\end{theorem}
 ...
 \pazl is in $\NP$ since given an offset for each route in an assignment, it is easy to check whether there are collisions, in linear time in the multigraph.
  ...
  Let $H=(V,E)$ be an undirected graph and let $d$ be its maximal degree. We consider the problem to determine whether $H$ is arc-colorable with $d$ or $d+1$ colors. The arc coloring problem is $\NP$-hard~\cite{holyer1981np} and we reduce it to \pazl to prove its $\NP$-hardness. To do that, we define from $H$ a routed network $(G,{\cal R})$ as follows. 

  The vertices of $G$ are $V$ and $s_{u,v}, t_{u,v}$ for each $(u,v) \in E$.
  Let us choose an arbitrary total order $<$ on $V$.
  For each edge $(u,v) \in E$, if $u<v$, there is a route $s_{u,v},u,v,t_{u,v}$ in ${\cal R}$. 
  All these arcs are of weight $0$. The set of arcs of G is the union of all arcs of the routes.
  Note that, $(G,{\cal R})$ is of contention depth $2$, as required by the theorem statement.
   
  The existence of a $d$-coloring of $H$ is equivalent to the existence of a $(d,1)$-periodic assignment of $(G,{\cal R})$ with zero waiting time. Indeed, a $d$-coloring of $H$ can be seen as a labeling of its edges by the integers in $[d]$. It induces a bijection between $d$-colorings of $H$ and offsets of the routes of ${\cal R}$, which represent the edges of $H$. Having no collision on some vertex $v$ implies that all offsets of routes going through $v$ are different since all arcs are of weigth $0$. Hence, edges of $H$ incident to $v$, colored by the offsets are all of distinct colors. Therefore we have reduced arc coloring to \pazl by a polynomial time transformation which concludes the proof. 
 \end{proof}
 
 Remark that we have used weigths of zero for all arcs in the proof. It is amso a restriction to the 
 class of graphs for which \pazl is $\NP$-hard. We could ask the weights to be strictly positive, another restriction which makes sense in our model, since weights represent the delay of physical links. Then, we can prove $\NP$-completness using the same proof, by setting them to be all of size $d$.
...
We now give a hardness proof which works for routed networks with small contention width (but large contention depth). We can in fact use the reduction to prove an inapproximability result. Let \minpazl be the problem, given a routed network and $\tau$, to find the minimal period $P$ such that there is a $P$-periodic assignment with waiting times zero (a positive instance for \pazl). 


\begin{theorem}\label{th:inapprox}
If $\P \neq \NP$, the problem \minpazl on the classes of routed networks of conflict width two cannot be approximated in polynomial time within a factor $n^{1-o(1)}$ where $n$ is the number of routes.
\end{theorem}

\begin{proof}
 We reduce graph coloring to \pazl. Let $H = (U,E)$ be a graph instance of the $k$-coloring problem. 
 We define the routed network $(G,{\cal R})$ as in Prop.\ref{prop:monotonic}.
 Let $G = (V, A)$ with $V = \{v_e \mid e \in E\} \cup \{u^1, u^2 \mid u \in U\}$. 
 ...
 $A$ is the set of all arcs of the routes in ${\cal R}$. The weights of all arcs is zero.
 By construction, a contention vertex corresponds to an edge and belong to exactly the two routes corresponding to the vertices of the edge, hence the routed network is of contention width $2$.


  The existence of a $k$-coloring of $H$ is equivalent to the existence of a $(k,1)$-periodic assignment in $G$, by converting an offset of a route into a color of a vertex and reciprocally. Indeed, since all weigths are zero, the absence of collision at contention point $v_e$, with $e = (u,w)$ is equivalent to the fact that the offset of $r_u$ and $r_w$ are different.

   Therefore, if we can approximate the minimum value of $k$ within some factor such that there is a $(k,1)$-periodic assignment, we could approximate the minimal number of colors needed to color a graph within the same factor. The proof follows from the hardness of approximability of finding a minimal coloring~\cite{zuckerman2006linear}.
\end{proof}

The previous theorem implies that \pazl is $\NP$-complete on the class of routed networks with contention depth two. This also underlines the fact that, for general graphs, the best $P$ such that there is a 
...
\begin{proof}
 Every value in this proof is given modulo $n = |V|$, the period use in the reduction.
 Again, we reduce graph coloring to \pazl. Let $H = (U,E)$ be a graph instance of the $k$-coloring problem.
 We define a routed network $G = (V, A)$ in two parts. 
 Let the elements of $U$ be $u_0,\dots, u_{n-1}$. There are $n$ routes in the network, denoted by $r_i$ for $i \in [n]$. In their first part, they go from $u_i^0$ to $u_i^1$, through many vertices that we later define. Moreover, $u_i^1 \in \cal{B}$, that is the potential waiting time is added at $u_i^1$. Assume that $r_i$ has offset $o_i$ and $r_j$ has offset $o_j$.
 ...
 It is very easy to choose the weigths of the two arcs incomning into $v_{i,j,k}$ to realize the previous condition, whatever the choice of weights of the previous arcs.

We ensure, using the previous construction using the vertices $v_{i,j,k}$,
that $o_{i} = o_{j} - j + i$. It implies that there is some $o$, such that 
...
With this construction, we have ensured, that (modulo $n$) the datagram of $r_i$ arrives at 
$v_i^1$ at time $o$. 

The second part of the routes is built exactly as in the proof of Th.~\th:inapprox}.
Hence, the waiting time in the vertices $v_i^1$ play the exact same role as the offset
in the graph of Th.~\th:inapprox}. Finally, set the weigths of the last arc going to $v_i^2$, 
for all $i\in [n]$, such that, for all $i,j \in [n]^2$, $\lambda(r_i) = \lambda(r_j)$.
Hence, valid assignments are in bijection with colorings of $H$, the waiting times corresponding here to the colors. To minimize $TR(A)$, $o$ must be $0$ and the waiting times should be minimal (but different). In other words, a valid assignment which minimizes $TR(A)$ is in bijection with a minimal proper coloring of $H$.
\end{proof}
    %\begin{figure}[ht]
   % \centering
   % \scalebox{0.37}{
...
\section{The Star Routed Network} \label{sec:star_routed_network}
   ...
   \todo{Simplifier la présentation du réseau physique, expliquer ici qu'il est symmétrique car full duplex.
   Donner le routed network correspondant. Donner une représentation canonique du routed network -> besoin d'un poids et de deadline. Expliquer qu'on considère le temps de calcul 0}
    
		In this section, we define a family of simple routed networks modeling a Point-to-Multipoint fronthaul (PtMP), which has been designed for C-RAN \cite{tayq2017real}. 
		The fronthaul network we model has a single shared link, which connect all RRHs at one end and 
		all BBus at the other end. The links are all \emph{full-duplex}, 



      The graph $G$ has two sets of vertices, $S=\{s_0,\dots,s_{n-1}\}$ and $T=\{t_0,\dots,t_{n-1}\}$ of cardinality $n$ and two special nodes: the central source node {\bf $c_s$} and the central target node {\bf $c_t$}.
...
\section{Assignments with no waiting time} \label{sec:PAZL}
  ...
  we give several simple heuristics and an exact algorithm. We show in the experiments of Section~\ref{sec:exp_PAZL}, that \pazl can very often be solved positively, in particular when the load is moderate. This phenomenon has been studied in details in a following work~\cite{guiraud2020scheduling}, in which we 
  solve \pazl for higher load using more complex, but polynomial time, algorithms. 

   The problem \pazl is similar to the minimization of makespan in a two flow-shop with delays (see Section~\ref{sec:braheuristic}), a problem known to be $\NP$-complete~\cite{yu2004minimizing}. It suggests that \pazl is $\NP$-complete, however we have not been able to prove it yet,  because the periodicity is hard to deal with. With additional constraints we can get rid of the periodicity in the hardness proof, for instance if we consider that the shared link is unidirectional, that is collisions can happen between messages going from $c_s$ to $c_t$ and messages  going from $c_t$ to $c_s$. This variant can be shown to be $\NP$-complete by a reduction from the subset sum problem as it is done for a similar problem of scheduling pair of tasks~\cite{orman1997complexity}.
...
In this paper, we proposed two deterministic methods to establish a low latency periodic communication between BBUs and RRHs in a star routed network. The first method uses no buffering and has no latency overhead. It works when the routes are short (Longest-Shortest policy) or when the load is less than $80\%$ (Exhaustive search of compact assignments).  
