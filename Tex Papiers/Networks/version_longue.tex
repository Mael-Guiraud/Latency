...
In this article, we focus on \emph{star shaped networks}, which represent a simple but common topology,
...
%The first vertex of a route models an antenna (RRH) and the last one a data-center (BBU) which processes the messages sent by the antenna.
   ...
   %\subsection{Messages dynamic}
    ...
    %Time is discretized, hence the unit of all time values is a tic, the time needed to transmit a minimal unit of data over the network. The weight of an arc is also expressed in tics, it is the time needed by a message to go through this arc.
     %   In the process we study, a message is sent on each route at each period, denoted by $P$.
      ...
      %then it goes through the same arc at time $t+kP$ for all positive integers $k$. Therefore, every time value can be computed modulo $P$ and we say that the first time tic at which a message sent at time $m$ on $r$ reaches a vertex $u_i$ in $r$ is $t(u_i,r) = m + \lambda(u_i,r)\mod P$. 
      
      %A message usually cannot be transported in a single time tic. We denote by $\tau$ the number 
      %of \emph{consecutive tics} necessary to transmit a message. In this paper, we assume that $\tau$ is the same for all routes. Indeed, the data flow sent by an RRH to its BBU is the same, regardless of the route. Let us call $[t(u,r)]_{P,\tau}$ the set of time tics used by route $r$ at vertex $u$ in a period $P$, that is $[t(u,r)]_{P,\tau} = \{t(u,r) + i \mod P \mid 0 \leq i < \tau \}$. 
...
\end{figure}
	...
	\subsection{Periodic assignment for low latency}
      	...
      	For simpler hardness proofs and easier reduction, we study the decision version of the optimization problem we are interested in, that we call \pall for  \textbf{P}eriodic \textbf{A}ssignment for \textbf{L}ow \textbf{L}atency. Each route must respects a time limit called a \emph{deadline}. These limits are encoded in a deadline function $d$, which maps to each route $r$ an integer such that $PT(r)$ must be less than $d(r)$.
     ...
     transmission time must be equal to the size of the route, which implies $w_r = 0$ for all $r \in \cal{R}$. This restriction is called \textbf{P}eriodic \textbf{A}ssignment for \textbf{Z}ero \textbf{L}atency or \pazl. Studying \pazl is simpler, in particular an assignment is given by a single value for each route,
     its offset. Moreover, a solution to \pazl is simpler to implement in real telecommunication networks, since we do not need to implement any buffering at all.    
	...
	\begin{lemma} \label{lemma:monotonic}
	 For any odd $P$, there is a routed network such that there is a $(2,1)$-periodic assignment but no $(P,1)$-periodic assignment with zero waiting times.
	\end{lemma}
\begin{proof}
	  ...
	  The weights of the arcs are set so that $\lambda(v_{ij},r_i) - \lambda(v_{ij},r_j)= P$, where $P$ is an odd number smaller than $n$. It is always possible by chosing appropriate values for $\omega(v_{i-1j}v_{ij})$
	  and $\omega(v_{ij-1}v_{ij})$. In such a graph, there is no $(P,\tau)$-periodic assignment with no waiting time, since the problem reduces to finding a $P$-coloring in a complete graph with $n > P$ vertices, the colors being the offsets of the routes.
...
\section{Hardness of \pall and \pazl}
  \label{sec:complexity}
  	...
  	Let us introduce a few parameters quantifying the complexity of of routed network.
	The  \textbf{contention depth} of a routed network $(G,{\cal R})$ is the size of the longest route of the network. The \textbf{width} of a vertex is the number of routes going through it, that is its indegree.
	...
	The first proof works even for contention depth two. However, in Sec.\ref{sec:PAZL} we will see
  	that for some graphs of contention depth $2$ and small load, \pazl can be slved in polynomial time.
	 For contention depth one, either the load is less than one and \pall can be solved using zero waiting time in linear time or there is no valid assignment. The second proof works for graphs with contention width $2$.   Finally, solving \pall is trivial on trees and it may be interesting to study its complexity on bounded treewidth (or dagwidth) networks, since it is a common property of real networks~\cite{de2011treewidth}.
...
\end{theorem}
 ...
 \pazl is in $\NP$ since given an offset for each route in an assignment, it is easy to check whether there are collisions, in linear time in the multigraph.
  ...
  Note that, $(G,{\cal R})$ is of contention depth $2$.
 ...
 Remark that we have used weigths of zero for all arcs in the proof. It is again a restriction to the 
 class of graphs for which \pazl is $\NP$-hard. If we ask the weights to be strictly positive, which makes sense in our model since they represent the delay of physical links, if they all are of size $d$, we get the same proof.
