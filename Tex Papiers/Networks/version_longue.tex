...
In current cellular networks, the RRHs are synchronized, but we can defer synchronization to specialized mechanisms~\cite{khalili2016uplink,yemini2016multiple}. In particular, let us quote the use of the Timing Advance technology, which is already deployed to guarantee the synchronization of receptions despite difference in length of the routes~\cite{mahmood2019time}
...
to \pazl. The other important parameter when solving \pall and \pazl is the number of RRHs, on which the complexity of the proposed algorithms depend. 
...
An alternative approach consists in using optical rings for both fronthaul and best-effort flows~\cite{rommel2020towards,luu2021dynamic}. This relies on overdimensionning the network and using optical multiplexing technologies to avoid contention. 
...
contrarily to solutions working over dedicated optical networks. In this context, the latency induced by contention buffers is one of the major manageable source of delay and we try to 
deterministically managing datagrams in each node to prevent contention. This is similar to the concept of Deterministic Networking: the DetNet working group from IETF~\cite{finn-detnet-architecture-08} works in collaboration with TSN (Time Sensitive Networking)~\cite{ieee802}, a task group of IEEE, to develop technical solutions for deterministic networking~\cite{bhattacharjee2020time,8613095,durr2016no}. However, these works deal with stochastic sources of traffic, while we manage deterministic and perodic sources. Hence, the latency guarantee given by these works are an upper bound on the random latency, while we aim to minimize our constant latency. %In the same spirit, network calculus relies on a statistical model to guarantee bounded latency~\cite{9217707}. %detnet sur random traffic alirs que nous deterministic traffic
