...
Therefore in the next two sections, we study a simple but very common topology, that we call the \emph{star routed network}, where all messages use a common arc.
...
\todo{Dire que notre problème est proche d'un two flow shop scheduling (problème dur) et que les notions de cyclic schedulings ne sont pas adaptés : - soit minimisés le nombre deprocesseurs utiles alors qu'on a un problème à un processeur, soit batch scheduling
minimiser la période pour avoir un periodic scheduling alors que nous elle est fixée}

Periodic or cyclic multiprocessor scheduling problems~\cite{aupy2017periodic,hanen1993cyclic,korst1991periodic} are also close to our problem, but the models consider some parallel multiprocessor architecture, while our model would be a serial processor architecture. 
...
\end{proof}
	...
	This assignment represents the following process: first a message is sent at $u$, through the route $r \in F$, at time $m_r$.
      ...
      \end{center}
      \caption{Three time periods in a RRH}
      \end{figure}
      ...
      \end{center}
           \caption{Three time periods in the BBU}
      \end{figure}
...
\end{theorem}
 \begin{proof}
 Problem $PRA$ is in $\NP$ since given an offset for each route in an assignment, it is easy to check in linear time in regard to the number of arcs whether there are collisions.
...
\section{The Star Routed Network: No Waiting Time} \label{sec:PAZL}
    ...
    \subsection{The star routed network}
      ...
      The symmetric routed networks $(G, \{r_i,\rho(r_i)\}_{i<n})$ is called a \textbf{star routed network}. This graph looks very trivial, but every tree in which all routes share a same arc can be reduced to a star routed network. Furthermore, one can consider that all the BBU are located in the same datacenter. In such a situation, we can see the weights of the arcs $(c_t,t_i)$ either all equals (in that case, the problem is trivial, Sec.\ref{sec:PALL}) or different due to different technologies or hardware between the different BBUs. In any case, this topology seems realistic in nowadays network topologies.
      \begin{figure}
       \begin{center}
	 \scalebox{0.8}{
\begin{tikzpicture}
 ...
 The complexity of the algorithm is dominated by the sorting of the routes in $O(n\log(n))$. 
    ...
    \begin{proof}
     ...
     \begin{figure}
      ...
      \end{center}
      \caption{Both forward and backward period, represented at any step of the algorithm}
      \end{figure}
% 	\begin{algorithm}[H]
...
%        ~\cite{webpage}
      under a copyleft license.
       ...
       In our experiments we try to understand how our algorithms work with regards to the load. To change the load, we set the parameters $\tau$ and $n$ and modify the period $P$, which allows for a smoother control of the load and does not change the run time of our algorithms.
...
\begin{figure}[h]
     ...
     \end{figure}
      ...
      The computation time of the exhaustive search is in $O(n!)$ as shown in Th.~\ref{th:FPT}, hence we need to study its scalability when $n$ grows. In the following experiment, the weights of the arcs $(c_t,t_i)$ are drawn following an uniform distribution between $0$ and $20,000$ slots. We chose  $95\%$ of load.
...
Finally the complexity is $O(n)$ since we have to find the maximum of the length of the $n$ routes and the computation of each $w_i$ is done by a constant number of arithmetic operations.
     ...
     The offsets of the forward routes will be chosen so that all messages have no collisions on the central arc and such that there are no free slots inbeetween the end of a message on the central arc and the beginning of the next one. 
...
%     \end{algorithm}
   ...
   Considering smaller load is not relevant since we know how to solve the problem even without waiting times as shown in Section~\ref{sec:exp_PAZL}. 
   
   We first try to understand what is the best choice of order for the first stage of the algorithm which is followed by the GD algorithm in this experiment. In Fig.~\ref{fig:success95},\ref{fig:success80}, we represent the success rate of the five kind of orders with regards to the margin. The value is an average computed over $10,000$ random star routed networks.  Note that for the random order, we compute $1000$ random orders and count it as a success as soon as there is a solution for one order. 
...
\begin{figure}[h] 
  \centering
     ...
     The random order is the best by far: with a load of $95\%$, there is solution with margin $0$ most of the time. Note that, when disallowing waiting times, there were no instances with an assignment for these parameters (see Section~\ref{sec:exp_PAZL}), which justifies the interest of studying the \pall problem.
