...
%       which makes the first cut more likely to apply. A last cut could be implemented: compute for every route not in $R(S)$ the set of possible positions in the backward period and verify whether at least one is contained in the useful slots of $S$.
   ...
   \subsection{Experimental evaluation}\label{sec:exp_PAZL}
      ...
      It corresponds to slots of $64$ bits, messages of approximately $1$Mb and links of bandwidth $10$Gbps when $P$ is one millisecond.
       The code in C is available on the web page of the authors\footnote{\url{http://www.prism.uvsq.fr/~ystr/textesmaths.html}} under a copyleft license. The code has been run on a standard $2016$ laptop, and most experiments run in a few dozen of seconds.
      ...
      In our experiment, we generate $1,000$ random instances of \pazl for $1$ to $14$ routes. We represent in Figure.~\ref{fig:short} the average of the maximal load for which each algorithm finds a solution. A bound on the load is given by the exhaustive which always finds a solution if there is one. 
...
\begin{figure}[h]
     ...
     \end{figure}
      ...
      compact assignments is pruned during the algorithm. We study the scalability  of the algorithm when $n$ grows in Figure.~\ref{fig:runtime}. The weights of the arcs $(c_t,t_i)$ are drawn following an uniform distribution between $0$ and $20,000$ slots. We chose  $95\%$ of load.  Figure.~\ref{fig:runtime} shows the time before the exhaustive search ends, for $8$ to $16$ routes, averaged on $100$ random star routed networks. This shows that for less than $16$ routes, which corresponds to all current topologies, the algorithm is efficient enough, but we should improve it further to work on more routes.
   ...
   Time (s) & $6.10^{-5}$&$8.10^{-4}$&$2.10^{-2}$& $0.4$& $11$\\
    \hline
      \end{tabularx}
      \end{center}
      \caption{Running time of the exhaustive search.} \label{fig:runtime}
      \end{figure}
 ...
 %A more realistic assumption would be that all weigths of the arcs $(s_i,c_s)$ are the same. It would correspond to a situation where all the BBUs are in the same datacenter.
	...
	Finally the complexity is $O(n)$ since we have to find the maximum of the length of the $n$ routes and the computation of each $w_i$ is done by a constant number of arithmetic operations.
     ...
     Assume that for any pair of indices $(i,j)$ the difference between the weights of the arcs $(s_i,c_s)$ and $(s_j,c_s)$ is bounded by $d$ and the length of the longest route is $l$. Using the same idea as in Theorem.~\ref{th:asym}, there is an assignment with maximum process time bounded by  $2l + d$.
   ...
   \subsection{Greedy scheduling of backward routes}
    ...
    Algorithm~\ref{alg:GD} is the formal description of the previous algorithm. 
     ...
     \begin{algorithmic}\label{alg:GD}
...
The problems \bra and \pall can be slightly generalized by allowing different process times for each route instead of fixing a global maximum. Note that the algorithms of previous sections are able to solve these variants. We call $d_i$ the \emph{deadline} of the route $r_i$, that is the maximum time at which a packet can go through $c_t$. This allows to consider a simpler form of the problems \pall or \bra inspired by the scheduling problem of the previous section. An instance is a set of route, the route $r_i$ being defined by its latency $\lambda_i \in [P]$ and its deadline $d_i$. In the corresponding star routed network, all arcs $(s_i,c_s)$ and $(c_s,s_i)$ have weight zero and all arcs $(c_t,t_i)$ and $(t_i,c_t)$ have weight $\lambda_i/2$.
We prove by self reduction from \pall to \pall (and similarly \bra to \bra) that we can always restrict ourselves to such a star routed network. First, the weight of all arcs $(s_i,c_s)$ can be set to $0$, by subtracting $2\omega(s_i,c_s)$ to $d_i$
...
and  when $y_i >= P - \tau$ we replace $y_i = x_i + \lambda_i + w_i$ by $y_i = x_i + \lambda_i + w_i - P$ and we replace the deadline $d_i$ by $d_i -P$. In that way we guarantee that all $y_i$ are less than $P-\tau$.  
 ...
 To express that there are no collision between forward routes, the following equations must be  satisfied: for all $i$, $x_i + \tau \leq x_{i+1}$. We assume that the routes are here indexed in order of growing $x_i$.
 Let $\sigma : [n] \rightarrow [n]$ be the permutation such that $r_{\sigma(i)}$ are in order of growing $y_{\sigma(i)}$.
