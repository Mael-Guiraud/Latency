...
%     
  %   Moreover, by studying the computation time of the FPT-MLS and the PMLS algorithm during the experience of Figure~\ref{fig:success21000}, we remark that the average computation time to find a solution is often $10$ times higher with FPT-MLS than with PMLS ($\sim300\mu s$ for FPT-MLS versus $\sim30\mu s$ for PMLS). Nevertheless this remark is true when PMLS finds a solution at $100\%$. On the other hand, when PMLS can not find a solution that FPT-MLS finds, because the algorithm needs to draw the $1000$ random orders before failing, the computation time of an instance by PMLS is, in average, in the same range as the one with FPT-MLS ($\sim 1ms)$.
      Then, we study the scalability of the PMLS and the FPT-PMLS algorithms when the number of routes grows. In the following experiment, we fix the margin to $0$ and we take the same parameters as previously: we draw uniformly the weights of the arcs between $0$ and $20,000$, the period is chosen so that the load is always $95\%$, and we vary the number of routes. The table on Figure~\ref{fig:tps_fpt} shows the computation times\footnote{Those measures are made on a $2.2$ Ghz Intel Core i7, running under macOS high sierra, version $10.13.1$.} of the PMLS and the FPT-PMLS algorithms, averaged on $1,000$ instances. 
   ...
   \begin{tabularx}{0.5\textwidth}{|c|X|X|X|X|X|X|}
    ...
    \hline
    FPT-PMLS (ms) & $0.45$ &$5.75$&$29.18$&$198.1$&$3279$\\
    \hline
     PMLS ($\mu$s) & $84$ &$43$&$55$&$66$&$90$\\
    \hline
      \end{tabularx}
      \end{center}
