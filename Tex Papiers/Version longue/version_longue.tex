...
\end{proof}
     ...
     \begin{figure}
      ...
      To represent the time limits the process we have described must respect, we introduce the \emph{threshold function}.
      We denote it by $TH$ and to each route $r$ it maps an integer such that $PT(r)$ must be less than $TH(r)$.
...
%     The {\bf maximum process time} of the assignment $m$ of $(G,{\cal R})$ is defined by $MPT(m)=\max\limits_{r \in {\cal R}} PT(r)$.
      ...
      \noindent {\bf Input:}  A symmetric routed network $(G,{\cal R})$, the integers $P$, $\tau$ and a threshold function $TH$.
      \noindent {\bf Question:} does there exist a $(P,\tau)$-periodic assignment $m$ of $(G,{\cal R})$ such that for all $r \in {\cal R}$, $PT(r) \leq TH(r)$?
...
\begin{proof}
 ...
 Let us define $TH(r) = 2 \times \max_{r' \in {\cal R}} \lambda(r') + P$. We define $(G',{\cal R}')$ a symmetric routed network from $(G,{\cal R})$ where for every route we add a symmetric route with new arcs of opposite orientation and the same weights.
 We prove that the instance $((G',{\cal R'}),P,\tau,TH)$ is in \pall if and only if $((G,{\cal R}),P,\tau)$ is in $PRA$.
 If $((G',{\cal R'}),P,\tau,TH)$ is in \pall, then a $(P,\tau)$-assignment of $(G',{\cal R'})$ restricted to $\R$ is a $(P,\tau)$-assignment of $((G,{\cal R})$ since they cannot be any collision between routes of ${\cal R}$.
 
 Assume now that $((G,{\cal R}),P,\tau)$ is in $PRA$. First, remark that the waiting time of each route is by definition less than $P$ and thus we have for all $r\in {\cal R}$, $PT(r) \leq TH(r)$. Moreover a $(P,\tau)$-assignment of $(G,{\cal R})$ can be extended into a $(P,\tau)$-assignment of $(G',{\cal R'})$ in the following way. For each route $r \in \cal{R}$ of last vertex $v$, the time at which the message arrives is $t(v,r)$, then we choose as offset for $\rho(r) \in \cal{R}'$ $-t(v,r) \mod P$. The symmetry ensures that each new route $\rho(r)$ in ${\cal R'}$ uses exactly the same time slots as $r$ on each of its node and thus avoid collisions.
\end{proof}
...
\end{tikzpicture}
  ...
  \begin{proposition}\label{prop:canonical}
   Let $I = (G,{\cal R}), P, \tau , TH$ be an instance of \pall, then there is another instance 
   $I' = (G',{\cal R}), P, \tau , TH'$ such that:
   \begin{itemize}
    \item $I \in \pall \Leftrightarrow I' \in \pall$
    \item $\omega(c_s,c_t) = 0$
    \item $\forall i, \, \omega(s_i,c_s) =0$
    \item $\forall i, \, \omega(c_t,t_i) + \omega(t_i, c_t) < P$
   \end{itemize}
  \end{proposition}

  \begin{proof}
   We explain how we build $G'$ and $TH'$ from $G$ and $TH$ in such a way that there is a bijection 
  ...
  the satisfying assignment. If we subtract its weight to $TH(r)$ for each $r$, and set its
  weights to zero in $G'$, then the constraints $PT(r) \leq TH(r)$ will be the same in $I$ and $I'$.
      
  In $G'$, the weights of the arcs $(s_i,c_s)$ is set to $0$ and $2\omega(s_i,c_s)$
  is subtracted to $TH(r_i)$. Hence the threshold constraints are the same in $I$ and $I'$.
  There is a bijection between the assignments of $I$ and $I'$ by mapping $m_i$ to $m_i + \omega(s_i,c_s)$.
  
  Finally, we can subtract $P/2$ to $\omega(c_t,t_i)$ and $\omega(t_i,c_t)$ if we subtract $P$ to $TH(r_i)$.
  The constraints on collisions of message are modulo $P$ ans thus do not change and the threshold constraints are left unchanged since the two modifications cancel out. Therefore to obtain $I'$, we remove $P/2$ enough time to each  $\omega(t_i,c_t)$ and $\omega(c_t,t_i)$ so that $\omega(c_t,t_i) + \omega(c_t,t_i)< P$. 
  \end{proof}

   From now on, we assume that all routed networks have the properties given in Proposition~\ref{prop:canonical}.
  ...
  We ask for a $(P,\tau)$-periodic assignment {\bf with all waiting times equal to $0$}, that is $TH(r) = 2\lambda(r)$ and we call this restriction {\bf Periodic Assignment for Zero Latency} or \pazl. We study \pazl since it is simpler, in particular 
  choosing the offset $m_r$ also sets the offset of the route $\rho(r)$ to $m_{\rho(r)} = m_{r} + \lambda(r) \mod P$.
  We get better theoretical results and practical algorithms for \pazl than for \pall. Moreover, as we show in our experimentations of Sec.\ref{sec:exp_PAZL}, this problem can very often be solved positively (albeit less often than the general problem). Finally, a solution to \pazl is simpler to implement in real telecommunication networks, since we do not need to implement any buffering at all.    
...
%       which makes the first cut more likely to apply. A last cut could be implemented: compute for every route not in $R(S)$ the set of possible positions in the backward period and verify whether at least one is contained in the useful slots of $S$.
   ...
   \subsection{Experimental evaluation}\label{sec:exp_PAZL}
      ...
      In our experiment, we generate $1,000$ random instances of \pazl for $1$ to $14$ routes. We represent in Fig.~\ref{fig:short} the average of the maximal load for which each algorithm finds a solution. A bound on the maximum load is given by the exhaustive search which always finds a solution if there is one. 
...
\begin{figure}[h]
   ...
   Time (s) & $6.10^{-5}$&$8.10^{-4}$&$2.10^{-2}$& $0.4$& $11$\\
    ...
    In this section, we consider the more general \pall problem on star routed networks. The messages are allowed to wait in the target vertices (BBUs) to yield more possible assignments. Hence, we allow the process time of the routes to be greater than twice the weights of the routes, but it is bounded by the threshold function.

	\subsection{Special cases}
		
		
	Often in real networks, the length of the routes are not arbitrary and we may exploit that to solve \pall easily. For instance if all the weights on the arcs $(c_t,t_i)$ are the same, we can replace them all by $0$ and subtract this weight to the thresholds. It corresponds to a situation where all the BBUs are in the same data-center and have the same processing power. The assignment in that case is trivial, just send all messages so that they follow each other without gaps in the central arc. Since the arcs $(c_t,t_i)$ are of weight $0$, all messages will go through $(c_t,c_s)$ on their way back in the same order and thus do not collide. 
	
	Another possible assumption would be that all weights of the arcs $(s_i,c_s)$ are the same, or approximately the same.
	
	 \begin{theorem}\label{th:asym}
	 Let $(G,{\cal R})$ be a star routed network with $n$ routes and let $P \geq n\tau$. Let 
   ...
   Either the time between two messages on the central arc is random and we call this policy Random Order and Random Spacing (RORS) or the gap is evenly distributed between messages and we call this policy Random Order and Balanced Spacing (ROBS).
...
The problems \bra and \pall can be slightly generalized by allowing different process times for each route instead of fixing a global maximum. Note that the algorithms of previous sections are able to solve these variants. We call $d_i$ the \emph{deadline} of the route $r_i$, that is the maximum time at which a packet can go through $c_t$. This allows to consider a simpler form of the problems \pall or \bra inspired by the scheduling problem of the previous section. An instance is a set of routes, the route $r_i$ being defined by its latency $\lambda_i \in [P]$ and its deadline $d_i$. In the corresponding star routed network, all arcs $(s_i,c_s)$ and $(c_s,s_i)$ have weight zero and all arcs $(c_t,t_i)$ and $(t_i,c_t)$ have weight $\lambda_i/2$.
...
The FTP algorithm is the following: for each subset $S$ of routes, remove $P$ to the release time and to the deadline of each route in $S$ and run PMLS on the new instance. If there is an assignment, then we have proved that there is a $S$ such that the modified instance has a solution with all messages going through the central arc before $P - \tau$, a solution that $PMLS$ will find.
\end{proof}

The algorithm of Theorem~\ref{th:braFPT} has a complexity of $O(2^nn^3\log(n))$. Moreover, for each backward route to be scheduled between $P$ and $2P-\tau$, it must have a deadline larger than $P + \tau$ since between $P$ and $P + \tau$ the first message goes through the central arc. When the deadline is not large with regards to the release time, 
only a small fraction of the routes will have this possibility, say $t$ of them and we will consider only $2^t$ cases.
...
and  when $y_i >= P - \tau$ we replace $y_i = x_i + \lambda_i + w_i$ by $y_i = x_i + \lambda_i + w_i - P$ and we replace the deadline $d_i$ by $d_i -P$. In that way we guarantee that all $y_i$ are less than $P-\tau$.  
 ...
 To express that there are no collision between backward routes, the following equations must be  satisfied: for all $i$, $y_{\sigma(i)} + \tau \leq y_{\sigma(i+1)}$. Finally, the deadline of each route must be respected, that is for each $i$, $y_i \leq d_i$. 
