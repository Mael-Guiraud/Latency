...
%      \STATE  deadline[$r_i$]  $\leftarrow$  $m_{i} + T_{max} - \Omega(s_i,c_s)$
 ...
 \STATE insert(${\cal H}$,$r_i$).
    ...
    Since we set the first backward route, we take $m_{min} = 0$, and we then want all release times $r \leq P-\tau$.
     It can be done by replacing each release time by $r \mod P$ and by subtracting $r / P$ to the corresponding deadline. 
     Furthermore, if a release time of a route $i$ is between $  P-\tau$ and $P$, we set $r_i = 0$, and $d(i) = d(i) - P$.
     We assume in the rest of this section, that the instances always satisfy this property. 
...
\end{theorem}
\begin{proof}
 Consider an instance of \bra, which can be characterized by a release time and a deadline for each route. W.l.o.g. say that the first backward route has release time $0$ and $\Omega(c_t,t_0) = 0$. As for PMLS, we can change the releases time and deadlines such that all release times are less than $P - \tau$. If there is an assignment such that the messages of all routes goes through $c_t$ before $P-\tau$, then the periodicity does not come into play for this assignment and the algorithm MLS will find it as explained in Section~\ref{sec:braheuristic}.

We show how to modify the instance so that its periodic assignments are a subset of the periodic assignments of the original instance and such that one of its assignments satisfy the previous condition.
 Because of Proposition~\ref{prop:canonical}, for all $i$, $\Omega(c_t,t_i) + \Omega(t_i,c_t) < P$. Hence, in a periodic assignment the time at which a message goes through $c_t$ is always smaller than $2P-\tau$. We select all messages in the periodic assignment which go through $c_t$ between $P-\tau$ and $2P-\tau$. We define a new instance where the release time of these messages
is zero and $P$ is subtracted to their deadline. Then the considered periodic assignment is a solution of the new instance with all messages going through $c_t$ before $P-\tau$. Moreover any solution of the new instance is a solution of the original problem, since we have made the constraints harder.   
