...
\subsection{Two others FPT algorithms}

As a warm-up, let us give a simple FPT alogirthm for \bra which is practical,
and then we will give an FPT algorithm for \pall whose dependency on $n$ the number of routes is 
to large tu be useful. 
...
\end{theorem}
\begin{proof}
 Consider an instance of \bra such that there is a solution. 
W.l.o.g. say that the first message on a backward route is sent through the central arc at time $0$. Assume that 
the delays and the waiting times of the solution are such that all messages are sent before $P - \tau$. Then the periodicity does not come into play for this solution and the algorithm PMLS will find it as explained in Section~\ref{sec:braheuristic}. 

 If a message in a solution goes through the central arc after $P - \tau$, since all delays and waiting time
 can be taken modulo $P$ without changing the solutions, we can say that it goes through the central arc 
between $P$ and $2P - \tau$. Now, if we subtract $P$ to the release time of the message and to its deadline it does not change the solutions, but the message goes through the central arc before $P- \tau$.

The FTP algorithm is thus the following: for each subsets of routes, remove $P$ to the release times and to the deadlines of the routes in the subset and run PMLS on the instance. If there is a solution, then as explained, by removing $P$ to the release times of all routes which are sent after $P$ in the solution, we obtain a new instance which can be solved by PMLS.
\end{proof}

The algorithm of Theorem~\ref{th:braFPT} has a complexity of $O(2^nn^3\log(n))$. However, it is easy to compute for each backward route if it is possible to have an offset larger than $P + \tau$. When the deadline is not large with regards to  the release time, 
...
We consider the offsets of the forward routes on the forward period, 
up to renaming the routes, we have for all $i$ $m_i + \tau \leq m_{i+1}$ since there are no collisions.
For the backward routes, there is a permutation $\sigma : [n] \rightarrow [n]$, such that the offsets of the  backward routes on the backward period satisfies  $m_{\rho(\sigma(i))} + \tau \leq m_{\rho(\sigma(i+1))}$ for all $i$ since there are no collisions. 

We denote by $w_i$ the waiting time of the route $r_i$, then the offsets $m_i$ and $m_{\rho(i)}$
satisfy the following equation: $m_{\rho(i)} = m_i + 2*\omega(c_t,t_i) + w_i$.

Assume also that the first route has delay and waiting time zero, 
that is $m_0 = m_{\rho(0)} = 0$. As in Theorem~\ref{th:braFPT}, may assume 
that all $m_{\rho(i)} < P$ by subtracting $P$ to their release time,
which corresponds to the equation $m_{\rho(i)} = m_i + 2*\omega(c_t,t_i) + w_i - P$.

We must guarantee that no offset is larger than $P-\tau$ which can be written for all 
$i$, $m_i \leq P-\tau$ and $m_{\rho(i)} \leq P-\tau$. 
Finally, the maximum process time of the solution must be less than $T_{max}$ given in the instance,
that is for each $i$, $2\lambda(r_i) + w_i \leq T_{max}$. 
...
all messages have an offset between $0$ and $P-\tau$. 

Remark that $S$ may have a solution with rational values, while offsets must be integers. 
However, remark that $x +e_1 \leq y + e_2$ implies $\lceil x \rceil +e_1 < \lceil y \rceil + e_2$ when $e_1$ and $e_2$ are integers.
Therefore, since all the inequations and equations of $S$ have this form, if we take the upper floor of the values of a solutionof $S$, it is still a solution of $S$ with \emph{integer} values. As a consequence, any solution of $S$ yields a solution of the considered instance of \pall.

Now remark that the construction of $S$ depends on choosing the right order of the offsets on the forward period and the backward period, and on the choice of the backward routes where $P$ has been subtracted from the offset. Hence to solve \pall, we must solve $2^n(n!)^2$ systems of size linear in $n$, a complexity of $O(2^n(n!)^2n^3)$ which is FPT in the number of routes $n$.
\end{proof}
