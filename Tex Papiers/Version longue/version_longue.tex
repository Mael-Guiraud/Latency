...
%      \STATE  deadline[$r_i$]  $\leftarrow$  $m_{i} + T_{max} - \Omega(s_i,c_s)$
 ...
 \STATE insert(${\cal H}$,$r_i$).
    ...
    or looking for the next free interval, the time needed to maintain it is $O(n)$. 
     ...
     Let $RT(r)$ be the release time of $r$, we subtract it to all release time and deadline.
     Therefore $RT(r)$ is zero in this instance and the time at which the message of $r$ crosses $c_t$ is also set to zero, hence its waiting time is zero.  We then modify the instance so that all release times are in $[0,P-\tau]$: Each release time $RT(r_i)$ is replaced by $RT(r_i) \mod P$ and $RT(r_i) - (RT(r_i) \mod P)$ is subtracted to the deadline $d(r_i)$. Furthermore, if a release time of a route $r_i$ is between $ P-\tau$ and $P$, we set it to $0$ and $d(i) = d(i) - P$. 
...
process time of $r_i$ by $P$. Since $y_i - P > P$, it is a valid assignment, that is the route $r_i$ sends its message after its release time. Moreover, there are no $y_i \in ]2P-\tau, 2P]$ for all $k$ and $i \neq j$, since by construction
$y_j = 0$.
 

From a periodic assignment satisfying that for all $i$, $y_i \leq 2P - \tau$, we define 
a new instance whose periodic assignments are a subset of the periodic assignments of the original instance. Moreover, one of the periodic assignments of the new instance satisfies for all $i$, $y_i \leq P - \tau$ and will thus be found by MLS. Let $S$ be the set of routes $r_i$ such that  $y_i \in]P - \tau, 2P - \tau]$. The new instance is defined by setting the release time of the routes in $S$ to $0$ and by subtracting $P$ to their deadlines. Then the considered periodic assignment is a solution of the new instance with all $y_i \leq P -\tau$. Moreover any solution of the new instance is a solution of the original one, since we have made the constraints harder.   
...
\end{proof}
   ...
   We first try to understand what is the best choice of order for the first stage of the algorithm which is followed by the GD algorithm in this experiment. In Figure~\ref{fig:success80} and \ref{fig:success95}, we represent the success rate of the seven kind of orders with regards to the margin. The value is an average computed over $10,000$ random star routed networks. For RO, RORS and ROBS, we draw $1000$ random orders and count it as a success as soon as there is a solution for one order. Each random order drawn is used with RO, RORS and ROBS to make the comparison fairer.
...
\begin{figure}[h] 
  \centering
    \includegraphics[width=0.5\textwidth]{departs_gp_21000.pdf}
     ...
     Using many random orders is much better than our arbitrary choices of order. With a load of $95\%$, a solution is found with margin $0$ most of the time. The three random order policies have similar performances, but RO gives slightly more solutions than the two others ones, under high load and low margins. This is the reason why in the following experiments on the performances of our algorithms to solve \bra, we will always use  RO to fix the offsets of the forward routes.  Note that, when disallowing waiting times, there were no instances with an assignment for $95\%$ of load and long routes (see Section~\ref{sec:exp_PAZL}), which justifies the interest of studying \pall rather than \pazl.
     
      We now want to compare the performances of the four different algorithms used in the second stage. Since GD already showed excellent results on mild loads, it is more interesting to focus on the behavior of the algorithms with high load. We will draw $1,000$ random orders using the policy RO to set the offsets of the forward routes since it gives the best results. In Figure~\ref{fig:success21000}, we represent the success rate of the four algorithms with regards to the margin,  computed over $10,000$ random star routed networks generated with the same parameters as previously. In order to show relevant results, we chose a load of $95\%$.
    ...
    \begin{figure} [h] 
     ...
     The GD algorithm is close to $100\%$ success rate for margins larger than $1,500$ while the PMLS and FPT-PMLS algorithms find a solution in more than $99\%$ of the experiments, even \emph{with a margin $0$}. In other words, for very high load and no margin, there are only a few instances for which we do not find an assignment. With a margin of $300$, which corresponds to about $15\mu$s of additional delay with the chosen parameters, we always find a solution. 
     
     It turns out that the performances of PMLS and FPT-PMLS 
     are almost identical. Even with a load of $100\%$ and a margin of $0$, we needed to draw $100,000$ random instances before finding one which can be solved by FPT-PMLS and not by PMLS. Since FPT-PMLS is of exponential complexity in $n$, it does not seem relevant to use it. We now present the computing time of PMLS and FPT-PMLS to understand the size of instances which can be solved. To stress the algorithms, we fix the margin to $0$ and the load to $95\%$. The table on Figure~\ref{fig:tps_fpt} shows the computation times of PMLS and  FPT-PMLS, averaged on $1,000$ instances. 
   ...
   \begin{tabularx}{0.5\textwidth}{|c|X|X|X|X|X|X|}
    ...
    \hline
    FPT-PMLS (ms) & $0.45$ &$6.02$&$39.73$&$170.9$&$3297$\\
    \hline
     PMLS (ms) & $0.04$ &$0.06$&$0.07$&$0.09$&$0.13$\\
    \hline
$ \frac{\text{FPT-PMLS}}{\text{PMLS}}$& $11$ &$57$&$538$&$1902$&$24881$\\
    \hline
      \end{tabularx}
      \end{center}
  ...
  The complexity of both of those algorithm depends on the number of routes. As shown in Figure~\ref{fig:tps_fpt}, PMLS scales linearly with the number of routes, while  cubic in theory and FPT-PMLS scales exponentially as expected. Both algorithms are usable for instances of $20$ routes, but for $40$ routes or more,  FPT-PMLS becomes to long. As a conclusion, since 
  FPT-PMLS almost never finds a solution when PMLS does not and is much slower, one should always prefer to use PMLS. 
...
%     
    ...
    We investigate the success rate of our algorithm with regards to the number of random orders drawn, a load of $95\%$ and a margin $0$. The table of Figure~\ref{fig:randomdrawing} presents the average success rate for each number of sending orders, on $10,000$ instances, for GD, PMLS and FPT-PMLS.
         \begin{figure}[h] 
       \begin{center}
   \begin{tabularx}{0.5\textwidth}{|c|X|X|X|X|X|X|}
    \hline
    ...
    \hline
    GD & $0.7$ &$6.32$&$34.0$&$75.7$&$90.1$&$92.4$\\
    \hline
  PMLS & $81.70$ &$98.88$&$99.77$&$99.83$&$99.83$&$99.77$\\
    \hline
    FPT-PMLS & $90.86$&$99.13$&$99.80$&$99.83$ &$99.83$&$99.77$\\
    \hline
      \end{tabularx}
      \end{center}
...
First, observe that the better the algorithm in stage $2$, the less random order it needs in stage $1$ to achieve almost optimal performances. 

FPT-PMLS gives some better results than PMLS for less than $1,000$ random orders, but not beyond. This enforce our choice to draw $1,000$ random orders, to have the same performances with PMLS, which have a better complexity.
Also, remark that we can improve our previous results by taking $10,000$ random orders,
instead of $1,000$. The number of different orders is $7!= 5,040$ since we have $8$ routes and the solutions are invariant up to a circular permutation of the order. Instead of doing $10,000$ random draws we could test every possible order in less time. However the computation time of this method scales badly with $n$. The fact that PMLS and FPT-PMLS have already high success rates for $10$ random orders hints that even for larger $n$, drawing $1000$ random order is sufficient.



     \subsubsection{Some harder topologies to stress PMLS}
     
     The previous experiments use instances where the weigths of the arcs in the network are uniformly drawn in a large interval. However, it is quite natural to consider that most routes are of the same length or can be arranged in two groups of similar sizes if the network involves two datacenters.
     
     Using Theorem~\ref{th:asym}, we can bound the margin by the difference
     between the size of the routes. Hence if the routes have all almost the same size, the needed margin is small. If the routes are drawn in a large interval, then the expected difference between the longest route and the second longest is large. This difference can be seen as a free latency for all routes, hence we expect to need little margin in this regime.  
     As a consequence, the harder instances should be for routes with length 
     drawn in an interval of a moderate size.
     
     The figures show the cumulative distribution of the margin needed by PMLS to find a solution, computed on $10,000$ instances.
     Figure~\ref{fig:1grp} represents the success rate of PMLS for 
     instances where length of routes are drawn in $[0,I]$, where $I$ goes from $0$ to $3200$. As expected the success rate decreases until $I$ is equal to $800$ and then increases again.   
    
     In Figure~\ref{fig:2grp}, we do the same experiment, except that the weight of half of the routes is drawn in $[0,I]$ and the length of the other half is drawn in $[P/2,P/2 + I]$. The situation is the same as for the previous experiment but with better success rate. 
          
              \begin{figure}
       \begin{center}
      \includegraphics[width = 0.5\textwidth]{departs_distrib1Grp.pdf}
      \end{center}
      \caption{Cumulative distribution of the margin needed to find a solution with 1 group of routes}
      \label{fig:1grp}   
     \end{figure}    
      ...
      \end{center}
    \caption{Cumulative distribution of the margin needed to find a solution with 2 group of routes}
      \label{fig:2grp}   
     \end{figure}    
     
     First, remark that, when we have one group of routes, it is harder to find some solutions than when we have two groups of routes. Also, as we can observe, the closer the size of the routes are from each others, the easier it is to find a solution for \pall. Moreover, when the range is large enough, the conflict between the routes is less critical and PMLS finds most of the time a solution. The most difficult instances to solves are then when all the routes are drawn in the same range of value, between $P-1,600$ and $P$. Indeed, in this case, only $87\%$ of the instances can be solved with margin $0$, and we need a margin of $1,600$ to ensure that PMLS finds a solution.
     This observation gives us some details on the network topologies which cannot be solved with PMLS. In practice, it can thus be interesting to artificially lengthen the size of some routes to avoid having several routes with similar lengths, and thus, increases the chance to find a solution to \pall.
...
% the forward and backward routes are not symmetric. For other applications, it could be interesting to optimize the average latency of the routes rather than the worst latency, a problem which may be solved using linear optimization.   
   ...
   Finally, the routes may not be fixed but chosen in the graph to minimize the maximum process time, which would make the problem even more difficult ($\Pi_2$-complete instead of $\NP$-complete). 
