...
\begin{figure}[h] 
  \centering
   ...
   \begin{tabularx}{0.5\textwidth}{|c|X|X|X|X|X|X|}
    ...
    \hline
$ \frac{\text{FPT-PMLS}}{\text{PMLS}}$& $27$ &$78$&$523$&$2122$&$14882$\\
    \hline
      \end{tabularx}
      \end{center}
...
<<<<<<< HEAD
  The complexity of both of those algorithm depends on the number of routes. As shown in Figure~\ref{fig:tps_fpt}, PMLS scales linearly with the number of routes, while  cubic in theory and FPT-PMLS scales exponentially as expected. Both algorithms are usable for instances of $20$ routes, but for $40$ routes or more,  FPT-PMLS becomes too slow. Since FPT-PMLS almost never finds a solution when PMLS does not and is much slower, one should always prefer to use PMLS. 
...
=======
 ...
 The complexity of both of those algorithm depends on the number of routes. However, PMLS have a polynomial complexity, while FPT-PMLS scales exponentially with the number of routes. As we measured on Figure~\ref{fig:tps_fpt}, the time taken to compute an instance with more than $20$ routes with FPT-PMLS is too long to be acceptable. It is not really worth it since, most of the time, the FPT-PMLS algorithm do not have a better success rate than the PMLS algorithm, which runs in few microseconds, even with a large number of routes.

>>>>>>> 2ff5d2f1a7f743cc03019147bf7ce74d4d17b7d4
...
%     
    ...
    We investigate the success rate of our algorithms with regards to the number of random orders drawn, a load of $95\%$ and a margin $0$. The table of Figure~\ref{fig:randomdrawing} presents the average success rate for each number of sending orders, on $10,000$ instances, for GD, PMLS and FPT-PMLS.
         \begin{figure}[h] 
       \begin{center}
   \begin{tabularx}{0.5\textwidth}{|c|X|X|X|X|X|X|}
    \hline
...
First, observe that the better the algorithm to solve $\bra$, the less random order it needs in stage one to achieve its best performances. In particular, FPT-PMLS has better results than PMLS for less than $1,000$ random orders, but not beyond. This justify our choice to draw $1,000$ random orders, to obtain the best success rate with the best time complexity.

Also, remark that the number of different orders is $7!= 5,040$ since we have $8$ routes and the solutions are invariant up to a circular permutation of the order. Hence for $8$ routes, itis possible to test every possible order. However the computation time of this method scales badly with $n$. The fact that PMLS and FPT-PMLS have already high success rates for $10$ random orders hints that even for larger $n$, drawing $1000$ random order is sufficient.



     \subsubsection{Harder random topologies}
     
     The previous experiments use instances where the weights of the arcs in the network are uniformly drawn in a large interval. However, it is quite natural to consider that most routes are of the same length or can be arranged in two groups of similar lengths if the network involves two data-centers.
     
     Using Theorem~\ref{th:asym}, we can bound the minimum margin by the difference
     between the size of the routes. Hence if the routes have all almost the same size, the needed margin is small. If the routes are drawn in a large interval, then the expected difference between the longest route and the second longest is large. This difference can be seen as a free latency for all routes, hence we expect to need little margin in this regime. As a consequence, the harder instances should be for routes with length 
     ...
     Figure~\ref{fig:1grp} represents the success rate of PMLS for instances where length of routes are drawn in $[0,I]$, where $I$ goes from $0$ to $3200$. As expected the success rate decreases for a given margin until some point ($I = 800$) and then increases again.   
     In the the most difficult settings only $87\%$ of the instances can be solved with margin $0$, and we need a margin of $1,600$ to ensure that PMLS finds a solution.
    
    
    
     In Figure~\ref{fig:2grp}, we do the same experiment, except that the weight of half of the routes is drawn in $[0,I]$ and the length of the other half is drawn in $[P/2,P/2 + I]$. The situation is the same as for the previous experiment but with better success rates. 
