...
\begin{figure}[h] 
  \centering
    ...
    \begin{figure} [h] 
     ...
     The GD algorithms is close to $100\%$ success rate for margins larger than $1,500$ while the PMLS and FPT-PMLS algorithms find a solution in more than $99\%$ of the experiments, even with a margin $0$. Therefore, for the worst possible constraints on load and margin, there are only a few instances for which we do not find a solution. With a margin of $300$, which corresponds to about $15\mu$s of additional delay with the chosen parameters, we always find a solution. 
     Note that the PMLS and FPT-PMLS algorithm have some very similar performances. In fact, it appears that the FPT-PMLS algorithm have, most of the time, the same success rate than the PMLS algorithm, even with a load of $100\%$ and a margin of $0$ (only $2$ instances of $100,000$ have been found by FPT-PMLS and not by PMLS). 
...
%     
  %   Moreover, by studying the computation time of the FPT-MLS and the PMLS algorithm during the experience of Figure~\ref{fig:success21000}, we remark that the average computation time to find a solution is often $10$ times higher with FPT-MLS than with PMLS ($\sim300\mu s$ for FPT-MLS versus $\sim30\mu s$ for PMLS). Nevertheless this remark is true when PMLS finds a solution at $100\%$. On the other hand, when PMLS can not find a solution that FPT-MLS finds, because the algorithm needs to draw the $1000$ random orders before failing, the computation time of an instance by PMLS is, in average, in the same range as the one with FPT-MLS ($\sim 1ms)$.
      Then, we study the evolution of the computing time of the PMLS and the FPT-PMLS algorithms when the number of routes grows. In the following experiment, we fix the margin to $0$. In order to stress the algorithms, the period is chosen so that the load is always $95\%$, and we draw uniformly the weights of the arcs between $0$ and $P$. Since we only want to measure the computing time of the algorithms of the \bra problem, we draw $1$ random order for each instances. The table on Figure~\ref{fig:tps_fpt} shows the computation times of the PMLS and the FPT-PMLS algorithms, averaged on $1,000$ instances. 
...
$ \frac{\text{FPT-PMLS}}{\text{PMLS}}$& $11$ &$57$&$538$&$1902$&$24881$\\
    \hline
      \end{tabularx}
      \end{center}
   \caption{Scalability of PMLS and FPT-PMLS computing time with the number of routes}
        \label{fig:tps_fpt}
     \end{figure}
