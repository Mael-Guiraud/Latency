...
We give experimental evidences that they find periodic assignments when the network is not too loaded. Finally in Section\ref{sec:PALL}, we propose polynomial time heuristics and an exact FPT algorithm for the general \pall problem and experimentally show that they work extremely well even in loaded networks. In particular we show that the deterministic communication schemes we design largely outperform the traditional stochastic multiplexing with regard to latency. 
...
\end{tikzpicture}
  ...
  the satisfying assignment. If we subtract its weight to $d(r)$ for each $r$, and set its
  weights to zero in $G'$, then the constraints $PT(r) \leq d(r)$ will be the same in $I$ and $I'$.
...
%      \STATE  deadline[$r_i$]  $\leftarrow$  $m_{i} + T_{max} - \omega(s_i,c_s)$
 ...
 \STATE insert(${\cal H}$,$r_i$).
    ...
    or looking for the next free interval, the time needed to maintain it is $O(n)$. 
     ...
     Remark that we do not deal with the periodicity. When MLS finds an assignment $m$, it always satisfies $PT(r) < d(r)$ for all $r$. On the other hand, it is a $(P,\tau)$ periodic assignment only if $m_{max} - m_{min} \leq P -\tau$ where $m_{max}$ is the largest offset and $m_{min}$ the smallest one. The algorithm we use minimize this quantity, so it may work in practice (as shown in Section~\ref{sec:resultsPALL}), but not for all instances.
     
     We now present a variant of the previous algorithm that we call {\bf Periodic Minimal Latency Scheduling (PMLS)}
     which takes into account the periodicity. We set arbitrarily a backward route so that its message is the first to go through the central arc at time $t$. Then we modify the deadline of each backward route to be the minimum of their previous deadline and $t + P - \tau$. Because of this choice of  deadlines, MLS finds a solution for this instance, it is always a $(P,\tau)$ periodic assignment since all messages goes through $c_t$ between $t$ and $t+P-1$.  The algorithm PMLS, is the execution of MLS for each possible first backward route with the modifications to the deadlines until we find a solution otherwise the algorithm fails. Since we run MLS at most $n$ times, the complexity is in $O(n^3\log(n))$. Remark that PMLS finds an assignment more often than MLS, because if MLS finds a solution with first route $r$ and such that $m_{max} - m_{min} \leq P -\tau$, then this solution will be found by PMLS when it selects $r$ as the first route. We take the periodicity into account in PMLS but not completely and it may fail to find assignments. Indeed, in PMLS we restrict the assignments we look for by not allowing to choose an offset for a route after $t+P- \tau$.
...
The algorithm of Theorem~\ref{th:braFPT} has a complexity of $O(2^nn^3\log(n))$. Moreover, for each backward route to be scheduled between $P$ and $2P-\tau$, it must have a deadline larger than $P + \tau$ since between $P$ and $P + \tau$ the first message goes through the central arc. Often, only a small fraction of the routes have a deadline larger than $P + \tau$, say $t$ of them and we explore only $2^t << 2^n$ cases.
...
and  when $y_i \geq P - \tau$ we replace $y_i = x_i + \lambda_i + w_i$ by $y_i = x_i + \lambda_i + w_i - P$ and we replace the deadline $d(r_i)$ by $d(r_i) -P$. In that way we guarantee that all $y_i$ are less than $P-\tau$.  
...
%      \todo{ajouter l'algo FPT qui ameliore PMLS}
    ...
    In Figure~\ref{fig:randomdrawing}, we investigate the success rate of our algorithm with regards to the number of random orders drawn, a load of $95\%$ and a margin $0$. The following tables presents the average success rate for each number of sending orders, on $10,000$ instances, for GD and PMLS.
         \begin{figure}[h] 
       \begin{center}
