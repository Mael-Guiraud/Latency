...
%DIF DEL juillet.tex          Wed Dec  6 16:00:04 2017
%DIF ADD version_longue.tex   Thu Dec  7 17:23:58 2017
...
%DIF -------
\usepackage[colorlinks=true,breaklinks=true,linkcolor=blue]{hyperref} %DIF > 
%DIF -------
...
%DIF 43c44
%DIF < \renewcommand{\thefootnote}{\*}
%DIF -------
% \renewcommand{\thefootnote}{\*} %DIF > 
%DIF -------
...
%DIF 53-54c53-54
%DIF < %\author[1]{Dominique Barth}
%DIF < %\author[1]{Ma\"el Guiraud}
%DIF -------
\author[1]{Dominique Barth} %DIF > 
\author[1]{Ma\"el Guiraud} %DIF > 
%DIF -------
% \author[1]{Christian Cad\'er\'e}
%DIF < %\author[2]{Brice Leclerc}
%DIF -------
\author[2]{Brice Leclerc} %DIF > 
%DIF < %\author[2]{Olivier Marc\'e}
\author[2]{Olivier Marc\'e} %DIF > 
%DIF < %\author[1]{Yann Strozecki}
\author[1]{Yann Strozecki} %DIF > 
%DIF < %\affil[1]{David Laboratory, UVSQ}
\affil[1]{David Laboratory, UVSQ} %DIF > 
%DIF < %\affil[2]{Nokia Bell Labs France}
\affil[2]{Nokia Bell Labs France} %DIF > 
...
antennas \DIFaddbegin \DIFadd{until now}\DIFaddend . The main challenge is to guarantee that the latency of the periodic messages sent from the antennas to their processing
units and back\DIFaddbegin \DIFadd{, }\DIFaddend fulfills protocol time constraints. We show that traditional statistical multiplexing does not allow such a low latency, due to collisions and buffering at nodes. Hence, we propose in this article to use a deterministic scheme for sending periodic messages without collisions in the network thus \DIFdelbegin \DIFdel{without buffering and the resulting additional latency . 
}\DIFdelend \DIFaddbegin \DIFadd{saving the latency incurred by buffering.
}

\DIFaddend We give several algorithms to compute such schemes for a common topology where one link is shared by all antennas.
...
%DIF < \footnote{This work is partially supported by the french ANR project N-GREEN.}
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \section{Introduction}

Next generations of mobile network architectures evolve toward centralized radio network architectures called C-RAN for Cloud Radio Access Network, to reduce energy consumption costs~\cite{mobile2011c} and more generally the total cost of ownership. The main challenge for this type of architecture is to reach a latency compatible with transport protocols. The latency is measured between the sending of a message by a Remote Radio Head (RRH) and the receptions of the answer, computed by real-time virtualized network functions of a BaseBand Unit (BBU)\footnote{Others terminologies exist in the literature. The results of this work are fully compatible with any C-RAN architecture definition.} in the cloud. For example, LTE standards \DIFdelbegin \DIFdel{requires }\DIFdelend \DIFaddbegin \DIFadd{require }\DIFaddend to process functions like HARQ (Hybrid Automatic Repeat reQuest) in $3$ms~\cite{bouguen2012lte}. In 5G, some services need \DIFdelbegin \DIFdel{end to end }\DIFdelend \DIFaddbegin \DIFadd{end-to-end }\DIFaddend latency as low as $1$ms~\cite{3gpp5g}. The specificity of the C-RAN context is not only the latency constraint, but also the periodicity of the data transfer in the \emph{frontaul} network between RRHs and BBUs: frames need to be emitted and received each millisecond~\cite{bouguen2012lte}.
Our aim is to operate a C-RAN on a low-cost shared switched network.
The question we address is the following: is it possible to schedule messages such that there \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{are }\DIFaddend no collisions to avoid latency caused by queuing delays? 
Eliminating this source of latency leaves us with more time budget for latency \DIFdelbegin \DIFdel{coming from }\DIFdelend \DIFaddbegin \DIFadd{due to }\DIFaddend the physical length of the routes in the network, and thus allows for wider deployment areas.


\DIFaddbegin \renewcommand{\thefootnote}{\*}

\footnote{\DIFadd{This work is partially supported by the french ANR project N-GREEN.}}

\DIFaddend Let us expose briefly our model: the network topology is modeled by a directed weighted graph and a set of paths (routes) from source nodes (RRHs) to target nodes (BBUs). Time is discretized and a unit of time or slot corresponds to the \DIFdelbegin \DIFdel{transmission of }\DIFdelend \DIFaddbegin \DIFadd{time needed to transmit }\DIFaddend a minimal unit of data over the network. \DIFdelbegin \DIFdel{The process must be  predictable (}\DIFdelend \DIFaddbegin \DIFadd{Since statistical multiplexing does not ensure a good latency we want to avoid any  buffering in internal nodes of the graph.  We take advantage of the deterministic nature of the messages we must manage }\DIFaddend i.e. the \DIFdelbegin \DIFdel{date }\DIFdelend \DIFaddbegin \DIFadd{dates }\DIFaddend of arrival of \DIFdelbegin \DIFdel{packets is known beforehand). Following }\DIFdelend \DIFaddbegin \DIFadd{messages are known beforehand. In fact, following }\DIFaddend LTE standard~\cite{bouguen2012lte}, \DIFdelbegin \DIFdel{in this article }\DIFdelend we assume that the \DIFdelbegin \DIFdel{process is periodic . That is, if we look at }\DIFdelend \DIFaddbegin \DIFadd{arrivals of all the packets are periodic with the same period. We propose to design a }\emph{\DIFadd{periodic}} \DIFadd{process to send the messages through the network without collisions. By periodic process we mean that }\DIFaddend the network at times $t$ and $t+P$ where $P$ is the period, \DIFdelbegin \DIFdel{all messages are at the same position in the network. 
As we want to avoid any  buffering in internal nodes of the graph}\DIFdelend \DIFaddbegin \DIFadd{is in the exact same state. 
}

\DIFadd{We assume that the routes of the messages are already fixed, and there are no buffering allowed inside the network}\DIFaddend . Hence we \DIFdelbegin \DIFdel{need to design a periodic process with no collision between messages. To ensure that this property is always true we propose a deterministic process.  We }\DIFdelend \DIFaddbegin \DIFadd{only }\DIFaddend have two sets of values that we can set when building a periodic sending process, called a \emph{periodic assignment}\DIFdelbegin \DIFdel{in this article}\DIFdelend : the time at which each packet is sent by a RRH in each period and the waiting time in the BBU before the answer is sent back to the RRH. When building a periodic assignment, we must take into account the periodicity which makes many scheduling methods unusable. Not only a message must not collide with the other messages sent by the others BBU/RRH in the same period, but also in the previous and following periods. The latency, that is the time between the emission of a message and the complete return of its answer must be minimized. This means that the only buffering we are allowed -- the waiting time before sending back the answer-- must be small, in particular when the route is long. Note that the model is technology agnostic, i.e. it is compatible with an optical network with a fixed packet size.   
...
\DIFaddbegin \DIFadd{The problem PALL on a star routed network is very close to a two flow shop scheduling problem~\mbox{%DIFAUXCMD
\cite{yu2004minimizing}
...
Either the aim is to minimize the number of processors on which the periodic tasks are scheduled~\mbox{%DIFAUXCMD
\cite{korst1991periodic,hanen1993cyclic} }\hspace{0pt}%DIFAUXCMD
while we have a single processor and a constraint similar to makespan minimization. Or, in cyclic scheduling~\mbox{%DIFAUXCMD
\cite{levner2010complexity}}\hspace{0pt}%DIFAUXCMD
, the aim is to minimize the period of a scheduling to maximize the throughput, while our period is fixed. 
}\DIFaddend 
...
The first vertex of a route models an antenna (RRH) and the last one a data-center (BBU) which processes the messages sent by the antenna.
   ...
   \subsection{Messages dynamic}
      ...
      then it goes through the same arc at time $t+kP$ for all positive integers $k$. Therefore, every time value can be computed modulo $P$ and we say that the first time slot at which a message sent at time $m$ on $r$ reaches a vertex $v$ in $r$ is \DIFdelbegin \DIFdel{$t(v,r) = m + \lambda(v,r) \mod P$}\DIFdelend \DIFaddbegin \DIFadd{$t(v,r) = m + \lambda(v,r)\mod P$}\DIFaddend . 

      A message usually cannot be transported in a single time slot. We denote by $\tau$ the number 
      of \emph{consecutive slots} necessary to transmit a message. \DIFaddbegin \DIFadd{In this paper, we assume that $\tau$ is the same for all routes. Indeed, the data flow sent by an RRH to its BBU is the same, regardless of the route. }\DIFaddend Let us call $[t(v,r)]_{P,\tau}$ the set of time slots used by route $r$ at vertex $v$ in a period $P$, that is $[t(v,r)]_{P,\tau} = \{t(v,r) + k \mod P \mid 0 \leq k < \tau \}$. Usually $P$ and $\tau$ will be clear from the context and we will denote $[t(v,r)]_{P,\tau}$ by $[t(v,r)]$.
      Let $r_1$ and $r_2$ be two routes, on which messages are sent at time $m_1$ and $m_2$ in their first vertex.
      We say that the two routes have a {\bf collision} if they share an arc \DIFdelbegin \DIFdel{of first vertex $v$ }\DIFdelend \DIFaddbegin \DIFadd{$(v,w)$ }\DIFaddend and $[t(v,r_{1})] \cap [t(v,r_{2})] \neq \emptyset$.
...
\begin{proof}
      ...
      \DIFdelbegin \DIFdel{Consider }\DIFdelend \DIFaddbegin \DIFadd{We consider a generalization of }\DIFaddend the routed network $(G,{\cal R})$ given in \DIFdelbegin \DIFdel{the previous subsection.We change the weights }\DIFdelend \DIFaddbegin \DIFadd{Fig.~\ref{fig:example}. 
      There are $n$ routes in the routed network and each pair of routes $(r_i,r_j)$ share a single arc, which is not in any other route. The weights are chosen }\DIFaddend so that for $v$, the first vertex which belongs to $r_i$ and $r_j$, we have $\lambda(v,r_i) - \lambda(v,r_j)= P$, where $P$ is an odd number smaller than $n$\DIFdelbegin \DIFdel{, the number of routes in ${\cal R_{\cal C}}$}\DIFdelend . In such a graph, there is no $(P,\tau)$-periodic assignment, since the problem reduces to finding a $P$-coloring in a complete graph with $n > P$ vertices, the colors being the offsets of the routes.\\
...
\end{proof}
      ...
      In the context of cloud-RAN applications, we need to send \DIFaddbegin \DIFadd{a }\DIFaddend message from a RRH $u$ to a BBU $v$ and then 
      we must send the answer from $v$ back to $u$. We say that a routed network $(G, {\cal R})$ is \textbf{symmetric} if the set of routes is partitioned into the sets $F$ of forward routes and $B$ of backward routes. Moreover there is a bijection $\rho$ between $F$ and $B$ such that for any forward route $r \in F$ with first vertex $u$ and last vertex $v$, the backward route $\rho(r) \in B$ has first vertex $v$ and last vertex $u$. In all practical cases the routes $r$ and $\rho(r)$ will be the same with the orientation of the arcs reversed, which corresponds to \DIFdelbegin \DIFdel{bidirectional links }\DIFdelend \DIFaddbegin \emph{\DIFadd{bidirectional links}} \DIFaddend in the networks, but we need not to enforce this property.
...
%DIFDELCMD <       \includegraphics[width=0.47\textwidth]{rrh.pdf}
%DIFDELCMD <       \end{center}
%DIFDELCMD <       

%DIFDELCMD <       
%DIFDELCMD <       %%%
\DIFdelend This message is received by $v$\DIFaddbegin \DIFadd{, i.e., }\DIFaddend the last vertex of $r$ at time $t(v,r)$. It is then sent back to $u$ on the route $\rho(r)$ in the same period at time $m_{\rho(r)}$ if $m_{\rho(r)} > t(v,r)$, otherwise at time $m_{\rho(r)}$ in the next period. The time between the arrival of the message and the time it is sent back is called the \textbf{waiting time} and is defined by $w_r = m_{\rho(r)} - t(v,r)$ if $m_{\rho(r)} > t(v,r)$ and $w_r = m_{\rho(r)} + P - t(v,r)$ otherwise.

     \DIFaddbegin \begin{figure}
      \DIFaddendFL \begin{center}
      \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.47\textwidth]{BBU.pdf}
%DIFDELCMD <       %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.47\textwidth]{rrh.pdf}
      \DIFaddendFL \end{center}
      \DIFaddbeginFL \caption{\DIFaddFL{Periodic process}}
      \end{figure}
      \DIFaddend 

      Thus, the whole process time for a message sent on the route $r$ is equal to
      $PT(r)=\lambda(r)+ w_r+\lambda(r)$.      
      In the process time, we count the time between the \DIFdelbegin \DIFdel{first time at which a }\DIFdelend \DIFaddbegin \DIFadd{time the first slot of the }\DIFaddend message is emitted and the first time at which the \DIFaddbegin \DIFadd{first slot of the }\DIFaddend message comes back. Alternatively we could consider the time between the emission of the first slot and the reception of the last slot of the message, which \DIFdelbegin \DIFdel{would add }\DIFdelend \DIFaddbegin \DIFadd{adds }\DIFaddend $\tau$ to the process time.
      \DIFdelbegin \DIFdel{However, since all messages are of size $\tau$, it will }\DIFdelend \DIFaddbegin \DIFadd{We chose the first definition which is slightly simpler since the choice of definition does }\DIFaddend not change the problems we consider in the rest of the article\DIFaddbegin \DIFadd{, assuming all messages are of size $\tau$}\DIFaddend .
      Finally, we could add to the process time the computation time a BBU needs to deal with one message, but it can be encoded  in the weight of the last arc leading to the BBU and thus we \DIFdelbegin \DIFdel{need not }\DIFdelend \DIFaddbegin \DIFadd{do not need }\DIFaddend to consider it explicitly in our model.
    ...
    The {\bf maximum process time} of the assignment $m$ of $(G,{\cal R})$ is defined by $MPT(m)=\max\limits_{r \in {\cal R}} PT(r)$. We consider the following decision problem.
      ...
      In Sec.~\ref{sec:PALL} we will study heuristics \DIFdelbegin \DIFdel{which try }\DIFdelend \DIFaddbegin \DIFadd{used }\DIFaddend to solve the search version of PALL (computing an assignment), also denoted by PALL for simplicity. \DIFaddbegin \DIFadd{In PALL, we have chosen to bound the }\emph{\DIFadd{maximum}} \DIFadd{process time, that is the worst case latency. It is justified by our C-RAN application with hard constraint on the latency. It would be interesting to study the case of a constraint on the }\emph{\DIFadd{average}} \DIFadd{process time, which may be more relevant in other contexts.
}\DIFaddend 
    ...
    \hline
     ...
     \hline
      ...
      $\lambda(u_i,r)$ & Latency of the vertex $u_i$ in $r$\\
         ...
         \hline
         $ [t(v,r)]$& Set of time slots used by route $r$ at vertex $v$ \DIFdelbegin %DIFDELCMD < \\
%DIFDELCMD <          & %%%
\DIFdelend in a period $P$\\
         \hline 
...
\section{Hardness of PRA}
 ...
 We will prove the hardness of PRA and PALL for $\tau =1$ which implies the hardness of \DIFdelbegin \DIFdel{the general problems }\DIFdelend \DIFaddbegin \DIFadd{problems with $\tau$ unconstrained}\DIFaddend . 
...
the graph can be seen as a set of disjoint pair of routes, on which PRA and PALL can be solved in linear time. 
 The second proof reduces the problem to graph coloring and implies inapproximability when one tries to find the smallest possible $P$.  \DIFdelbegin %DIFDELCMD < \\
%DIFDELCMD <  %%%
...
In particular, this reduction shows that even with small maximal load, the minimal period can be large.
    ...
    \begin{figure}[ht]
    \centering
    \DIFdelbeginFL %DIFDELCMD < \scalebox{0.37}{
%DIFDELCMD <     \begin{tikzpicture}
%DIFDELCMD <     \tikzset{
%DIFDELCMD <       LabelStyle/.style = { rectangle, rounded corners, draw,
%DIFDELCMD < 			  font = \bfseries },
%DIFDELCMD <       EdgeStyle/.append style = {->} }
%DIFDELCMD <       \SetGraphUnit{5}
%DIFDELCMD <       
...
%DIFDELCMD <       \tikzstyle{VertexStyle}=[shape = circle, draw, minimum size = 20pt]
%DIFDELCMD < 	\tikzset{
%DIFDELCMD <       VertexStyle/.append style = {blue} }
%DIFDELCMD < 	\Vertex[x=-8,y=3]{1}
%DIFDELCMD < 	      \tikzset{
%DIFDELCMD <       VertexStyle/.append style = {green} }
%DIFDELCMD < 	  \Vertex[x=-7,y=5]{2}
%DIFDELCMD < 

%DIFDELCMD < 	    \tikzset{
%DIFDELCMD <       VertexStyle/.append style = {red} }
%DIFDELCMD < 	  \Vertex[x=-6,y=4]{3}
%DIFDELCMD < 		\tikzset{
%DIFDELCMD <       VertexStyle/.append style = {black} }
%DIFDELCMD <       

%DIFDELCMD <       
%DIFDELCMD <       \SetVertexNoLabel
%DIFDELCMD <       \Vertex[x=2,y=5]{A}
%DIFDELCMD <       \Vertex[x=4,y=5]{B}
%DIFDELCMD <       \Vertex[x=10,y=5]{C}
%DIFDELCMD <       \Vertex[x=12,y=5]{D}
%DIFDELCMD <       \Vertex[x=6,y=3]{E}
%DIFDELCMD <       \Vertex[x=8,y=3]{F}
%DIFDELCMD <       \tikzset{
%DIFDELCMD <       EdgeStyle/.append style = {green} }
%DIFDELCMD <       \Edge(s2)(A)
%DIFDELCMD <       \Edge(A)(B)
%DIFDELCMD <       \Edge(B)(C)
%DIFDELCMD <       \Edge(C)(D)
...
%DIFDELCMD <       
%DIFDELCMD <       \tikzset{
%DIFDELCMD <       EdgeStyle/.append style = {red} }
%DIFDELCMD <       \Edge(s3)(E)
%DIFDELCMD <       \Edge(E)(F)
%DIFDELCMD <       \Edge(F)(t3) 
%DIFDELCMD < 	\tikzset{
%DIFDELCMD <       EdgeStyle/.append style = {blue} }
%DIFDELCMD <       \Edge(s1)(A)
%DIFDELCMD <       \Edge(A)(B)
%DIFDELCMD <       \Edge(B)(E)
%DIFDELCMD <       \Edge(E)(F)
...
%DIFDELCMD < 	\tikzset{
%DIFDELCMD <       EdgeStyle/.append style = {black,-} }
%DIFDELCMD < 
...
%DIFDELCMD <     }
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \scalebox{0.37}{
    \begin{tikzpicture}
    \tikzset{
      LabelStyle/.style = { rectangle, rounded corners, draw,
			  font = \bfseries },
...
%     \node (3) at (10,0){\Huge G};
    ...
    \DIFaddendFL \caption{Reduction from k-coloring to MIN-PRA}
    \label{fig:reduction}
    \end{figure}
...
\end{tikzpicture}
  ...
  \end{center}
  \DIFaddbeginFL \caption{\DIFaddFL{A star routed network}}
  \end{figure}
	\DIFaddend 
  ...
  \DIFdelbegin \DIFdel{One may consider a variant of PAZL where the }\DIFdelend \DIFaddbegin \DIFadd{The problem PAZL is similar to the problem of minimizing makespan in a two flow-shop with delays, a problem known to be $\NP$-complete~\mbox{%DIFAUXCMD
\cite{yu2004minimizing}}\hspace{0pt}%DIFAUXCMD
. It suggests that PAZL is $\NP$-complete, however we have not been able to prove it yet,
  because the periodicity is hard to deal with. With additional constraints we can get rid of the periodicity in the hardness proof, for instance if we consider that the }\DIFaddend central link is unidirectional, that is collisions can happen between messages going from $c_s$ to $c_t$ and messages  going from $c_t$ to $c_s$. This \DIFdelbegin \DIFdel{problem }\DIFdelend \DIFaddbegin \DIFadd{variant }\DIFaddend can be shown to be $\NP$-complete by a reduction from the subset sum problem as it is done for a similar problem of scheduling pair of tasks~\cite{orman1997complexity}.
  \DIFdelbegin \DIFdel{We conjecture that the problem PAZL is also $\NP$-complete,  but we have yet not been able to prove it.
  }\DIFdelend On the other hand we show positive results:  when the period is large or when the routes are short there is always a solution to PAZL and it can be found in polynomial time. We then give \DIFdelbegin \DIFdel{an exponential time exhaustive search algorithm which always }\DIFdelend \DIFaddbegin \DIFadd{fixed parameter tractable algorithm which }\DIFaddend finds a solution \DIFaddbegin \DIFadd{to PAZL }\DIFaddend if there is one \DIFaddbegin \DIFadd{in time exponential in the number of routes only}\DIFaddend . 
...
\subsection{Shortest-longest policy}
    ...
    In other words, we assume that the route $r_i$ are sorted by increasing $\lambda(r_i)$ and we set $m_{r_i}$ the offset of $r_i$ to $i\tau$. We call this algorithm {\bf Shortest-Longest}.
     ...
     By definition, there are no \DIFdelbegin \DIFdel{collision }\DIFdelend \DIFaddbegin \DIFadd{collisions }\DIFaddend in the forward period and if the period is long enough, 
...
\DIFdelend \DIFaddbegin \begin{figure}
      \DIFaddendFL \begin{center}
      \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.48\textwidth]{ex3nt.png}
%DIFDELCMD <       %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.48\textwidth]{ex3nt.pdf}
      \DIFaddendFL \end{center}
      \DIFaddbeginFL \caption{\DIFaddFL{Forward and backward period, represented at step $k$ of the algorithm}}
      \end{figure}
...
This algorithm, contrarily to the previous one, may work well, even \DIFdelbegin \DIFdel{when the condition on the load is not true}\DIFdelend \DIFaddbegin \DIFadd{from load higher than $1/3$}\DIFaddend .
...
\end{theorem}
\begin{proof}
Let $(G, {\cal R})$ be a star routed network and let $m$ be a $(P,\tau)$-periodic assignment of $(G, {\cal R})$. First\DIFaddbegin \DIFadd{, }\DIFaddend remark that for a given assignment and a route $r_0$ with offset $m$, by removing $m$ to all offsets, we can always assume that its offset is zero. Therefore we need only to consider all \emph{compact assignments} with an \emph{offset $0$} for the route $r_0$. 
We now evaluate the number of compact assignments and prove that it only depends
on $n$ the number of routes which proves the theorem. \DIFdelbegin \DIFdel{We give }\DIFdelend \DIFaddbegin \DIFadd{To count the compact assignment, we describe }\DIFaddend a way to build a compact assignment $m$ by determining its offsets one after the other\DIFaddbegin \DIFadd{, which gives a bound on their number and an algorithm to generate them all}\DIFaddend . We fix an arbitrary total order on ${\cal R}$.
...
tests at each step whether there is a collision\DIFaddbegin \DIFadd{, }\DIFaddend which can be done in time linear in the size of 
...
%       which makes the first cut more likely to apply. A last cut could be implemented: compute for every route not in $R(S)$ the set of possible positions in the backward period and verify whether at least one is contained in the useful slots of $S$.
   ...
   \subsection{Experimental evaluation}\label{sec:exp_PAZL}
      ...
      Notice that both Greedy algorithm and Shortest-Longest are polynomial time algorithms but are not always able to find a solution, depending on the load or the size of the routes. On the other hand, the exhaustive search finds \DIFdelbegin \DIFdel{an optimal }\DIFdelend \DIFaddbegin \DIFadd{a }\DIFaddend solution if it exists, but works in exponential time. We compare the performance of the algorithms in two different regimes: routes are either short with regard to $\tau$, or unrestricted.
      From \DIFdelbegin \DIFdel{our }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend C-RAN context we choose the following parameters: the number of routes is at most $n = 20$, $\tau$ is equal to $2,500$.
      It corresponds to \DIFaddbegin \DIFadd{slots of $64$ bits, }\DIFaddend messages of approximately $1$Mb \DIFdelbegin \DIFdel{for }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend links of bandwidth $10$Gbps.
       The code in C \DIFdelbegin \DIFdel{will be }\DIFdelend \DIFaddbegin \DIFadd{is }\DIFaddend available on the webpage of the authors%DIF <         ~\cite{webpage}
      \DIFaddbegin \footnote{\url{http://www.prism.uvsq.fr/~ystr/textesmaths.html}} \DIFaddend under a copyleft license. \DIFaddbegin \DIFadd{The code has been run on a standard $2016$ laptop, and most experiments run in a few dozen of seconds.
}\DIFaddend 

       In \DIFdelbegin \DIFdel{our }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend experiments we try to understand how \DIFdelbegin \DIFdel{our }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend algorithms work with regards to the load. To change the load, we fix the parameters $\tau$ and $n$ and modify the period $P$, which allows for a \DIFdelbegin \DIFdel{smoother }\DIFdelend \DIFaddbegin \DIFadd{smooth }\DIFaddend control of the load and does not \DIFdelbegin \DIFdel{change }\DIFdelend \DIFaddbegin \DIFadd{impact }\DIFaddend the run time of \DIFdelbegin \DIFdel{our }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend algorithms.
      ...
      In our experiment, we generate $1,000$ random instances of \pazl for $1$ to $14$ routes. We represent in Fig.~\ref{fig:short} the average \DIFdelbegin \DIFdel{maximal possible load , }\DIFdelend \DIFaddbegin \DIFadd{of the maximal load }\DIFaddend for which each algorithm finds a solution. \DIFdelbegin \DIFdel{Remind that the exhaustive search always finds the highest load for a given instance}\DIFdelend \DIFaddbegin \DIFadd{A bound on the load is given by the exhaustive which always finds a solution if there is one}\DIFaddend . 
%       The lower and upper bound $n\tau$ and $3n\tau$ are also represented.
	 ...
	 \includegraphics[width=0.47\textwidth]{periode_petite.pdf}
      \end{center}
      \caption{\DIFdelbeginFL \DIFdelFL{Minimum period }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Maximal load }\DIFaddendFL averaged over $1,000$ random instances}\label{fig:short}
      \end{figure}
...
\begin{figure}[h]
     ...
     \end{figure}
      ...
      The computation time of the exhaustive search is \DIFdelbegin \DIFdel{in }\DIFdelend \DIFaddbegin \DIFadd{bounded by }\DIFaddend $O(n!)$ as shown \DIFdelbegin \DIFdel{int }\DIFdelend \DIFaddbegin \DIFadd{in }\DIFaddend Th.~\ref{th:FPT}, 
      \DIFdelbegin \DIFdel{hence we need to study its scalability  }\DIFdelend \DIFaddbegin \DIFadd{but it can be much better in practice, either because it finds a solutions quickly or because a large part of the tree of 
      compact assignments is pruned during the algorithm. We study the scalability  of the algorithm }\DIFaddend when $n$ grows \DIFdelbegin \DIFdel{. In }\DIFdelend \DIFaddbegin \DIFadd{in }\DIFaddend the following experiment\DIFdelbegin \DIFdel{, the }\DIFdelend \DIFaddbegin \DIFadd{. The }\DIFaddend weights of the arcs $(c_t,t_i)$ are drawn following an uniform distribution between $0$ and $20,000$ slots. We chose  $95\%$ of load.  %DIF <        , that corresponds to loads of $\simeq 70\%$ (for $1$ route) to $\simeq 98\%$ (for 21 routes).
      The following table shows the time before the exhaustive search ends, for $8$ to $16$ routes, averaged on $100$ random star routed networks. This shows that for less than \DIFdelbegin \DIFdel{$15$ }\DIFdelend \DIFaddbegin \DIFadd{$16$ }\DIFaddend routes, which corresponds to all current topologies, the algorithm is efficient enough, but we should improve it further to work on more routes.
      \DIFaddbegin 
   ...
   Time (s) & $6.10^{-5}$&$8.10^{-4}$&$2.10^{-2}$& $0.4$& $11$\\
    \hline
      \end{tabularx}
      \end{center}
      \DIFaddbeginFL \caption{\DIFaddFL{Running time of the exhaustive search.}}
      \end{figure}
      \DIFaddend 

         \section{Solving \pall on Star Routed Networks}\label{sec:PALL}

    In this section, we consider the more general \pall problem on star routed networks. It means that the messages can wait in the target vertices (BBUs) \DIFdelbegin \DIFdel{in order }\DIFdelend to find assignments\DIFdelbegin \DIFdel{which is not possible for }%DIFDELCMD < \pazl%%%
\DIFdelend \DIFaddbegin \DIFadd{, when there are no assignments with zero waiting time}\DIFaddend . We allow the process time of the routes to be greater than twice the weights of the routes, but it should be less than $T_{max}$.
	...
	Another possible assumption would be that all weights of the arcs $(s_i,c_s)$ are the same.
	 ...
	 \end{theorem}
      \begin{proof}
        ...
        set the waiting times of all routes so \DIFdelbegin \DIFdel{they }\DIFdelend \DIFaddbegin \DIFadd{their messages }\DIFaddend behave exactly as the message in $r_0$.

        The offset of the forward route $r_i$ is set to $i\tau$, which ensures that there are no \DIFdelbegin \DIFdel{collision }\DIFdelend \DIFaddbegin \DIFadd{collisions }\DIFaddend on the arc $(c_s,c_t)$ as soon as $P \geq n\tau$ which is the minimal possible period. We set the waiting time for the route $i$ to $w_i = 2(\lambda(r_{0}) - \lambda(r_{i}))$. We can compute the time at which the message of the route $r_i$ arrives at the vertex $c_t$ on its way back: $t(c_t,\rho(r_i)) = w_i + i\tau + 2\lambda(r_{i})$
        by replacing $w_i$ by its value we obtain $t(c_t,\rho(r_i)) =  i\tau + 2\lambda(r_{0})$
        As a conclusion there are no \DIFdelbegin \DIFdel{collision }\DIFdelend \DIFaddbegin \DIFadd{collisions }\DIFaddend on the arc $(c_t,c_s)$ as soon as the 
        period is larger than $n\tau$ (there are no gaps between the messages).
        The process time of the route $r_i$ \DIFdelbegin \DIFdel{defined by }\DIFdelend \DIFaddbegin \DIFadd{is }\DIFaddend $PT(r_i) = w_i + 2\lambda(r_{i}) $. We obtain $PT(r_i) = 2\lambda(r_{0})$ and thus the maximum process time of the assignment is also equal to $2\lambda(r_0)$.
Finally the complexity is $O(n)$ since we have to find the maximum of the length of the $n$ routes and the computation of each $w_i$ is done by a constant number of arithmetic operations.
     ...
     Assume that for any pair of indices $(i,j)$ the difference between the weights of the arcs $(s_i,c_s)$ and $(s_j,c_s)$ is bounded by $d$ and the length of the longest route is $l$. Using the same idea as in Th.~\ref{th:asym}, there is an assignment with maximum process time bounded by  $2l + d$\DIFaddbegin \DIFadd{.
     }\DIFaddend 
   ...
   %DIF <      We tried the five following orders. 
%DIF < 	\begin{itemize}
%DIF < 	 
%DIF < 	 \item Longest-Shortest on Routes (LSR): Decreasing order on the length of the routes.
%DIF < 	 \item Shortest-Longest on Routes (SLR): Increasing order on the length of the routes. 
%DIF < 	 \item Longest-Shortest on last Arc (LSA): Decreasing order on the length of the arcs $(c_t,t_i)$.
%DIF < 	 \item Shortest-Longest on last Arc (SLA): Increasing order on the length of the arcs $(c_t,t_i)$. This sending order yields a $(P,\tau)$ periodic assignment in which all the $w_i = 0$, if the period is large enough (see proposition \ref{prop:SL}).
%DIF < 	 \item Random: A random order of the routes.
%DIF < 	\end{itemize}
%DIF <  
%DIF <     In the rest of the section we will study different methods to compute the offsets of the backward routes, once
%DIF <     the offsets of the forward routes are fixed by one of the previous orders.
%DIF <     
   \subsection{Greedy scheduling of backward routes}

    \DIFdelbegin \DIFdel{We can decompose any algorithm solving the }%DIFDELCMD < \pall %%%
...
\DIFdelbegin \DIFdel{The function  min\_non\_assigned(eligible\_time) returns the route with lowest time in eligible\_time, which is not assigned yet. The function update(t, free\_intervals) removes an interval of size $\tau$ beginning at t, which correspond to the message,  from free\_intervals.
    ...
    The \DIFdelbegin \DIFdel{greedy algorithm can be made to work in time $O(n\log(n))$.
    To do that the }\DIFdelend \DIFaddbegin \DIFadd{complexity of Algorithm~\ref{alg:GD} is $O(n\log(n))$, using the proper data structures.
    The }\DIFaddend set of eligible routes must be maintained in a binary heap
    to be able to find the one with smallest deadline in time $O(\log(n))$. 
    To deal with the possible collisions, one \DIFdelbegin \DIFdel{can maintain }\DIFdelend \DIFaddbegin \DIFadd{maintains }\DIFaddend a list of the intervals
    of time \DIFdelbegin \DIFdel{at }\DIFdelend \DIFaddbegin \DIFadd{during }\DIFaddend which a message can be sent on the arc $(c_t,c_s)$. Each time the offset of a 
    route is fixed an interval is split into at most two intervals in constant time. 
    Since the algorithm goes over the elements of \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{this }\DIFaddend list at most twice when doing an insertion
    or looking for the next free interval, the time needed to maintain it is $O(n)$. 
  %DIF <     
%DIF <      
%DIF <           We compared the two algorithms on $10000$ graphs on which the weight of the links are drawn uniformly between $0$ and $20000$, with a period of $21000$, and giving, for each instance $T_{max} = 2\lambda(r_{longest-route})$ . We then look at the success rate of each algorithms, with every sending order. Both algorithms reach the higher success rate with the Random sending order, but the success of Greedy is $5,65 \%$, while the success rate of Greedy Periodic is to $30,02 \%$.
...
%     \end{algorithm}
    ...
    We \DIFdelbegin \DIFdel{fix }\DIFdelend \DIFaddbegin \DIFadd{set }\DIFaddend the number of routes to $8$ to make comparisons with the results of Section~\ref{sec:exp_PAZL} easier. 
    We draw uniformly the weights of the arcs between $0$ and $20,000$. \DIFdelbegin \DIFdel{For a given routed network}\DIFdelend \DIFaddbegin \DIFadd{When studying many networks with different longest route's size, comparing the smallest process time for which an algorithm finds a solution is not relevant since it depends mostly on the size of the longest route. Hence}\DIFaddend , we define the {\DIFdelbegin %DIFDELCMD < \em %%%
\DIFdelend \DIFaddbegin \bf \DIFaddend margin} as the difference between $T_{max}$ and twice the longest route. \DIFdelbegin \DIFdel{Hence, for a given routed network choosing the margin or $T_{max}$ is the same, but since we will consider many networks with different longest route's size, it will be more meaningful to set a global margin rather than $T_{max}$.
    In fact the margin }\DIFdelend \DIFaddbegin \DIFadd{The margin }\DIFaddend represents the latency imposed by the communication process without taking into account the physical length of the network which cannot be changed. In our experiments the margin range from  $0$ to $3,000$.
   We look at two different regimes, a medium load of $80\%$ and a high load of $95\%$.
   Considering smaller load is not relevant since then we \DIFdelbegin \DIFdel{know how to }\DIFdelend solve the problem \DIFdelbegin \DIFdel{even }\DIFdelend without waiting times as shown in Section~\ref{sec:exp_PAZL}. 

   We first try to understand what is the best choice of order for the first stage of the algorithm which is followed by the GD algorithm in this experiment. In Fig.~\ref{fig:success95} \DIFaddbegin \DIFadd{and \ref{fig:success80}}\DIFaddend , we represent the success rate of the five kind of orders with regards to the margin. The value is an average computed over $10,000$ random star routed networks.  Note that for the random order, we compute $1000$ random orders and count it as a success as soon as there is a solution for one order. 

\begin{figure}[h] 
  \centering
          \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.5\textwidth]{departs_gp_21000.pdf}
%DIFDELCMD <       %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.5\textwidth]{departs_gp_25000.pdf}
      \DIFaddendFL \caption{Success rate of different \DIFaddbeginFL \DIFaddFL{sending }\DIFaddendFL orders, \DIFdelbeginFL \DIFdelFL{$95\%$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$80\%$ }\DIFaddendFL load.}
           \DIFdelbeginFL %DIFDELCMD < \label{fig:success95}
%DIFDELCMD <           %%%
\DIFdelendFL \DIFaddbeginFL \label{fig:success80}
     \DIFaddendFL \end{figure}

\begin{figure}[h] 
  \centering
    \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.5\textwidth]{departs_gp_25000.pdf}
%DIFDELCMD <           \vspace{-0.5cm}
%DIFDELCMD <       %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.5\textwidth]{departs_gp_21000.pdf}
      \DIFaddendFL \caption{Success rate of different \DIFaddbeginFL \DIFaddFL{sending }\DIFaddendFL orders, \DIFdelbeginFL \DIFdelFL{$80\%$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$95\%$ }\DIFaddendFL load.}
      \DIFdelbeginFL %DIFDELCMD < \label{fig:success80}
%DIFDELCMD <      %%%
\DIFdelendFL \DIFaddbeginFL \label{fig:success95}
          \DIFaddendFL \end{figure}
    ...
    Recall that the random order corresponds to the best of $1,000$ orders drawn uniformly which means that a computation can be up to $1,000$ time slower than for a fixed order. The choice of the number of random orders drawn yields a trade-off between the computation time and the success rate. Up to now, we \DIFdelbegin \DIFdel{chose $1,000$ random orders arbitrarily. We }\DIFdelend \DIFaddbegin \DIFadd{have chosen the number of random orders to be $1000$ arbitrarily.
    }

    \DIFadd{In Fig.~\ref{fig:randomdrawing}, we }\DIFaddend investigate the success rate of our algorithm with regards to the number of random orders drawn, a load of $95\%$ and a margin $0$. The following tables presents the average success rate for each number of sending orders, on $10,000$ instances, for GD and PMLS.
         \begin{figure}[h] 
       \begin{center}
  ...
  PMLS & $38.9$ &$83.0$&$95.3$&$97.2$&$97.5$&$97.7$\\
   ...
   \caption{Impact of the number of random sending orders}
        \DIFaddbeginFL \label{fig:randomdrawing}
     \DIFaddendFL \end{figure}

First\DIFaddbegin \DIFadd{, }\DIFaddend remark that we can improve our previous results by taking $10,000$ random orders,
instead of $1,000$. The number of different orders is $7!= 5,040$ since we have $8$ routes and the solutions are invariant up to a circular permutation of the order. Therefore instead of doing $10,000$ random draws we could test every possible order in less time. However the computation time of this method would scale badly with $n$. On the other hand, remark that the success rate of PMLS is already high for $100$ random orders, which means it will work even better than the other methods when $n$ is larger and that the number of random orders drawn becomes critical.
     ...
     In Fig.~\ref{fig:sto}, we represent the \DIFdelbegin \DIFdel{cumulative distribution of margins }\DIFdelend \DIFaddbegin \DIFadd{probability of success }\DIFaddend for 
     statistical multiplexing and PMLS \DIFaddbegin \DIFadd{for different margin}\DIFaddend . The distribution is computed from $1000$ star routed networks drawn with the same parameters as before. We represent the distribution under high and light load for statistical multiplexing and under high load only for PMLS since under light load the margin is always $0$. 
...
%DIFDELCMD <       %%%
\DIFdelendFL \caption{\DIFdelbeginFL \DIFdelFL{Cumulative distribution }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Probability }\DIFaddendFL of \DIFdelbeginFL \DIFdelFL{the margin }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{success }\DIFaddendFL of statistical multiplexing \DIFaddbeginFL \DIFaddFL{and PMLS for several margins}\DIFaddendFL }
      \label{fig:sto}   
     \end{figure}    
