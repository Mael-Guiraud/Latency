...
The model we study was recently introduced in~\cite{dominique2018deterministic} to find sending schedules for C-RAN messages, with \emph{buffering allowed}. This problem has also been studied for a cycle topology instead of a shared link~\cite{Guir1905:Deterministic}. In these works, the main results are heuristics, using classical scheduling algorithms as subroutines, and FPT algorithms which find a sending scheme with \emph{minimal latency}. They also briefly introduce and study the problem of finding sending schemes with \emph{no additional latency}, a problem which is the subject of this article under the name \pma. A simple greedy algorithm for \pma, which is guaranteed to work when the load of the link is less than $1/3$, is given in~\cite{dominique2018deterministic} and is recalled in Theorem~\ref{th:metaoffset}. In this article, we propose many new algorithms which solve \pma on instances of higer loads. In particular, we provide a very efficient algorithm for the special case of messages of size one and a proof that greedy algorithms work well on average instances.
...
The algorithms presented in this article are not directly applicable to this case, but we hope that several ideas presented here could be adapted.
...
The time is discretized and the process we consider is periodic of fixed integer \textbf{period $P$}. We use the notation $[P]$ for the set $\{0,\dots,P-1\}$. Since the process is infinite but periodic, it is enough to consider any interval of $P$ units of time to completely represent the state of our system by giving the times, in this interval, at which each message goes through the two contention points. We call the representation of the interval of time in the first contention point the \textbf{first period} and the \textbf{second period} for the second contention point. 
...
Given three messages $\{1,2,3\}$, then either $\{1,2\}$, $\{1,3\}$ or $\{2,3\}$ form a compact pair. 
\end{lemma}
\begin{proof}
...
On random instances, we expect the potential to be higher than the stated bound and to be better spread on the messages, which would make \swapandmove works for even larger loads, as it is indeed observed in experiments (see Appendix~\ref{sec:perf_small}).
...
Remark that combining Greedy Random and Th.~\ref{th:double_load} allows to solve $\pma$ on random instances, with probability one when the number of messages goes to infinity and the load is strictly less than $1/2$. 
...
\end{center}
\caption{An execution of \compactfit}
