...
We now precisely describe the \textbf{Swap and Move} algorithm:  messages are scheduled while possible by First Fit and then the Swap operation is applied while it increases the potential. When the potential is maximal, the algorithm tries to schedule a new message by moving at most two scheduled messages to other available offsets. If it fails to schedule a new message, the algorithm stops, otherwise the whole procedure is repeated. We now give an analysis of the algorithm, showing that it always works for a load striclty larger than $1/2$.
...
We now prove that Swap and Move is in polynomial time. Remark that all Swap operations 
