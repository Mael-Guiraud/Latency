...
\documentclass[a4paper,cleveref, autoref, thm-restate,UKenglish]{lipics-v2019}
...
The method of this section is described in~\cite{dominique2018deterministic} and achieves the same bound on the load using a different method. It is recalled here to help understand several algorithms in the article.
...
From now on, we use Lemma~\ref{lemma:multiple} to assume $P = m\tau$. It makes the analysis of the next algorithms simpler and tighter. The load changes from $n \tau / P$ to at most $n (\tau +1)/ P$: the difference is less than $n /P < 1/\tau$, thus very small for large $\tau$. The transformation of Lemma~\ref{lemma:multiple} does not give a bijection between assignments of both instances but only an injection, which is enough for our purpose. 
...
Let us precisely describe the algorithm \swapandmove:  messages are scheduled while possible by \firstfit and then the Swap operation is applied while it increases the potential. When the potential is maximal, the algorithm tries to schedule a new message by moving at most two scheduled messages to other available offsets. If it fails to schedule a new message, the algorithm stops, otherwise the whole procedure is repeated. We give an analysis of the algorithm, showing that it always works for a load striclty larger than $1/2$.

\begin{theorem}
The \swapandmove algorithm solves positively \pma, in polynomial time, for instances with $\tau =1$ and load less than $1/2 + (\sqrt{5}/2 -1) \approx 0,618$.
\end{theorem}
...
\subsection{To Size One by Increasing the Load}
...
the absence of conflict on the first period of \metaoffset and the compactness of the solution of \firstfit.
...
\end{center}
\captionof{figure}{Experiment for $\tau = 1000$, $P=100,000$, and $0 \leq d_i < \tau$ }
...
\end{center}
\caption{Computation time in logarithmic scale}
\label{fig:timelog}
\end{figure}
