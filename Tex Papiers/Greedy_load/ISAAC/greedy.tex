...
time is periodic and these are relative positions on a circle. Moreover, when it is unimportant and can hinder comprehension, we may omit to write $\mod P$ in some definitions and computations.
...
\end{figure}
\begin{theorem}
Compact Pairs always solves \pma positively in on instances of load less than
$3/8$.
\end{theorem}
\begin{proof}
...
produce enough compact $10$-tuples, that is more than $333$ messages. It is not a problem for two reasons. First, Lemma~\ref{lemma:uple_find} can be improved by using a better (polynomial time) algorithm to find compact tuples. We do not present it here because it is more complex to analyze. Second, on random instances, the probability that $k$ messages do not form a compact $k$-tuples is low, and we can just build the tuples greedily. Therefore, for most instances, forming compact $k$-uples is not a problem and in practice the algorithm works even for small $n$.
...
We now precisely describe the \textbf{Swap} algorithm:  messages are scheduled while possible by First Fit and then the Swap operation is applied while it increases the potential. When the potential is maximal, the algorithm tries to schedule a new message by moving one or two scheduled messages to other available offsets. If it fails to find such messages the algorithm stops, otherwise it continues repeating the same steps. We now give an analysis of the algorithm, showing that it always works for a load larger than $1/2$.

\todo{polynomial time, borne précise (dans les expériences bien meilleur)}

\begin{theorem}
The Swap algorithm solves positively \pma, in polynomial time for instances with $\tau =1$ and load $1/2 + (\sqrt{5}/2 -1) \approx 0,618$.
\end{theorem}

\begin{proof}
We determine for which value of the load the Swap algorithm
...
This explains why we have not presented nor analyzed in details an algorithm for large $\tau$ on random instances, since 
\todo{donner le nom de l'algo qu'on va rajouter, expliquer pourquoi limite} 
