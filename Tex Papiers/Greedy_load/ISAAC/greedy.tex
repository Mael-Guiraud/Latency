...
build more compact solutions. Then, we present deterministic and probabilistic algorithms for the case $\tau = 1$ in Sec.~\ref{sec:small}, which work for much higher loads than the algorithms for large messages. The deterministic algorithm is not greedy, contrarily to the previous ones, since it uses a swap mechanism which can move already scheduled messages. 
 In Sec.~\ref{sec:reduction} of the Appendix, we justify why we can reduce to message of size one for the price of doubling the load or a small additional latency. Also in the Appendix, the performance of all presented algorithms (and some more) are shown on random inputs, both on large messages in Sec.~\ref{sec:perf_large} and small messages in Sec.~\ref{sec:perf_small}. It turns out that our algorithms work even better on random instances than what we have established for their worst case.
...
The theorem we give is obtained for $k=8$, but taking arbitrary large $k$ and using more refined bounds on $Fmo_i(A)$ is not enough to an algorithm working for a load of $41/100$ (and it only works from larger $n$).
...
Lemma~\ref{lemma:uple_find} ensures there are enough compact $k$-tuples, if there are enough messages,
that is $n - \sum_{j \leq i \leq 8} n_j$ is larger than $i + i(i-1)(2i-1)/6$. 
...
produce enough compact $8$-tuples in the first phase. It turns out, is not a problem for two reasons. First, Lemma~\ref{lemma:uple_find} can be improved by using a better polynomial time algorithm to find compact tuples, by better taking into account the repetitions of values. We do not present it here because it is more complex to analyze. Second, on random instances, the probability that $k$ messages do not form a compact $k$-tuples is low, and we can just build the tuples greedily. Therefore, for most instances, forming compact $k$-uples is not a problem and in practice the algorithm works even for small $n$.
...
We now precisely describe the \textbf{Swap and Move} algorithm:  messages are scheduled while possible by First Fit and then the Swap operation is applied while it increases the potential. When the potential is maximal, the algorithm tries to schedule a new message by moving one or two scheduled messages to other available offsets. If it fails to find such messages the algorithm stops, otherwise it continues repeating the same steps. We now give an analysis of the algorithm, showing that it always works for a load striclty larger than $1/2$.
...
To prove the algorithm is in polynomial time, it is enough to remark that all Swap operations 
strictly increase the potential. On the other hand, when one or two messages are moved, the potential can decrease but
a message is added to the partial assignement. The potential is bounded by $O(n^2)$ and the move operation all together can only remove $O(n^2)$ to the potential, hence there are at most $O(n^2)$ Swap operations during the algorithm. A Swap operation can be performed in time $O(n^2)$, since for a given message, all free offsets must be tested and the potential must be computed in time $O(n)$, which proves that Swap and Move is in $O(n^4)$.  
\end{proof}
