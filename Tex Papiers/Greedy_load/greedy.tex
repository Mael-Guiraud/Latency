...
For message of size $1$, we prove that it is always possible to schedule them, when the load is less than $58\%$ \todo{tu m'a dis 63 au telephone non ?} using a polynomial time algorithm. Moreover, using a simple random greedy algorithm, we show that almost all
instances of a given load admit a solution, explaining why most greedy algorithms work so well in practice.  
\end{abstract}

\todo{est-ce qu'on parle des protocols qui imposent la latence, ou au moins mettre les refs ? Dans l'abstract ou dans l'intro ?}
...
\todo{il ne faut pas mettres de virgules apres "that is"?}
...
\end{center}
\caption{A representation of a compact pair of message in the first and the second period}
\end{figure}
...
\end{lemma}
\todo{en fait le dessin montre pour les kuples, faut il en faire un autre ? car du coup tu dis que la figure justifie le lemme mais pour moi ce n'est pas direct,surtout a cause du trait que j'ai mis en pointillé.}
...
\caption{Representation of the meta offsets forbiden by a 3-uple when scheduling a compact pair} 
\end{figure}
...
\begin{theorem}
Compact $10$-uples always solves \pma positively on instances of load less than $4/10$, for instance with $n$ large enough.
\end{theorem}
\begin{proof}
