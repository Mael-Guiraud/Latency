...
The complexity of \pma is not yet known. However, we have proven that when parametrized by
...
produce enough compact $10$-uples. Theoritically, we need more than $333$ messages. This bound can be improved
by a better algorithm to select the compact tuples, but on random instances, the probability that 
$k$ messages do not form a compact $k$-uples is low. This means, that for random instances, forming compact $k$-uples
...
On a regular laptop, all algorithms terminates in less than a second when solving $10000$ instances with $100$ messages except the exact resolution, whose complexity is exponential in the number of routes (but polynomial in the rest of the parameters). Hence, the theoritical best value given by the exact resolution is only available in the experiment with at most $10$ messages. The code used to generate these figures can be found on the author website.
...
a solution when it exists. Moreover, we have found an instance with load $8/10$ with no assignment, which gives an upper bound on the highest load for which there is always a solution to \pma.
...
Let us denote by $\Pr(m,n)$ the probability that Greedy Uniform fails at the $n^{th}$
...
Again, the success rate one random instances is much better than what we obtain in our worst case 
...
for large $\tau$ and load  $0.4$ or for $\tau = 1$ and load $0.61$. Moreover, the performance of the presented algorithms over average instances are shown to be excellent empirically but also theoritically for Greedy Uniform.
Hence, we can use the simple algorithms presented here to schedule C-RAN messages \emph{without buffer nor additional latency}, if we are willing to use only half the bandwith of the shared link. 
...
First Fit or Meta Offset can easily adapted to network topologies with more than a single link
and analysed, but we could also try to adapt Compact Tuples and Swap.
