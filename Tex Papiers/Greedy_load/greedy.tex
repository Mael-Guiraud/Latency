...
Mettre ici notre travail sur un anneau ou sur un seul lien mais avec du délai possible.
Un peu comme du wormhole ou on ferait un routage préalable SDN pour éviter les buffers et passer l délai
à 0.

Généralisation du problème.
Probleme avec des messages periodiques back and forth:
, for instance any sensor network which must periodically
transmit informations to some gateway. 
Sonar
Wormhole mais dire la différence.


En général, n'importe quel problème avec periode est fixée: probleme de scheduling
comme two flow shop. Des applications similaires ou il n'y a pas de réponse mais un
réseau plus compliqué de profondeur $2$ ou plus. 
...
Autres travaux avec une approche similaire
Periodic mais ici on ne peut pas toucher à la periode
Approche des allemands, autrichien par prog lineaire
...
with the additional constraint of periodicity. To our knowledge, all periodic scheduling problems are quite different from \pma.
