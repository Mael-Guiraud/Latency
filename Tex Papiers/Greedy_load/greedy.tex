...
We made some simulations in order to evaluate the performance of the algorithms presented above.
We generated for each point of the following figure $1,000$ instances of \pma. A random instance is generated as follow: Given a fixed number of messages, draw uniformly a delay for each message between $0$ and the period $P$.
Then we try to solve \pma with the following algorithms:
\begin{itemize}
	\item First fit
	\item Meta offset
	\item Compact Pair
	\item Random Offset : randomly choose the offset between the possible offsets, for each message.
	\item An exhaustive search, which has an FPT complexity on the number of messages. We then computed the exhaustive search only when the number fo message was small.  
\end{itemize}

We set the size of the period $P$ and the size of the message $\tau$, and we increase the number of routes to increase the load.
