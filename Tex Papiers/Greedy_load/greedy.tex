...
\begin{center}
\includegraphics[width=0.8\textwidth]{{instance}
\end{center}
...
A compact tuples is a sequence of messages $i_1,\dots,i_k$ such that $r_{i_1},\dots,r_{i_k}$
are increasing and for which meta offsets can be chosen so that: there are no collision,
...
The algorithms \emph{Compact k-uples} works by scheduling compact $k$-uples
...
\end{theorem}
\begin{proof}
We use two facts to prove this theorem.
Given $ k^3$ messages, one can build from them a compact $k$-uple. 
