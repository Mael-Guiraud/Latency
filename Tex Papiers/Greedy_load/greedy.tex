...
We now describe the algorithm to solve \pma with load $1/2 + \epsilon$. The first two step schedule messages so that the $Pot(A)$ is large enough. Then, it produces a set of messages $R$ in the following way: select the message of largest potential
not in $R$, add it to $R$ and remove it if necessary from the partial assignment. Then schedule or swap any unscheduled message until the potential is as large as before adding the previous message to $R$. When $R$ is of size $\epsilon P$ stops
this process. Finally, using any greedy algorithm, the messages in $R$ are scheduled.
