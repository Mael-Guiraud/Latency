...
Hence, because the three sets partition $[P]$, $|P^2_u| = 2(n-1) - P = 2(\epsilon P - 1)$ and $|P^1_u| = |P_f| = (1/2 - \epsilon)P - 1$.
...
$pot_{msg}(i) = 2(n-1) - P = 2(\epsilon P -1)$. For the same reason, a message $i$ with $(A(i) + d_i \mod P \in P_f)$ has the same potential.
...
It is possible to schedule both messages when one is of potential at least $2\epsilon P -1$ and the other at least $2\epsilon P + 1$. 
Since a message is of potential at most $n-1$, both messages can always be rescheduled when the sum of the two potentials is at least $2(\epsilon P -1) + n$.
Hence we may assume that their sum is less than $2(\epsilon P -1) + n$. 
...
By summing both equalities, using $|P^1_u| = |P_f| = (1/2 - \epsilon)P - 1$ and 
$pot_{msg}(i) \leq 2(\epsilon P -1)$ for $A(i) \in P^1_u$ and  $A(i) + d(i) \in P_f$, we obtain  


$$2Pot(A) \leq 2((1/2 - \epsilon)P - 1)2(\epsilon P -1) + \sum_{\substack{i \in [n] \\ A(i) \in P^2_u} } pot_{msg}(i) + \sum_{\substack{i \in [n] \\ A(i) + d_i \in P^2_u} } pot_{msg}(i).$$


We have proved that $pot_{msg}(i) + pot_{msg}(j) < 2(\epsilon P -1) + n$ when $A(i) \in P^2_u$ and $A(i) = (A(j) + d_j \mod P)$ and  $|P^2_u| = 2(\epsilon P - 1)$.
Hence, $$\sum_{\substack{i \in [n] \\ A(i) \in P^2_u} } pot_{msg}(i) + \sum_{\substack{i \in [n] \\ A(i) + d_i \in P^2_u} } pot_{msg}(i) < (2(\epsilon P - 1) + n)(2(\epsilon P - 1)).$$
Using this inequality and simplifying the expression, we obtain 


$$  Pot(A) < (\epsilon P -1) (P + n - 2).$$
...
If we consider all $n'$ used positions $p$ and all times $p+d$ in the second period, 
