...
Otherwise, the message has no available offset, which means its potential is equal to $2(\epsilon P -1)$.
...
For instance, with $n = 8$ and $P=12$ (thus $\lambda = 2/3$), the probability of success of \greedyuniform is larger than $0,92$.


\section{Generalizations}\label{sec:gen}

In this section, we prove that we can remove several hypotheses ...

\subsection{Period Value}


In Lemma~\ref{lemma:multiple}, we show that we can assume $P = m\tau$. This hypothesis was done to make the analysis of algorithms based on meta-offsets simpler and tighter. The load increases from $\lambda = n \tau / P$ to at most $\lambda (1 + 1/m)$: the difference is less than $1/m < 1/n$, thus very small for most instances. If we drop this hypothesis, the theorems of Sec.~\ref{sec:large}
