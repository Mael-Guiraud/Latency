...
The method of this section is described in~\cite{dominique2018deterministic} and it achieves the same bound on the load using a different method. It is recalled here as an introduction to the algorithms of the next section.
...
The problem $\pma$ (under the name \textsc{PAZL}) is proven to be $\NP$-hard in~\cite{dominique2018deterministic} and the instances used for the reduction have a coherent routing. 
...
In this article, we have proved that there is always a solution to \pma and that it can be found in polynomial time for large $\tau$ and load  $0.4$ or for $\tau = 1$ and load $0.61$. Moreover, the presented algorithms find valid assignments for random instances for a much higher load as we have shown empirically but also theoretically for \compactpair.
...
%Mélanger les tau: que se passe t-il ?
 ...
 Allowing messages of different sizes would help model applications where several types of periodic messages use the same network, further work to adapt the algorithms presented in this paper is required to handle this potential generalization.
