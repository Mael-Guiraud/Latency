...
Let us fix the load $\lambda = n/P$. We can bound $\Pr(P,n)$ using Stirling approximation by an expression of $\lambda$ and $P$.
 The computation yields a constant $C_{\lambda}$ such that $\Pr(P,n) \leq C_{\lambda} \left(\frac{\lambda^{2\lambda}}{(2\lambda -1)^{2\lambda -1}}\right)^P$.
...
if $i > j$, hence we bound the probability that \greedyuniform fails by $C_{\lambda} \lambda P/2 f(\lambda)^P$. For any fixed $\lambda$, the previous expression goes to zero, exponentially quickly, when $P$ goes to infinity. It explains why \greedyuniform is good in practice for large $P$, even when the load is large. 
...
In this section, we give methods to reduce the size of messages up to $\tau = 1$, at the cost of increasing the or allowing buffering in the network. This further justifies the interest of Sec.~\ref{sec:small}, where specific algorithms for $\tau = 1$ are given.
...
\end{figure}
 ...
 We can apply the algorithms presented in this article to find an assignment for $N'$, which can then be turned back into an assignment of $N$. Let us define the load of a general network as $n\tau /P$, then the load of $N'$ is the same as the load of $N$. We have proved that \pma for general networks with coherent routing can be solved positively when the load is less than $2/5$ or less than $1/2 + (\sqrt{5}/2 -1)$ and $\tau = 1$. 

However, there is a more relevant way to define the load for general networks. Let $n_c$ be the number of messages going through the contention point $c$, then the load at $c$ is $n_c\tau/P$. The \textbf{local load} of the network is then defined as the maximum of the load of the contention points. It is always less than the load defined as $n\tau /P$ and it can be significantly so. For the same reason as for a single contention point, in a network with coherent routing, any greedy algorithm finds a solution when the local load is less than $1/4$. However, the algorithms of this article do not seem to work for a local load larger than $1/4$, since the bounds on $\Fo$ or $\Fmo$ do not hold anymore. 
