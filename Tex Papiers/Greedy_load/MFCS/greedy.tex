...
This shared link is the only contention point for the messages going to the data-center and it is also the contention point for the messages sent back by the data-center to the antenna.
This model with a single contention point may also capture other situation such as processors sharing a bus or sensors doing periodic radio transmissions on the same frequency.
...
Given a period, a message size and a delay between the two contention points for each message, set a departure time in the period for each message, so that they go through both contention points without collision. It is similar to the two flow shop scheduling problem~\cite{yu2004minimizing} with periodicity. The periodicity adds more constraints, since the sending pattern for a single period must be repeated without creating collision at contention points. In flow shop problems, the aim is usually to minimize the makespan, or schedule length, but in our periodic variant this quantity is infinite. Hence, we choose to look for any periodic schedule without buffering between the two contention points, to minimize the trip time of each message. 
 ...
 To our knowledge, most periodic scheduling problems studied in the literature are quite different from the one we present. In some works~\cite{korst1991periodic,hanen1993cyclic}, the aim is to minimize the number of processors on which the periodic tasks are scheduled, while our problem corresponds to two processors and a constraint similar to makespan minimization. In cyclic scheduling~\cite{levner2010complexity}, the aim is to minimize the period of a scheduling to maximize the throughput, while our period is fixed. The train timetabling problem~\cite{lusby2011railway} and in particular the periodic event scheduling problem~\cite{serafini1989mathematical} are generalizations of our problem, since they take the period as input and can express the fact that two trains (like two messages) should not cross. However, they are much more general: the trains can vary in size, speed, the network can be more complex than consecutive single tracks and there are precedence constraints. Hence, the numerous variants of train scheduling problems are very hard to solve (and always $\NP$-hard). Therefore, some delay is allowed in different parts of the network to make the problems solvable and most of the research done~\cite{lusby2011railway} is devising practical algorithms using branch and bound, mixed integer programming, genetic algorithms etc.
 
 Complex scheduling problems for time sensitive networks have been practically solved, using mixed integer programming~\cite{nayak2017incremental,steiner2018traffic} or an SMT solver~\cite{dos2019tsnsched}, but without theoretical guarantees on the quality of the produced solutions. Typical applications cited in these works (out of C-RAN) are sensor networks communicating periodically inside cars or planes, or logistic problems in production lines. We think our approach can be brought to these settings.

The model we study was recently introduced in~\cite{dominique2018deterministic} to find sending schedules for C-RAN messages, with \emph{buffering allowed}. This problem has also been studied for a cycle topology instead of a shared link~\cite{Guir1905:Deterministic}. In these works, the main results are heuristics, using classical scheduling algorithms as subroutines, and FPT algorithms which find a sending scheme with \emph{minimal latency}. 
The problem of finding sending schemes with \emph{no additional latency}, which is the subject of this article under the name \pma, is also briefly introduced and studied in~\cite{dominique2018deterministic}: A simple greedy algorithm for \pma, which is guaranteed to work when the load of the link is less than $1/3$, is given and we recall it in Theorem~\ref{th:metaoffset}. In this article, we propose many new algorithms which solve \pma on instances of higer loads. In particular, we provide a very efficient algorithm for the special case of messages of size one and a proof that greedy algorithms work well on average instances.
%plus insister sur les résultats et avantage d'éviter la contention par rapport aux autres travaux c'est de ne pas avoir à ajouter des buffers pou gérer ça et simplifier le problème. Dans l'annexe on montre une manière 
%d'utiliser le problème sans délai pour aider à résoudre le problème avec délai
...
Messages using the link in two different directions do not interact, since the link we model is full-duplex. In the C-RAN context we consider, \emph{all messages are of the same kind}, hence they are all of the same size denoted by $\tau$. This \textbf{size} represents the time needed to send a message through a contention point of the network, here the beginning of the link shared by all antennas. Allowing messages of different sizes would help model other applications than C-RAN. The algorithms presented in this article are not directly applicable to this more general case, but we think that several ideas presented here could be adapted.
%améliorer la justification (peux modéliser quand meme type de message, mais further work to understand the case of several sizes)

 We denote by $n$ the number of messages, which are numbered from $0$ to $n-1$. A message $i$ is characterized by its \textbf{delay} $d_i$: when message number $i$ arrives at the beginning of the shared link (first contention point) at time $t$, it returns to the other end of the link on its way back (second contention point) at time $t + d_i$. The model and problem can easily be generalized to any topology, that is any directed acyclic multigraph with any number of contention points, see~\cite{dominique2018deterministic}. We choose here to focus on the simplest, but realistic, non-trivial network, for which we can still obtain some theoretical results. %citer l'annexe ou je vais mettre un algo simple pour le cas coherent routing
...
In other words, we look for a way to send the messages without using buffering in the BBU and 
...
it is relevant to have $\tau > 1$ when the transmission time of a single message is large with regard to its delay, which is the case in C-RAN networks.
%dire qu'on choisit la granularité du temps et qu'on pourrait fixer tau = 1, mais que typiquement les liens sont de taille environ tau et que ça ne serait pas pertinent
%de les modéliser aussi imprécisément
...
Consider some partial assignment $A$, in the first period, the message $i$ uses the times in $[A(i), A(i) + \tau -1]$. If a message $j$ is scheduled by $A$, with $A(j) < A(i)$, then the last time it uses in the first period is $A(j)+\tau-1$ and it should be less than $A(i)$, which implies that $A(j) \leq A(i) - \tau$. Symmetrically, if $A(j) > A(i)$, to avoid collision between messages $j$ and $i$, we have $A(j) \geq A(i) + \tau$. Hence, message $i$ forbids the interval $[A(i) - \tau +1, A(i) + \tau -1]$ as offsets for messages still not scheduled, because of its use of time in the first period. The same reasoning shows that $2\tau -1$ offsets are also forbidden because of the times used in the second period. Hence, if $|S|$ messages are already scheduled, then at most $|S|(4\tau -2)$ offsets are forbidden for an unscheduled message. The real number of forbidden offsets may be smaller, since the same offset can be forbidden both because of a message on the first and on the second period.
 ...
 Let $A$ be a partial assignment defined over $S$ and $i\notin S$, we let $Fo(A)$ be the maximum over $d_i \in [P]$ of $|\left\{ o \in [P] \mid A[i \rightarrow o] \text{ has a collision}\right\}|$. 
 The notation $Fo(A)$ stands for \emph{forbidden offset by $A$} and depends only on $A$ and not on the unscheduled messages. We have computed in the previous paragraph that $Fo(A)$ is less than $(4 \tau -2)|S|$. 
...
The method of this section is described in~\cite{dominique2018deterministic} and it achieves the same bound on the load using a different method. It is recalled here to help understand several algorithms in this article.
...
with $i$ an integer from $0$ to $\lceil P / \tau \rceil$. We call \metaoffset the greedy algorithm which works as \firstfit, but consider only meta-offsets when scheduling messages. 

To study \metaoffset, we introduce a variant of $Fo(A)$ restricted to meta-offsets.
 Let $A$ be a partial assignment defined over $S$ and $i\notin S$, we let $Fo(A)$ be the maximum over $d_i= j\tau$ with $j \in [P/\tau]$ of $|\left\{ o \in [P] \mid A[i \rightarrow o] \text{ has a collision}\right\}|$.
...
We present in this section a family of greedy algorithms which solve \pma positively for larger loads. We try to combine the good properties of the two previous algorithms: the compactness of the assignments produced by \firstfit and the absence of collision in the first period of \metaoffset. The idea is to schedule several messages at once, using meta-offsets, to maximize the compactness of the obtained solution. We first describe the algorithm which schedules pairs of messages and then explain quickly how to extend it to any tuples of messages.


From now on, we use Lemma~\ref{lemma:multiple} to assume $P = m\tau$ in the rest of this section. This hypothesis makes the analysis of algorithms based on meta-offsets simpler and tighter. The load increases from $\lambda = n \tau / P$ to at most $\lambda (1 + 1/m)$: the difference is less than $1/m < 1/n$, thus very small for most instances. If we drop this hypothesis, the theorems of this section
...
and we say that $d'_i$ is the \emph{meta-delay}.
...
\end{center}
\caption{A compact pair scheduled using meta-offsets, with $d'_0 = 2$ and $d'_1 = 0$}
...
\caption{Meta offsets forbidden by a scheduled compact pair (in blue) when scheduling another compact pair (in red) with larger $r_i$'s} 
...
\end{theorem}
\begin{proof}
We use the following fact, which generalizes Lemma~\ref{lemma:pair_forbid}: A $k$-tuples forbids $k+j+1$ offsets in the second period when scheduling a $j$-tuple. %If the remainder of the messages in the $j$-tuples are larger than the remainder in the $k$-tuples, it forbids $k+j$ messages only.
It enables us to compute a lower bound on the number of scheduled $i$-tuples for $i$ equal $k$ down to $1$ by bounding $Fmo_i(A)$, the number of forbidden meta-offsets when placing compact $i$-tuple in the algorithm.
...
The algorithm is not greedy, since we allow to change the offsets of two scheduled message to make room for an unscheduled one. It cannot work online, since it requires to know all delays of the messages in advance. 
...
\begin{definition}
The potential of a position $p$, for a partial assignment $A$, is the number of messages $j$, such that 
$A(j \leftarrow p)$ has a collision. 
\end{definition}
...
select a free position $p$ in the first period, remove the message which uses the position $p+d$ in the second period from $A$ and extend $A$ by $i$ with offset $o$. We denote by $Swap(i,p,A)$ the partial assignment obtained by this operation.
...
Let us precisely describe the algorithm \swapandmove:  messages are scheduled while possible by \firstfit and then the Swap operation is applied while it increases the potential. When the potential is maximal, \swapandmove schedules a new message by moving at most two scheduled messages to other available offsets. If it fails to do so, \swapandmove stops, otherwise the whole procedure is repeated. We analyze \swapandmove in the following theorem.
...
$f(\lambda) < 1$ when $\lambda < 1$. By an union bound, the probability that \greedyuniform fails is bounded by $C \lambda m f(\lambda)^m$. For any fixed $\lambda$, the previous expression rapidly goes to zero when $m$ goes to infinity. It explains why \greedyuniform is good in practice for large $m$, even when the load is large. 
...
We also present the results of all algorithms of the paper run over synthetic data.


\section{The compact $k$-tuple algorithm}


%mettre ici l'explication de l'algo
...
\section{Coherent Routing}

In this section, we explain how our algorithms to solve the problem \pma may be used on more complex networks.
%donner la définition du coherent routing
%donner un exemple
%dire que le problème est NP dur pour de vrai -> oui en citant le papier arxiv, les exemples sont coherent routing, le mettre dans l'intro
%dire qu'on peut adapter tous les algos greedys-> la seule différence, c'est qu'on a une paire first period/second period différent pour chaque message 
%par contre pour l'algo swap, il faudrait de nouvelles idées
