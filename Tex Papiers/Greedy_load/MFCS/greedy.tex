...
When $\tau = 1$ and the load is less than $1/2$, \emph{any greedy algorithm} solves \pma positively since $\Fo(A) \leq (4\tau -2)|S| = 2|S|$ where $S$ is the number of scheduled messages. We give, in this section, a method which always finds an assignment for a load larger than $1/2$. We explain in Appendix~\ref{sec:reduction} how and at what cost a general instance can be reduced to an instance with $\tau = 1$.
