...
share either nothing or a single path (i.e. a sequence of contiguous links) in the network. See Fig.~\cite{fig:coherent_routing} for a network with coherent routing.
...
\end{minipage}
 ...
 \caption{Left, a C-RAN network network without coherent routing, and right, a C-RAN network with coherent routing. The multiple colored edges represent a single physical link.}

\label{fig:coherent_routing}
\end{center}
\end{figure}
...
have a coherent routing. However, we now explain why the algorithms proposed in this article for the single shared link case, can be easily transfered to
coherent routing case. To do that we prove that we can transform a network with coherent routing into a network with a single shared link such that 
...
routed through $c_1$ and $c_2$ is not a contention point anymore, see Fig~\cite{fig:transformation}. Let $l$ be the length of the link
...
\end{minipage}
 \caption{Transfromation of a network $N$ with coherent routing property into a network $N'$ with only one shared link.}
...
two contention points only to each message. However, the algorithm of this article does not seem to work well when bounding the local load. 
