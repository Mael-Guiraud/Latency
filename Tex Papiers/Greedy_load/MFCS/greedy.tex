...
we can choose the time granularity, and we could set it so $\tau = 1$ for simplicity. However, the size of a link and thus delay of the messages
is often of the same magnitude as $\tau$, therefore setting $\tau = 1$ is a too coarse granularity to correctly model the network.
...
of algorithm \texttt{Compact k-tuples} and its analysis.
...
Recall that the a delay $d_i$ is equal to  $d'_i*\tau + r_i$, we call $d'_i$ the \textbf{meta-delay}. The algorithm we describe relies only
