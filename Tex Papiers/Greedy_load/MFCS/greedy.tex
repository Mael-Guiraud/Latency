...
To study \metaoffset, we introduce a variant of $\Fo(A)$ restricted to meta-offsets.
 Let $A$ be a partial assignment defined over $S$ and $i\notin S$, we let $\Fmo(A)$ be the maximum over $i \in [n]$ of $|\left\{ j \in [P/\tau] \mid A[i \rightarrow j\tau] \text{ has a collision}\right\}|$.
...
that we need to increase by local modification when it is not possible to schedule a new message. 
...
\begin{definition}
The potential of a position $p$, for a partial assignment $A$, is the number of messages $i$, such that 
...
\begin{proof}
We determine for which value of the load \swapandmove always works. We let $n = (1/2 + \epsilon)P$ be the number of messages, the load is $1/2 + \epsilon$. We need only to study the case when $n-1$ messages are scheduled by $A$ and \swapandmove tries to schedule the last one, since previous steps are similar but easier. 
