  ...
  \end{subsubsection}
     ...
     we use a deadline function $d$, which maps to each route $r$ an integer such that $PT(r)$ must be less than $d(r)$.
      ...
      \noindent {\bf Question:} does there exist an assignment $m$ of $(G,{\cal R})$, in which $b(d,u,r) = 0$, for all $d,u,r$ such that for all $r \in {\cal R}$, $PT(r) \leq d(r)$?
        ...
        Thus, we define the \textbf{full transmission time} of a routed network $TR(G,R) = \displaystyle \max_{i \in \{0,\ldots,n\}} rt(St_i) - \displaystyle \min_{j \in \{0,\ldots,n\}} \theta_j(d_0)$. This is the time ellapsed before the sending of the first tic of the first stream, and the reception of the last tic of the last stream.
        \end{subsubsection}
   ...
   \begin{tabularx}{0.8\textwidth}{|c|X|}
    ...
    \hline
    $\theta_r(d)$ & Departure time of the datagram $d$ on route $r$ \\
    \hline
