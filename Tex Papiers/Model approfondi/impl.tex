...
To model this core of the network, we generate some random bipartite graphs. First of all, two parameters set the number of data-centers and switchs. The first set of the graph models the data-centers, and the second level of the trees models the switches. Then each arc is drawn uniformly with a given probability.
Here is an example of a graph generated with 2  nodes of level 1 and 3  node of level 2.
...
\end{center}
 ...
 We first to generate some bipartite graphs which seems to be a good model for the internet topology as mentioned in \cite{tarissan_towards_2013}. This papers also gives us some background to generate some good random bipartite graphs. 
 
 In a first time, we chose a simple model to generate our random bipartite graphs. Thus, two parameters set the number of data-centers and switchs. The first set of the graph models the data-centers, and the second set models the switches. We then randomly draw a link between each couple of node $(u,v)$, where $u$ is a node of one set and $v$ a node of the other set, with a given probability (the same for each couple). If the generated graph is not connexe, we generate another bipartite graph.
