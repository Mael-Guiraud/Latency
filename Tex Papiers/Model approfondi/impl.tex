...
$V'$ is composed of two sets: The first set, of size $a$, models the data-centers, and the second set, of size $b$, models the switches. We fix $a$ and $b$ and we uniformly draw the edges of $V'$ with a given probability. Note that $|V'|= a \times b$. If the generated graph  $G'$ is not connected, we generate another bipartite graph.
Figure~\ref{fig:random23} is an example of a random bipartite graph generated, with $a = 2$, $b=3$ and $|V'|=4$.
...
In order to create some arcs with conflict, the vertices of $G'$ are extended to become some arcs in $G$. The egdes in 
 
 A {\bf flow} is a group of antennas, which communicate with a same datacenter. There exaclty $|V'|+a$ flows; One for each arc of $V'$, and one flow directly connected to each datacenter. For each flow, we randomly draw between $1$ and $k$ antennas.
...
\includegraphics[width=0.3\textwidth]{extendnode}
\end{center}
