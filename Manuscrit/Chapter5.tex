...
We first investigate the impact of $M$ alone. To do so, we fixe $N = M$ (infinite memory) and we compute for a same instance the tabu search  for $M = 500$, $M = 1.000$ or $M = 2.000$. After some simulation on few instances ($100$) with $M \in [10;2000]$, it appears that with $N=M$, the numbers of steps to find the best assignment are very close when $M > 50$. This means that tabu search finds the optimal solutions very fast in average. Those simulations have been made with $8$ and $20$ routes and the observation is the same.
