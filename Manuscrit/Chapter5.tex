...
In this chapter, we deal with more general routed networks that a star routed networks. The algorithms proposed here covers the following cases:
\begin{itemize}
	\item 
Each routed network of contention depth two in which the routes between the RRH and BBU use the same links in both ways. This property induce that every routes of a subset of routes of a given contention point of contention depth one go to the same contention point of contention depth two. Thus, the routed network can be represented by several routed networks.
Contention depth two + symmetry of the paths from RRH to BBU and BBU to RRH can be reduced to several star routed network
J'ai vérifié, c'est vrai, il suffit de dire qu'à cause de la symétrie chaque premier sommet de contention est associé
à un deuxième, de manère unique. 
\item 

Represent a network of depth three which models a network with symmetries, two level of contention point in the fronthaul network but all BBU are at the same distance in each datacenter -> only one contention point when returning to the RRH.
\end{itemize}
...
Figure~\ref{fig:descente07} shows the difference between initalizing Hill Climbing with \hybridgreedynormalized, or with one or several random compact representation. Those results are computed from $1000$ random instances, with load of $0.8$.
\begin{figure}[h] 
	\centering
...
Initializing Hill Climbing with $100$ random compact representations seems to give better results. However, 
choosing $100$ random compact representations can still fail to produce one valid assignment. We investigate this issue
in experiments presented in Figures~\ref{tab:descenteload} and \ref{tab:descentenbroutes}. We represent the probability of drawing at least one compact representation that gives a valid assignment, when drawing $1$, $10$ or $100$ random compact representations. In Figure~\ref{tab:descenteload}, we fix the number of routes to $8$, and we change the load from $0.8$ to $1$. In Figure~\ref{tab:descentenbroutes}, we fix the load to $0.8$ and the number of routes goes from $8$ to $12$ in the routed network. Each value is computed from $1000$ random instances.
...
\end{center}
 ...
 The more steps Hill Climbing does, the more the initial solution is improved. When Hill Climbing starts from the result of \hybridgreedynormalized, it does not improve much the solution whereas when drawing a large number of random compact representation, the probability of drawing one that can be improved a lot is better.
 
 The idea of drawing a large number of random compact representations to initialize Hill Climbing is a naive version of Simulated Annealing. The next two presented meta-heuristics are designed to explore the compact representations, even if a local optimum is reached. Tabu Search remembers the explored solutions, in order to avoid them, and Simulated Annealing browses the compact representations with a stochastic approach.
...
The parameters $N$ and $M$ must be chosen appropriately to minimize the computation time of the Tabu Search, while maximising the quality of solutions found. 

We first investigate on Figure~\ref{tab:tabumn} the impact of $N$ alone. To do so, we fixe $N = M$ (infinite memory) and we compute  Tabu Search on an instance, with $N = 100$,$500$,$1000$ and $2000$ (and not more because of large computation times). Those simulations have been made with $8$ routes and a load $0.8$, and results are similar for $20$ routes.
 It appears that with $N=M$, the more steps Tabu Search computes, the better is the solution. For most instance, Tabu Search finds the optimal solutions in the first steps, however for some instances the solution is improved after a large number of steps, which impacts strongly the average margin. At each step, Tabu Search explores the entire neghborhood of the current compact assignment, the computation time is linear with $N$. Nevertheless, for $N > 1000$, the computation time may not be worth the improvement of the solution.
