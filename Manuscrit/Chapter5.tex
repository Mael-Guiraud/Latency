...
To generate random routed networks, several parameters must be chosen: The load of the network, the number of routes, the distribution of the length of the arcs, and the topology of the routed network. We would like to understand the impact of those parameters, in terms of computation time and quality, on the algorithms studied. In order to reduce the number of experiences presented here, we fix the topology of the routed network as shown in Figure~\ref{fig:randomnetworks}. The difference between the performances of the presented algorithms are not significantly impacted if we change the values of the number of contention points. 
 ...
 The impact of the load on the quality of the results has been investigated: When the load is increased, the performances of the local search algorithm does not changes significantly. Hence, we choose to fix the load to $0.8$, which is an already high load. This means that $P = \frac{\tau \times n}{0.8}$, whith $n$ the maximal number of routes over a contention vertex. The size of the C-RAN traffic depends of the service requirement~\cite{mobile2011c}. Here, we fix $\tau = 2500$ tics.
...
\hline
 ...
 \greedydeadline fails less than \greedynormalized on highly loaded networks, while \greedynormalized seems more robust on loads between $0.8$ and $0.9$. We now want to compare the performance of those algorithms. Figure~\ref{fig:90load} shows the margin needed by the assignments given by the algorithms, when there is one. As expected, the margin increases when the load increases and \greedypacked, that trades margin for success rate, performs worst than \greedydeadline and \greedynormalized when they are able to find an assignment.
...
We propose improved version of \greedydeadline and \greedynormalized that always find a solution. For each contention point, we first try \greedydeadline (or \greedynormalized), and if the algorithm fails, we apply \greedypacked. Let us call \hybridgreedydeadline and \hybridgreedynormalized those two algorithms. Figure~\ref{fig:greedysuccess} shows the performances of \hybridgreedydeadline, \hybridgreedynormalized, and \greedypacked on $1000$ routed networks. Here, the load is of $0.9$ to clearly show the difference between the algorithms.

 	\begin{minipage}[c]{.45\linewidth}
	\centering
	\includegraphics[scale=0.23]{Chapitre5/90load}
\captionof{figure}{Performance of \greedydeadline, \greedynormalized and \greedypacked over $1000$ instances on graphs with load of $0.9$.}
\label{fig:90load}
\end{minipage}
\begin{minipage}[c]{.45\linewidth}
	\centering
...
Figure~\ref{fig:descente07} shows the difference between initalizing the hill climbing with \hybridgreedynormalized, or with one or several random compact representation. Those results are computed from $1000$ random instances, with load of $0.8$.
\begin{figure}[h] 
	\centering
...
Initializing the hill climbing with $100$ random compact representation seems to give better results. However, 
choosing $100$ random compact representations can still fail to produce one valid assignment. We investigate this issue
in experiments presented in Figures~\ref{tab:descenteload} and \ref{tab:descentenbroutes}: we represent the probability of drawing at least one compact representation that gives a valid assignment, when drawing $1$, $10$ or $100$ random compact representations. In Figure~\ref{tab:descenteload}, we fix $8$ routes on the graph, and we change the load from $0.8$ to $2$. In Figure~\ref{tab:descentenbroutes}, we fix the load to $0.8$ and the number of routes goes from $8$ to $12$ in the routed network. Each value is computed from $1000$ random instances.
...
We first investigate on Figure~\ref{fig:tabumn} the impact of $N$ alone. To do so, we fixe $N = M$ (infinite memory) and we compute for a same instance the tabu search. Because of the large computation time, we made some simulation on $100$ instances with $N = 100$,$500$,$1000$ and $2000$. Those simulations have been made with $8$ routes and a load $0.8$, and the observation is the same for $20$ routes.
 It appears that with $N=M$, the more step tabu search computes, the better is the solution. It appears that tabu search finds the optimal solutions in the first steps, in average, but some instances improoves the solution after a large number of steps, which impact strongly the average. At each step, tabu search explores the entire neghborhood of the current compact assignment, the computation time is linear with $N$. Nevertheless, for $N > 1000$, the additional computation time is not worth the improvment of the solution.
...
\hline
    N & $100$&$500$& $1000$& $2000$\\
    \hline
    Average number of step & $4.52$ & $11.41$& $23.77$& $36.13$\\

    \hline
     Average margin & $2885$ & $2790$& $2756$& $2753$\\

    \hline
    Computation (s) & $2.2$ & $11.9$& $29.9$& $87.5$\\
...
Tabu search could either be improved or worsen by the value of $M$. Indeed, a large value for $M$ could restrict the tabu search to some component of the transposition graph while a small value of $M$ may allow loops.

We fix $N = 1000$ and we compute tabu search with $M$ equals $10$,$100$ or $1000$.
Figure~\ref{fig:tabudistrib} shows cumulative distribution of the margin needed by tabu search with different values for $M$. 
\begin{figure}[h]
	\centering
...
\hline
 ...
 \captionof{figure}{Comparison of two intial temperatures, considering the quality of the initial configuration}
     \label{tab:poorgood}
 \end{center}
 We assume that the initial solution given by hill climbing can be considered as ``good''. Indeed, increasing the initial temperature does not affect the quality of the solution, but increase the computation time.
 
 In simulated annealing, the temperature should decrease slowly. At each \textbf{level}, $N$ compact representations are drawn. At the end of a level, the temperature is decreased of $10\%$. The algorithm stops if less than $1\%$ of the compact representations darwn are accepted during two consecutive steps. Hence, drawing too few compact representations per level decreases the temperature too fast and reduces the efficiency of simulated annealing. However, drawing too much compact representations during a level increases the computation time of the algorithm, hence there is tradeoff between time an quality and lenth of a level should be set carefully.
  
Figure~\ref{fig:stepsrecuit} shows the margin needed by simulated annealing with different values for $N$. Those results are computed on $1,000$ random instances, in which the initial temperature is set to $2200$ and the load is $0.8$
...
%Définir l'arbre de recherche exhaustif sur les solutions partielles et expliquer les coupes qu'on peut faire grâce à la borne sup qu'on vient de définir (en gardant la valeur de la meilleur solution et en initialisant cette valeur par un algo de bonne qualité, genre hill clibming ou carrément recuit simulé).
