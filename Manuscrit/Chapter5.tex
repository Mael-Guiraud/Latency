...
We define the bijection recursively: let $v$ be a vertex and $u$ its parent, then if $u$ is mapped to $CA$,
...
\begin{itemize}
  ...
  Hence, if we fix $r_{O_i} \in S$, it will not change $ns(r_{O_1},r_{O_{i},c})$ and any extension of this compact assignment are dominated by the extension of the same compact assignment with $r_{O_i} \notin S$.
...
\caption{ If $ns(r_{O_1},r_{O_{i-1},c}) + \tau = ns(r_{O_1},r_{O_{i},c})$, every extension built by the branch $r_{O_i} \in S$ is dominated by the extension of the compact assignment of the branch $r_{O_i} \notin S$.}
\label{fig:cut1}
\end{figure}
\end{center}
...
\end{center}
 ...
 When the number of routes is lower than $10$, we can find a solution in a reasonable time. For $12$ routes, the time needed to compute a solution is in the range of one hour. We can imagine that increasing the quality of the cuts or being able to generate only minimal compact assignments could allows us to compute up to $14$ routes, $16$ with a perfectly optimized implementation. Beyond this value, Branch and Bound complexity makes the computation time too long to compute a solution.
...
\caption{Performance of Branch and Bound Against Statistical Multiplexing for $8$ routes.}
\label{fig:fptbandb}
\end{figure}
\end{center}
...
This chapter present a variant of the problems in previous chapters. Here, we consider the antennas sends their message at the same date, which correspond to the C-RAN context. By setting all offsets to $0$ and considering only one contention vertex in which it is possible to delay the message, it seems hard to solve \pall, even if we did not investigate this problem yet.
  We thus study the synchronized version of the minimization problem \mintra related to \pall, called \minstra. In both \mintra and \minstra, the objective is to minimize $TR(A)$. Because the offsets do not icrease the value of $TR(A)$ in the first case, we minimize the local value of each route. In \minstra, since all offsets are set to $0$, we want to minimize the global value of the process. 

  We relax the problem and we allow the messages to be buffered in every contention vertex of the network in order to have an higher degree of freedom.
  An assignment is thus a choice a buffering time in every contention vertex for every routes.
  We propose greedy algorithms for \minstra, that are able to find solutions, whatever the load. However, the solutions are far from the optimal solution. We optimize the solution found with classical Hill Climbing, Tabu Search and Simulated Annealing.
  We also show that \minstra is $FPT$ when parametrized by the number of routes and we propose a Branch and Bound algorithm that find the optimal solution for a reasonable number of routes. The performance evaluations let us to conjecture that Simulated Annealing finds good solutions, even for instances with a large number of routes.
...
optimize the source code as much as possible, but it seems not possible to compute a solution with Branch and Bound for more than $16$ routes.
Simulated Annealing can also be optimized by choosing a different neighborhood, or analyzing deeper the setting of the parameters, like temperature cooling schedule or stop criterias.
