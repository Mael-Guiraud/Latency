...
In order to describe the Branch and Bound Algorithm on the next section, we need to be able to compute an upper bound for $TR(A)$.
...
Given an instance $I$, one can compute an upper bound for the optimal solution of \spall.
\end{lemma} 
\begin{proof}First, build an instance $I'$ of \wta for each contention points of the first level as defined above. Then, use \ASPMLS to find an optimal assignment $A'$ that solve $I'$ in time $O(2^nn^3\log(n))$, where $n$ is the number of routes. By taking the maximum between all $TR(A')$, we obtain an upper bound for $I$.
\end{proof}
