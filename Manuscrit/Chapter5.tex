...
\node (k1) at (14,3.75) {\huge{$N^3$}};
\node (t4) at (12,4.5) {$t_3$};
\node (t2) at (12,3) {$t_1$};
\node (s4) at (4,4.5) {$s_3$};
\node (s2) at (4,3) {$s_1$};
\node (c4) at (8,3.75) {$c_3$};
\path (s2) edge [->] node[anchor=north]{$2$+\textcolor{green}{$0$}+2+\textcolor{green}{$0$}}  (c4);
\path (s4) edge [->] node[anchor=north]{$2$+\textcolor{orange}{$0$}+8+\textcolor{orange}{$0$}}  (c4);
\path (c4) edge [->] node[anchor=north]{$1$}  (t4);
\path (c4) edge [->] node[anchor=north]{$3$}  (t2);

\node (k1) at (28,3.75) {\huge{$N^4$}};
\node (t4) at (24,4.5) {$t_4$};
\node (t2) at (24,3) {$t_2$};
\node (s4) at (16,4.5) {$s_4$};
\node (s2) at (16,3) {$s_2$};
\node (c4) at (20,3.75) {$c_4$};
\path (s2) edge [->] node[anchor=north]{$7$+\textcolor{red}{$1$}+1+\textcolor{red}{$0$}}  (c4);
\path (s4) edge [->] node[anchor=north]{$5$+\textcolor{blue}{$2$}+6+\textcolor{blue}{$1$}}  (c4);
...
\path (s2) edge [->] node[anchor=north]{$2$+\textcolor{green}{$0$}}  (c4);
\path (s4) edge [->] node[anchor=north]{$7$+\textcolor{red}{$1$}}  (c4);
\path (c4) edge [->] node[anchor=north]{$1+2$}  (t4);
\path (c4) edge [->] node[anchor=north]{$2+3$}  (t2);

\node (k1) at (28,6.75) {\huge{$N^2$}};
\node (t4) at (24,7.5) {$t_4$};
\node (t2) at (24,6) {$t_3$};
\node (s4) at (16,7.5) {$s_4$};
\node (s2) at (16,6) {$s_3$};
\node (c4) at (20,6.75) {$c_2$};
\path (s2) edge [->] node[anchor=north]{$2$+\textcolor{orange}{$0$}}  (c4);
\path (s4) edge [->] node[anchor=north]{$5$+\textcolor{blue}{$2$}}  (c4);
\path (c4) edge [->] node[anchor=north]{$6+0$}  (t4);
\path (c4) edge [->] node[anchor=north]{$8+1$}  (t2);


\end{tikzpicture}
}
 \caption{Problem \spall over every single contention point $c_j$ with $\tau = 5$. }

\label{fig:relaxednetwork}
\end{center}
\end{figure}
\todo{me dire si la figure est bien, trop lourde?}

Figure~\ref{fig:relaxednetwork} shows a routed network $N$ and an assignment $A$ for which $TR(N) = TR(A) =14$ and on which, $TR(N^1) = 11$, $TR(N^2) = 13$, $TR(N^3) = 11$ and $TR(N^4) = 14$.
...
implement in linear time by computing $Real((O,S))$. On the other hand, we are not yet able to decide whether a compact assignment is minimal in polynomial time. We now propose several easy to compute heuristics to detect when a compact assignment is not minimal.

To guarantee that we only consider canonical compact assignments, we must guarantee that the 
buffering of the first route is zero. It is the same as requiring for a compact assignment $(O,S)$, that $r_{O_1}$, the first route in $O$, is not in $S$.


The first heuristic is to consider the assignment $(O,S)$ of $N(CA)$ and for each route
$r_i \in S$, we consider $Real((O,S))$ where $r_i$ has been removed. Then, the normalized sending time of all the other routes are considered, to check whether $r_i$ could be scheduled 

 to verify whether $(O,S\setminus \{r_i\})$ is a valid assignment. This can be done in linear time for each $r_i \in S$. If there is a valid assignment 
...
\todo{yann: faire une figure qui montre ces trois coupes là (en représentant les paquets sur la période)}
