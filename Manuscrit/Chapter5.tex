...
Let $CA$ be a partial compact assignment defined on ${\cal C}_{i-1}$, then $CA[(O_{c_{i}},S_{c_{i}})]$ is an extension of $CA$ to ${\cal C}_{i}$, defined by $CA[(O_{c_{i}},S_{c_{i}})](c_i) = (O_{c_{i}},S_{c_{i}})$.
...
\path (s1) edge [->] node[anchor=north]{\textcolor{blue}{$3$}}  (t1);
...
\path (k2) edge [<-] node[anchor=north]{$5$}  (t3);
\path (k3) edge [<-] node[anchor=north]{$2$}  (t4);
\path (k4) edge [<-] node[anchor=north]{$1$}  (t4);
...
\path (s1) edge [->] node[anchor=north]{\textcolor{blue}{$3$}+\textcolor{red}{$0$}+$2$}  (t3);
...
single contention point $c_j$, a set of routes $\matcal{R}'$ and a wiegth function $\omega'$ as follows. For each route $r$ of $N$ which goes through $c_j$, there is a route $r' = (s_r,c_j,t_r) \in \matcal{R}'$. We define $\omega'(s_r,c_j) = \lambda(r,c_j)$ and $\omega'(c_j,t_r) =  \lambda(r) - \lambda(r,c_j)$. The network $N^j$ represents all the routes going through $c_j$ and assuming further there are no constraint before $c_j$ and after $c_j$.   
...
Let $SCP(N)$, for Single Contention Point, be the function which associates to $N$ the integer 
$\max_{j} TR(N^j)$. By Lemma~\ref{lemma:lowerbound}, we have $TR(N) \geq TR(N^j)$, hence $SCP(N) \leq (N^j)$.
We can compute $SCP(N)$ in time $m2^kk^3log(k)$ where $k$ is the contention width of the network and $m$ the number 
of contention nodes. Indeed, for each $j$, solving \spall on $N^j$ is equivalent to solving \wta, which can be done
in time $2^kk^3log(k)$ by \ASPMLS, where $k$ is the number of routes in $N^j$, that is the contention width of $c_j$.



\todo{Yann: un exemple du calcul de SCP sur l'exemple précédent}
...
From now on, simulated annealing computes $1000$ compact assignment at each level. As we observed in section~\ref{sec:recuit}, increasing the number of compact assignment drawn increases the quality of the solution. While drawing $100$ compacact assignment was enough for routed network of the previous experiment, the two following experiment shows considerable better results by drawing $1000$ random compact assignments at each level, without increasing the computation time higher than the tabu search.
...
This number of routes is too large to compute Branch and Bound here. Thus, we draw the lower bound computed by Corollary~\ref{th:lowerbound} of Lemma~\ref{lemma:reduce}.
