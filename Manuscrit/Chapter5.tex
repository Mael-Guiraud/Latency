...
The quality of hill climbing depends on the the intial compact assignment. A first choice is to consider the compact representation $CR(A)$ of the assignment $A$ given by \hybridgreedynormalized. We can also choose a random compact assignment. Since a compact assignment does not always give a valid assignment, we can draw many random compact assignments and return
the best assignment found by hill climbing for these initial solutions.

Figure~\ref{fig:descente07} shows the difference between initalizing the hill climbing with \hybridgreedynormalized, or with one or several random compact representation. Those results are compued from $1000$ random instances, with $80\%$ of load.
\begin{figure}[h] 
	\centering
...
in the experiments presented in Figures~\ref{tab:descenteload} and \ref{tab:descentenbroutes}: we represent the probability of drawing at least one compact representation that gives a valid assignment, when drawing $1$,$10$ or $100$ random compact representations for several loads and number of routes. Each value is computed from $1000$ random instances.
...
We thus propose an hybrid initialization scheme for hill climbing: Between the assigments given by initializing the hill climbing either with HGN, or with $1$,$10$ or $100$ random compact representation, return the one that minimize $TR(A)$. We call this initialization respectively hybrid $1$, hybrid $10$ or hybrid $100$.
Figure~\ref{fig:hybridhill} shows the margin needed by the best solution given by hill climbing, with different initialisations. The results are computed on $1000$ random instances.
\begin{figure}[h]
	\centering
...
\hline
 ...
 The more the hill climbing does steps before ending, the more the initial solution is improved. When hill climbing starts from a \hybridgreedynormalized, it does not improve often the solution whereas with a huge number of random compact representation, the probability of drawing a solution that can be improved a lot is increased.
...
One can change the size $N$ and $M$ in tabu search. 

We first investigate the impact of varying $M$. To do so, we fixe $N = M$ and we compute for a same instance tabu serach on $500$, $1.000$ or $2.000$ steps. After some simulation on few instances ($100$) with $M \in [10;2000]$, it appears that with $N=M$, the numbers of steps to find the best assignment are very close when $M > 50$. This means that tabu search finds the optimal solutions very fast in average. Those simulations have been made with $8$ and $20$ routes and the observation is the same.
...
Tabu search could either be improved or worsen by the value of $N$. Indeed, a too large for $N$ could restrict the tabu search to highly connected parts of the transposition graph while a little value of $N$ could induce loops on the search.
...
\item Deduce $t_0$ from the relation: $e^{-\frac{\bar{\Delta}}{t_0}} = \tau_0$ 
 ...
 Tab.~\ref{tab:poorgood} shows the margin of the solutions produced by simulated annealing when intialized with temperatures computed from the previous routine. The initial solution used by simulated annealing is the solution given by $0$-HHC. The experiment is made on $100$ random instances.
\begin{center}
\begin{tabular}{ |c|c|c|c|c| }
\hline
