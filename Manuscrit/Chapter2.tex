...
\section{Algorithmic related works}
 ...
 We shortly describe the algorithmic problem studied in this thesis. We consider a network in which the routing is fixed.
 Several flows share the network, sending periodically a message from a source to a target. This message represents several packets in practice, but in our model we consider them contiguous. The messages can also be buffered in some nodes of the network in order to let another more critical message use a common ressource. The objective is to compute the buffering time of every message in the network, such that the global latency (i.e. the largest latency of a flow) is minimal.
 We present in this section the works related to ours.

\subsection{Graph theory problems}

\paragraph{Wormhole}
Because we consider contiguous messages, we first had a look at wormhole problems~\cite{ni1993survey,cole1996benefit}, in which the objective is to avoid deadlocks in networks used by long sequences of messages. Wormhole problems are solved by routing algorithms, while we consider the routing given. Furthermore, this problem differs from ours since we want to minimize the time lost in buffers and not just to avoid deadlocks. 

\paragraph{Graph coloring for ressources allocation}
Our first approach consisted to represent the shared ressources of the network by a graph, and to solve it with graph coloring.
...
Our problem of scheduling periodic messages for Cloud-RAN have been investigated in~\cite{nayak2017incremental,steiner2018traffic,silviu2017,naresh2016}. In those papers authors solves the practical problem of scheduling a few number of flows on star shaped networks using linear programming. In the same spirit, an SMT solver is proposed in~\cite{dos2019tsnsched}. Such solutions does not scale neither with the number of route nor networks on which there is many switches. 

\paragraph{Processors scheduling}

The problem we address in this thesis is similar to scheduling on multiple processors. Single pocessors scheduling problems are the following. Given a set of jobs, a release time and a deadline for each jobs, find the scheduling such that the completion time is minimized.
On a star routed network, the problem is very close to a two flow-shop scheduling problem~\cite{yu2004minimizing} in which there is a delay between the two processors for each jobs. However, our problem adds the constraint of periodicity. 

On mulitple processors scheduling, several processors are available in parallel and the aim is to minimize the number of processors on which the periodic tasks are scheduled~\cite{korst1991periodic,hanen1993cyclic,aupy2017periodic}. To our knowledge, there is not periodic scheduling on a networks of processors which can model our network. When we solve our problem on a star routed network, our problem is close from a single processor scheduling minimizing makespan.

\paragraph{Single processors scheduling}
The algorithm we focused on in this thesis is described in~\cite{simons1978fast}. This paper propose a polynomial algorithm to solve the problem of scheduling a set of jobs of the same size minimizing the makespan. This is similar to our problem on a signle contention point, as in star shaped networks. Nevertheless, there is still the periodicity constraint to take into consideration. In this thesis, we propose several FPT algorithms based on this single processor scheduling algorithm.
