...
\todo{Yann: la phrase précédente est bancale: quel rapport entre flexibility et low latency}
...
, it can be restricted to the radio part and the digital to analog conversion only. This can be identified by different names depending on the reference documents, including \textbb{RU} for Remote Unit or \textbb{RRH} for Remote Radio Heads. The latter will be used in the thesis. The other components of the C-RAN  are the processing units. One can distinguish two levels of processing units: the \texttbb{DUs}, for Distributed Units that are able to ensure a part of the computation tasks, and the \textbb{CUs} for Centralized Units that computes the most centralized tasks. In this thesis, we consider only CUs, that are also called \textbb{BBUs} for BaseBand Units, and we use this term in the thesis. Those BBUs are located in the cloud. 
...
\todo{la phrase écédente a un gros probleme} The execution may be indifferently performed on virtualized machines, or bare metal, or any combination of the two. The network between RRH and BBU is called “Fronthaul Network”, or “Fronthaul” for short. Figure~\ref{fig:fronthaul} illustrates an example of fronthaul in which several BBU are gathered in the same datacenter. 
  ...
  \begin{figure}[h]
      ...
      \todo{est ce qu'il ne faut pas dire qu'avec un split moins exigeant notre travail reste pertinent, mais c'est juste moins critique}
