...
Statistical multiplexing is the most common mechanism for packet based networks in the last $40$ years. While IETF tools~\cite{metricsietf} ensure a latency lower than a given value for $95\%$ of the packets, such a guarantee it is not enough to our context since it can not satisfy latency constraints fo all packets. While some mechanisms can be used to prioritize some packets over the others (e.g. Express Forwarding~\cite{exprforw} against Best Effort), they fail to guarantee the delivery of a given packet in a given time delay when several packets compete for the same resource. 
...
%est ce que c'est vraiment le même problème ? Si oui, notre travail ne sert à rien, mais ça n'est pas le cas.
