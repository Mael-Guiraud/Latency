...
Even if the performance are excellent on practical instances, the algorithm we propose here focuses on solving the problem \wta when the offsets are set, but not the entire : problem \pall. Furthermore, neither \greedydeadline, \PMLS nor \ASPMLShe algorithm are easily adaptable for more complex topologies than star routed networks, studied in next chapter for a synchronized version of \pall.
