...
\section{Periodic Assignment and Random Traffic on Star Routed Networks}\label{sec:comparison}
    ...
    Lemma~\ref{lemma:smaller_tau} gives a way to obtain a solution to the original instance from the instance with a larger message size as illustrated in Figure~\ref{fig:space}, whith the additional property that all datagrams are separated
    ...
    we can do so by a dichotomic search.
...
Les schémas déterministent respectent et aident
 les traffics randomisés si on leur laisse de la place régulièrement !