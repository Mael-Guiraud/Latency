...
Deterministic Networking are mentionned in the same survey cited ahead for TSN~\cite{8458130}. The researches about DetNet are untill now limited either to linear programming for finding deterministic policies for the network -as mentionned above- or traffic shaping (see~\cite{7092358} for a comparison of actual traffic shaping methods). Traffic shaping methods are based on stochastic models, with bounds on the arrival of the flows, that allows to bound the maximal latency. Nevertheless, traffic shaping do not propose a deterministic management of deterministic flows in order to ensure a minimal latency and $0$ jitter.
...
To synchronize clocks, Hyper-TSN considers the flow as a reference~\cite{leclerc2020optical} as we now explain. Since the exact arrival time of a datagram is known, if it arrives before or after the expected date, this means the physical delay between the sender of the datagram and the switch known by the controller is false. The monitoring circuitry of the switch detects this issue and an alert signal is sent to the controller that is able to reschedule correctly the GCL.
