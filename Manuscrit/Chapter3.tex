...
\end{proof}
 ...
 % We define two problems, \pall and it's variant \pazl, of finding an assignment on a class of routed networks: star routed networks for which every route shares the same contention vertices. We showed that \pazl and \pall are $\NP$-Hard, even for routed network of small contention depth or contention width, but not for star routed networks.
  ...
  We model C-RAN networks by weighted directed acyclic multigraphs called routed networks. The end vertices of the routed network represent either the sources or destinations of messages sent in the network, while the inner vertices represent the contention point of the network, that is, a ressource shared between at least two routes. A route is a sequence of vertices with one source, one or several contention vertices and one destination. An arc $(u,v)$ between two vertices belongs to only one route, and its weight correspond to the physical time taken by a message to go from $u$ to $v$. When a message of a C-RAN networks arrives in the BBU, an answer is computed and one can choose the date at which it is emitted. 
