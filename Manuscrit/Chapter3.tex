...
\end{proof}
  ...
  \section*{Conclusion}
  We model C-RAN networks by weighted directed acyclic multigraphs called routed networks and denoted by $N$. The end vertices of the routed network represent either the sources or destinations of messages sent in the network, while the inner vertices represent the contention point of the network, that is, a ressource shared between at least two routes. A route is a sequence of vertices with one source, one or several contention vertices and one destionation. An arc $(u,v)$ between two vertices belongs to only one route, and its weight is given by the function $\omega$ correspond to the physical time taken by a message to go from $u$ to $v$. The set of routes is denoted $\cal{R}$. The messages are allowed to be buffered in one contention vertex of it's route. The set of contention vertices in which the buffering is allowed is denoted $\cal{B}$. The triplet $(\cal{R},\,\cal{B},\,\omega)$ defines the routed network. 
