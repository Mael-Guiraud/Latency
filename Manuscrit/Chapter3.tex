...
\end{proof}
 ...
 % We define two problems, \pall and it's variant \pazl, of finding an assignment on a class of routed networks: star routed networks for which every route shares the same contention vertices. We showed that \pazl and \pall are $\NP$-Hard, even for routed network of small contention depth or contention width, but not for star routed networks.
  ...
  We model C-RAN networks by weighted directed acyclic multigraphs called routed networks. The end vertices of the routed network represent either the sources or targets of messages sent in the network, while the inner vertices represent the contention point of the network, that is, a ressource shared between at least two routes. A route is a sequence of vertices with one source, one or several contention vertices and one targets. An arc $(u,v)$ between two vertices belongs to only one route, and its weight correspond to the physical time taken by a message to go from $u$ to $v$. The time is expressed in tics, corresponding the the switching time of a unit of data in physical networks. When a message of a C-RAN network arrives in the BBU, an answer is computed and one can choose the date at which it is emitted by the BBU. 

The process we study is Periodic. Messages are sent by the sources of the routes at every period. Once a message have been sent, it cannot be buffered in the network. Thus, we aim to remove contention in the network. To do so, we compute the offset (emission time) and the waiting time (buffering in the BBU) of the messages such that two messages never collide. The function that associates to each route an offset and a waiting time is called an assignment. An assignment is computed for one period, and repeated all over the time for every periods. The value of an assignment $A$ is denoted $TR(A)$ and correspond to the largest value on all routes of the time elapsed between the sending of a message and the reception of the answer in the RRH.
