...
\begin{figure}[h]
       ...
       \begin{center}
      \includegraphics[width=0.6\textwidth]{Chapitre4PAZL/echec_longues.pdf}
      \end{center}
...
We now want to experiment to performance of our algorithm when the number of routes grows.
We experiment with several periods and message sizes. For each set of parameters, we try every possible load by changing the number of messages and give the success rate of each algorithm. Notice that all algorithm exept \ESCA are polynomial time algorithms but are not always able to find a solution, depending on the load or the size of the routes. On the other hand, \ESCA finds a solution if it exists, but works in exponential time in $n$.
 The success rate is measured on $10000$ instances of \pma generated by drawing uniformly and independently the delays of each message in $[P]$. 
