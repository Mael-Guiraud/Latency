...
We show in the experiments of Section~\ref{sec:perf_large}, that \pazl can be very often solved positively, in particular for short routes and when the load is moderate.
  ...
  \subsection{Shortest-Longest policy}
    ...
    The complexity of the algorithm is dominated by the sorting of the routes in $O(n\log(n))$. 
      ...
      If the period is slightly smaller that the bound of Proposition~\ref{prop:SL}, there is a collision with $r_0$ on $c_1$. Hence, this policy is not useful as a heuristic for longer routes, as confirmed by the experimental results of Section~\ref{sec:perf_large}. 
