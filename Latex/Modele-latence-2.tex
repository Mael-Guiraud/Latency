...
The first case is the easyest to solve. Indeed, it is obvious that if we schedule all messages following eachothers from the one with the longer route,
to the one with the shortest one, we will obtain an optimal $T_{max}$ : All the message will cross the first switch, not collisioning each others, then 
they will not be another conflict point between all routes.

This solution is optimal because any message will wait on source nodes, so $\omega(r) = 0$.
