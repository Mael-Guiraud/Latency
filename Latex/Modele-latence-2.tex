...
\todo{solution optimale?)

The third case is the harder to solve. We offer the following heuristic :
\begin{itemize}
 \item For all routes take the longer route not allocated yet, and give it the offset 0.
 \item Look at the message that can come back to source switch the first, and give it the waiting time 0.
 \item For all following routes, take the ones which can emit, and give the longer one the minimal waiting time such that it's message will not
 collide the previous one.
\end{itemize}
