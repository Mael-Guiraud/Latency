...
We define the time slot used by a route $r_i$ at any vertex $v$ of the route by $t(v,r_i) = m_i+\lambda(u,r_i)) \mod P$.
 ...
 \todo{On devrait peut-être plutôt définire $\lambda(u,r_i) - \lambda(u,r_j)$ comme un objet qui sera ensuite les poids 
...
\todo{Est-ce qu'on peut comparer ce problème au jeu spacechem ? peut-être queque'un a déjà fait une preuve sur ce jeu.}
