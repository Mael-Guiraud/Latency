...
where ${\Omega} = (\omega_0,...,\omega_{{\cal L}-1}) $ is $P$-periodic affectation of N. This means that $\forall i,j,x_i \notin [x_j,...,x_j+T]$, to avoid collisions
...
$T_{max}$ : All the message will cross the first switch, without collisions, then 
...
It is alwas possible when the windows $P$ is large enough(for example: send all messages one by one, waiting the previous one to be back),
...
\begin{itemize}
 \item l1 is sent before l2 : the message from l1 will leave A after 11 slots. Let us call X the minimal time slot in which l2 can emit. 
 To avoid collisions with the message from l1 in A , $X\ge1$.
 In the other way, the message from l1 will use B from time slot 21 to 21+10. The route from l2 is 12 units long from l2 to B. So, to avoid collisions with 
 the message from l1 in B $X+12 \ge 21+10 \rightarrow x\ge 19$. Then if we take $X=19$, our time
 windows is at least 29 slots(19 + 10 corresponding to the time to send the message)
 \item l2 is sent before l1: at the first node A, l2 will have to wait 19 time slots to send his message, so the windows will be at lest 29 too.
\end{itemize}
...
So the smaller time windows is 12, 1/3 smaller than best solution without waiting times.
...
Given this simplification, case 2 and 3 are the same.
...
route to use those time slots.
\todo{notations de la preuve pour expliquer l'algo}

From now, the period P is set and waiting times are allowed. The following heuristic is suggested :

\begin{algorithm}
\caption{Longest to shortest with waiting times}
\begin{algorithmic}
\REQUIRE Matched graph N, window size $P$
\ENSURE 2way Trip affectation in P
\STATE offset $\leftarrow$ 0
\FORALL{route i in $\rho$ from the longest to the shortest }
\STATE  $m_i \leftarrow$ offset
\STATE offset $\leftarrow$ offset + 2500
\ENDFOR
\STATE offset $\leftarrow$ 0
\STATE take i such that $r_i$ is the first route to come back in sources node
\STATE $w_i \leftarrow $ offset;
\STATE offset $\leftarrow$ $a_i$ + 2500
...
\todo{prouver bornes expliquer algo}
