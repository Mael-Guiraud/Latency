...
The operators are searching for a way to dissociate BBU from RRH, and regroup many BBU in some data centers, that would allow easier updates and maintenance.
...
In this article, we will work on latency constraints in the fronthaul.
...
Each arc  $(u,v)$ in $A$ is has an integer weight $Dl(u,v) \geq 1$ representing the time taken by the signal to go from $u$ to $v$ by using this arc.  
...
\noindent {\bf Input:} matched graphs $N$, integer $P$, $ T_{max}$, $\theta$.

\noindent {\bf Question:} does there exist a 2-way-trip affectation of $N$, such that $\forall r \in \rho$, $T(r) \le T_{max}$.

\noindent {\bf Optimization goal 1:} minimizing $\sum_{r \in \rho}  T(r)$.\\
