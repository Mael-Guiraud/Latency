...
int * simons_FPT(PreElems E, int taille_paquet, int periode, int premier,int * subset)
{	
	
	///////////////////////////////////////////////////////taille_paquet = 6;
	
	int nbr_route = E.nb_elems;


	
	
	int * w_i = malloc(sizeof(int)*nbr_route);
	int fin[nbr_route];
	...
	for(int i=0;i<nbr_route;i++)
	{
		...
		release[i] = E.release[i];
		arrivee[i] = E.release[i];
		deadline[i] =  E.deadline[i];
	}
...
int * rec_FPT(PreElems E, int taille_paquet, int periode, int premier, int* subset,int * candidats, int profondeur, int nbr_candidats)
{
	...
	if(profondeur == nbr_candidats)
	{
		...
		int routes[E.nb_elems];
		for(int i=0;i<E.nb_elems;i++)
			routes[i] = 0;
		for(int i=0;i<nbr_candidats;i++)
			if(subset[i])
				routes[candidats[i]]=1;
		test= simons_FPT(E,taille_paquet,periode,premier,routes);
		return test;
	}
	else
	{
		...
		subset[profondeur] = 0;
		//affiche_tab(subset,nbr_candidats);
		val_D =  rec_FPT(E,taille_paquet,periode,premier,subset,candidats,profondeur+1,nbr_candidats);
		
		if(val_D != NULL)// SI on ne trouve pas on essaye la branche soeur
			return val_D;
		//printf("PArcours a gauche %d\n",profondeur);
		subset[profondeur]= 1;
		//affiche_tab(subset,nbr_candidats);
		val_G =  rec_FPT(E,taille_paquet,periode,premier,subset,candidats,profondeur+1,nbr_candidats); 
		return val_G;
...
{	
	...
	for(int premier=0;premier<nbr_route;premier++)
	{
		...
		res = rec_FPT(E,taille_paquet,periode,premier,subset,candidats,0,nbr_candidats);
