...
void print_assignment(Graph g, Assignment a, int p){
	for(int i=0;i<g.arc_pool_size;i++){g.arc_pool[i].seen=0;}
	FILE* f;
	int vertex_id ;
	int previous_end ;
	...
	if( !(f = fopen("../view/view_assign.dot","w")) )
	{
		perror("Opening dot file failure\n");exit(2);
	}
	...
	for(int i=0;i<g.nb_bbu+g.nb_collisions;i++)
	{
		...
		if(i<g.nb_bbu)
		{
			fprintf(f,"%d [shape = \"none\",label=\"%d\",image=\"dc.png\"]\n",vertex_id+1,i);	
		}
		fprintf(f,"%d -- %d [label = \"%d\"]\n",vertex_id,vertex_id+1,g.arc_pool[i].length);
		vertex_id+=2;
	}
