...
Graph g;
typedef enum {
  NUL, FROM, TO, LENGTH, OBJ, ENDOBJ, PATH, ARRAY
} last_string;
last_string last;
...
static int dump(const char *js, jsmntok_t *t, size_t count, int indent) {
  ...
  if (t->type == JSMN_PRIMITIVE) {
    ...
    {
      g.arc_pool_size++;
      if(g.arc_pool_size == 0)
        g.arc_pool = malloc(sizeof(Arc));
      else
...
static int dumppath(const char *js, jsmntok_t *t, size_t count, int indent) {
  ...
  if (t->type == JSMN_PRIMITIVE) {
    ...
    if(last == ARRAY)
    {
      ...
      g.routes[g.nb_routes-1][g.size_routes[g.nb_routes-1]] = &g.arc_pool[val];
    }
    perror("This should not happend.\n ");
    exit(26);
    ...
    if(last!= NUL)
    {
      ...
      g.size_routes = realloc(g.routes,sizeof(int)*g.nb_routes);
